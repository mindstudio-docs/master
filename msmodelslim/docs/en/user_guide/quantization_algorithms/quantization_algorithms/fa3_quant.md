...
1. **Quantization objective**: Perform quantization on the `Q`, `K`, and `V` activation values within the attention mechanism.
2. **Quantization granularity**: INT8 or FP8.
...
4. **Quantization strategy**: `per-head`, which improves accuracy by using independent quantization parameters calculated for each attention head.
  ...
  - `per-head` – Static quantization; computes quantization parameters independently per attention head to accommodate varying activation distributions across heads.
...
- FA3 quantization is implemented in [processor.py](../../../../../msmodelslim/processor/quant/fa3/processor.py). The processing flow consists of three phases.
...
- **Quantization mode constraints**
  - Currently supports INT8/FP8 static symmetric quantization and FP8 dynamic quantization.
...
| ------- | ------------------ | ------------- | ------ | -------------------------------------------------- |
| `type`    | Processor type identifier.    | string        | -      | The value is fixed to `fa3_quant`, which identifies the object as the FA3 quantization processor.|
| `qconfig` | Unified processor quantization configuration | `string` | `"int8-per_head"` | Specifies the quantization configuration. |
| `include` | Attention layers to include | `array[string]` | `["*"]` | Supports wildcard matching; specifies which attention layers to quantize with FA3. |
| `exclude` | Attention layers to exclude | `array[string]` | `[]` | Supports wildcard matching; takes precedence over `include`. |
| `details` | Detailed processor quantization configuration | `object` | `[]` | Specifies quantization settings. |

**Note**
Each layer's quantization method is determined by `quant_type`, formatted as follows:
{activation1}\_{precision1}\_{strategy1}\_{activation2}\_{precision2}\_{strategy2}

1. Activations with the same `{precision}_{strategy}` are merged under a single `{activation}` name.
2. If the `{activation}` prefix is omitted, QKV uses the same quantization method. For example, `FP8_DYNAMIC` is equivalent to `QKV_FP8_DYNAMIC`.
3. If `fa_quant_type` is present but `quant_type` is not, the default is `QKV_INT8`.
4. For dynamic quantization (`DYNAMIC`), `scale` and `offset` do not apply to the corresponding activations.
5. `qconfig` and `details` fields cannot be configured simultaneously.

**`quant_type` Examples**

- `Q_INT8_K_FP8_DYNAMIC_V_FP8` – Q uses INT8 static quantization; K uses FP8 dynamic quantization; V uses FP8 static quantization.
- `Q_FP8_DYNAMIC_KV_FP8` – Q uses FP8 dynamic quantization (no `fa_q.scale`/`offset` fields); K and V use FP8 static quantization.
- `FP8_DYNAMIC` – Q, K, and V all use FP8 dynamic quantization.
...
- **Procedure**
  For details, see the implementation of [model_adapter.py](../../../../../msmodelslim/model/deepseek_v3/model_adapter.py) for DeepSeek.
