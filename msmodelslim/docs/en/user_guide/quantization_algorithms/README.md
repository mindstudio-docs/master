...
## Sensitive Layer Analysis Algorithms

Sensitive layer analysis is performed via `msmodelslim analyze` to measure the sensitivity of each layer or substructure to quantization on calibration data. The results are ranked to assist with fallback strategies and YAML parameter tuning.

| Algorithm | Scope | Core Idea | Use Case | Details |
| :--- | :--- | :--- | :--- | :--- |
| **Std** | Linear | Sensitivity = activation dynamic range/standard deviation | Coarse screening of linear layers before quantization; one of the default strategies | [Details](sensitive_layer_analysis/std.md) |
| **Quantile** | Linear | Using quantiles and IQR to construct a score; relatively robust to outliers | Heavy-tailed activations; reducing outlier dominance | [Details](sensitive_layer_analysis/quantile.md) |
| **Kurtosis** | Linear | Estimating activation kurtosis to capture the impact of peaks and extremes | Distributions with sharp peaks; fallback or mixed‑precision tuning | [Details](sensitive_layer_analysis/kurtosis.md) |
| **Attention MSE (`mse`)** | Attention structures | MSE between attention outputs under floating‑point and quantized weights | Sensitivity of attention weights to quantization (requires adapter interface) | [Details](sensitive_layer_analysis/attention_mse.md) |
| **Layer‑wise MSE (`mse_layer_wise`)** | Decoder blocks | Mean MSE across selected submodules within each block | Fallback for entire layers or blocks (e.g., MLP or attention segments) | [Details](sensitive_layer_analysis/mse_layer_wise.md) |
| **Model‑wise MSE (`mse_model_wise`)** | Layers (chained forward) | MSE of **final model output** under layer‑by‑layer quantization perturbations | Layer sensitivity from the perspective of final hidden states | [Details](sensitive_layer_analysis/mse_model_wise.md) |
...
- **Sensitive layer analysis and fallback**: Before finalizing the YAML, use [Sensitive Layer Analysis](../feature_guide/sensitive_layer_analysis/usage.md) with the metrics above to rank layers or structures. For `linear` layers, **Kurtosis** is recommended; for `layer`-level analysis, **mse_layer_wise** is preferred.
