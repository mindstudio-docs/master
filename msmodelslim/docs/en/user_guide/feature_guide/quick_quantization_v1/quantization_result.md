...
| `{model_type}_best_practice.yaml` | **Quantization configuration protocol file**, which records the complete configuration parameters used during execution. For details, see [Quantization Configuration Protocol](usage.md#5-quantization-configuration-protocol).|
...
| `optional/quarot.safetensors` | **QuaRot global rotation matrix file**, which is generated only when QuaRot is used with `export_extra_info` set to `True`. It stores the global rotation matrix `Q`. For details, see [QuaRot - Rotation Quantization](#5-quarot---rotation-quantization).|
| `debug_info/` | **Debug information directory** (generated only when the `--debug` option is specified), which contains the quantization context for troubleshooting and algorithm analysis. For details, see [Debug Information Output](#6-debug-information-output).|

## 3. `quant_model_description.json`
...
#### 3.2.2 Quantization Type Enumerated Values
...
#### 3.2.3 Weight Tensor Quantization Type
...
> [!TIP]
> For detailed formula derivations, parameter semantics, and comprehensive mapping tables for each quantization mode, refer to the [AscendV1 Format Specification](../../quantization_formats/ascendv1.md). This document focuses on a high-level overview of output files and a quick-reference for parameter structures.
 
### 4.1 Quantization Mode Parameters

The following content describes the parameter structure of each quantization mode based on the `AscendV1Saver` implementation. For details, see the [AscendV1 Format Specification](../../quantization_formats/ascendv1.md).

#### 4.1.1 FLOAT (Unquantized)
...
#### 4.1.2 W16A16S (Sparse Quantization)
...
#### 4.1.3 W8A8 (Static Quantization)
...
#### 4.1.4 W8A8_DYNAMIC (Dynamic Quantization)
...
#### 4.1.5 W8A8_MIX (Mixed Quantization)
...
#### 4.1.6 W8A16 (Weight Requantization)
...
#### 4.1.7 W4A4_DYNAMIC (W4A4 Dynamic Quantization)
...
#### 4.1.8 WFP8AFP8_DYNAMIC (FP8 Dynamic Quantization)
...
#### 4.1.9 W8A8_MXFP8 (MXFP8 Quantization)
...
#### 4.1.10 W4A8_MXFP (W4A8 MXFP Quantization)
...
#### 4.1.11 W4A4_MXFP4 (W4A4 MXFP4 Quantization)
...
#### 4.1.12 C8 (KV Cache Quantization)
...
#### 4.1.13 W4A8_DYNAMIC (W4A8 Dynamic Quantization)
...
#### 4.1.14 FlatQuant_DYNAMIC (FlatQuant Dynamic Quantization)
...
#### 4.1.15 NonFusionSmoothQuant (Smooth Quantization)
...
#### 4.1.16 FAQuant (Flash Attention Quantization)
...
## 5. QuaRot - Rotation Quantization
...
#### 5.2.2 Description Fields in `quant_model_description.json`
...
### 6.1 Debug Information Directory Structure
