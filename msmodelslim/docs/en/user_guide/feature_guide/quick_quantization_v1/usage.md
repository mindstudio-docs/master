...
| device_id         | Specifies the quantization device indices.     | No               | 1. Type: `list[int]`, separated by spaces, e.g. `0` or `0 1 2 3`.<br>2. If multiple device indices are specified (such as `0 1 2 3`), the system initiates data parallelism (DP) layer-wise quantization. Ensure the specified algorithm supports distributed execution. For details, see [Layer-wise and DP Layer-wise Quantization](#41-layer-wise-and-dp-layer-wise-quantization).                                                                                                                                                                                                             |
