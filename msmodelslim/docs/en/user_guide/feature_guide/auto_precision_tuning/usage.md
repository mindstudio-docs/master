...
| device            | Specifies the quantization device.     | No               | 1. Type: `str`.<br>2. Example values: `'npu'`, `'npu:0,1,2,3'`, `'cpu'`<br>3. Default value: `'npu'` (single device).<br>4. When multiple devices are specified (such as, `'npu:0,1,2,3'`), the system can start distributed layer-wise quantization (DP layer-wise). For details about the algorithm support scope and configuration method, see [Quick Quantization Usage](../quick_quantization_v1/usage.md#41-layer-wise-and-dp-layer-wise-quantization).|
