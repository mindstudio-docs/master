...
Based on experience, W8A8 dynamic quantization usually causes only a small accuracy loss, so it does not need an outlier suppression algorithm and rarely requires fallback. Therefore, in the scenario example, we only need to support quantization scheduling. We do not need outlier quantization, sensitive-layer analysis, or other extra functions. If you need to integrate other algorithms, refer to [`Algorithm Overview`](../user_guide/quantization_algorithms/README.md)
...
## Auto-Tuning and Sensitive Layer Analysis

When using the [Auto-Tuning User Guide](../user_guide/feature_guide/auto_precision_tuning/usage.md) and the strategy requires **automatic fallback candidate generation** (`standing_high` always; `binary_fallback` when `rollback_candidates` is not configured as a non-empty list; `standing_high_with_experience` delegates to Standing High), the model adapter must implement **`ModelSlimPipelineInterfaceV1`** as follows:

```python
from msmodelslim.model.interface_hub import ModelSlimPipelineInterfaceV1
```

This interface shares the same model protocol as the CLI command **`msmodelslim analyze`** and `PipelineAnalysisService`. It requires implementing methods such as `init_model`, `handle_dataset`, `generate_model_visit`, and `generate_model_forward`. Within the tuning strategy, `PipelineAnalysisService` invokes these methods; **the strategy side does not pre‑load the model via `load_model`**.

| Tuning Strategy | Sensitive Layer Analysis | Additional Interface |
|-----------------|--------------------------|-----------------------|
| `standing_high` | Always runs automatically. | None |
| `binary_fallback` | Skips when a non‑empty. `rollback_candidates` is configured | None |
| `standing_high_with_experience` | Delegates to Standing High. | **`StandingHighWithExperienceInterface`** (requires `load_model` for outlier suppression capability detection); **`ModelSlimPipelineInterfaceV1` must be inherited separately** |

For details, refer to the [Auto-Tuning Configuration Protocol Guide](../user_guide/feature_guide/auto_precision_tuning/configuration_protocols.md) and the algorithm documentation for each strategy.
...
### Executing One-click Quantization
...
- For details about the usage and parameters, see [Quick Quantization Guide](../user_guide/feature_guide/quick_quantization_v1/usage.md).
