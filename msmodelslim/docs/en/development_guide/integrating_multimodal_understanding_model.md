...
You are advised to create a separate directory in [`msmodelslim/model/`](../../../msmodelslim/model), such as `qwen3_vl_moe/`, with the following files:
...
Calibration data is specified by the YAML `dataset` field. `dataset` can be configured as a short identifier (can be found in [`lab_calib`](../../../lab_calib)), an absolute path, or a relative path. Three usage patterns are supported: `index.json` or `index.jsonl`, a directory that contains only images, and an image directory plus a single `json` or `jsonl` file. For details, see [Quick Quantization Guide: `dataset` - Calibration Data Path Configuration](../user_guide/feature_guide/quick_quantization_v1/usage.md#dataset---calibration-data-path-configuration).
...
For detailed descriptions of each field in the configuration, see [Quantization Configuration Protocol](../user_guide/feature_guide/quick_quantization_v1/usage.md#54-multimodal_vlm_modelslim_v1-configuration).
...
For details, see: [Iterative Smooth Adaptation](../user_guide/quantization_algorithms/outlier_suppression_algorithms/iterative_smooth.md#model-adaptation)
...
For details, see [QuaRot Adaptation](../user_guide/quantization_algorithms/outlier_suppression_algorithms/quarot.md#model-adaptation).
...
- [Quick Quantization Guide](../user_guide/feature_guide/quick_quantization_v1/usage.md): CLI parameter details
