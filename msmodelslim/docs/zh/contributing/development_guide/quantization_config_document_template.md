...
>
> 当子配置类型数量较少（建议不超过 5 个）时，直接将每个子类型作为一行填入下表。当子配置类型较多时，下表仅保留按字段路径分组的汇总行（“type 或辨识值”列填写“见下方索引”），并在下方增加子配置索引表（如“处理器索引”“保存格式索引”），索引表只包含 type 和配置文档等变化列，避免在汇总表中大量重复相同字段值。
