...
> **注释：** 填写案例时替换为指向对应流程指南的超链接，不在此重复流程规范。
...
> **注释：** 产品形态填写实际硬件形态（如Atlas 800I A2产品），并说明是否限定（限定 / 不限定；限定时写明适用边界）。依赖项按需增删；若多项互为替代方案（如多种推理框架），仅保留本次实际使用的项。版本需求可先填写版本号，待该版本正式上线后替换为文档或发布页链接；其余项给出当前使用的版本链接。
...
**输出**：{{step_output}}  
**记录**：{{execution_record}}  
**[OPTIONAL] 下一步**：{{next_step}}

> **注释：** 按执行顺序复制本节，未覆盖的步骤删除。操作须可执行（命令与配置放入代码块）；输出与记录须可追溯。「下一步」为可选项，不需要则整行删除。执行前检查、通过条件、失败处置见关联流程，此处不重复。
