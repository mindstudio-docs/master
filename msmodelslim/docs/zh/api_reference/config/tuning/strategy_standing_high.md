...
| `anti_outlier_strategies` | `list[list[object]]` | 必选 | 无 | — | 离群值抑制处理器链列表，至少 1 个；每个元素是一条处理器链，链内每个处理器是 `type` 分派的处理器配置（如 smooth_quant 等） | 本页 <a href="#2-2-autoprocessorconfig">§2.2</a> |
