...
|----------|------|-----------|--------|----------------|------|----------|
| `apiversion` | `string` | 可选 | `multimodal_sd_modelslim_v1` | `multimodal_sd_modelslim_v1` | — | 无 |
...
| `save` | `list[object]` | 可选 | `[]` | — | 保存格式列表，每个元素是 `type` 分派的保存格式配置。 | 本页 <a href="#2-2-3-quantformatconfig">§2.2.3</a> |
| `dataset` | `string` | 可选 | `mix_calib.jsonl` | — | 校准数据集名称（`lab_calib` 下的文件名）或数据集路径。 | 无 |
| `multimodal_sd_config` | `object` | 可选 | 由工厂函数生成 | — | 多模态生成模型的专用配置，可为字典或 `MultimodalSDConfig` 实例，含 `dump_config` 与 `inference_config`。 | 本页 <a href="#2-2-4-multimodal-sd-config">§2.2.4</a> |

**配置约束**

- 本协议不支持 `adapt_rotation`：量化服务不执行 `spec.prior`，且多模态生成适配器未实现 `AdaptRotationInterface`；请保持 `prior` 为空，且勿在 `process`/`per_expert` 中配置 `adapt_rotation`。
...
<h4 id="2-2-4-multimodal-sd-config">2.2.4 MultimodalSDConfig</h4>

多模态生成（SD）模型的专用配置，含 dump 与推理参数。

| 字段路径 | 类型 | 必选/可选 | 默认值 | 取值范围或格式 | 含义 | 引用配置 |
|----------|------|-----------|--------|----------------|------|----------|
| `dump_config` | `object` | 必选 | 无 | — | 浮点 dump 配置，必选。 | 本页 <a href="#2-2-5-dump-config">§2.2.5</a> |
| `inference_config` | `object / null` | 可选 | `null` | — | 推理参数字典。字段形状由当前模型适配器的 `get_inference_config_class()` 返回的 InferenceConfig 决定，量化时用该类做校验；与迁移期字段 `model_config` 互斥。 | 无 |

**配置约束**

- 校验 inference_config 与迁移期字段 model_config 互斥：两者同时提供时报错。

<h4 id="2-2-5-dump-config">2.2.5 DumpConfig</h4>
...
| `dump_data_dir` | `string` | 可选 | `` | — | 浮点 dump 数据的输出目录；为空时回退到本次量化的权重保存目录 `save_path`。 | 无 |
