...
| `metrics` | `string` | 可选 | `mse_layer_wise` | — | 分析指标 `mse_layer_wise`。 | 无 |
...
| `BinaryOperatorLayerWiseProcessorConfig` | `binary_operator_layer_wise` | 逐层敏感度分析处理器配置（对比逐块浮点与量化输出）。 | 本页 <a href="#2-1-binary-operator-layer-wise">§2.1</a> |
...
| `GroupProcessorConfig` | `group` | 处理器合并器配置。 | 《[group 配置说明](group.md)》 |
