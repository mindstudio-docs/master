...
| 2  | msModelSlim | 多进程上下文共享 | 单机本地 | 不涉及（Unix Domain Socket） |        | 单机本地 | 不涉及（Unix Domain Socket） | 不涉及（使用 Linux 抽象命名空间） | Unix Domain Socket | 通过 Unix Domain Socket 进行多进程间上下文数据共享，默认使用 Linux 抽象命名空间（`@peercred_manager_<pid>_<timestamp>`），不在文件系统创建 socket 文件。基于 SO_PEERCRED 进行内核级 UID 身份认证，仅允许同 UID 进程连接 | 不涉及 | 抽象 socket 名称可通过代码指定 | SO_PEERCRED内核级UID认证 | 不涉及（本地通信） | 业务面 | master | 仅Linux平台 |      |
