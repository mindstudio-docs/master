...
本指南面向需要通过 `msmodelslim analyze layer` 命令行，按 **Decoder 层级**（或整块 Attention、MLP、MoE）粒度识别量化敏感层，并据此调整量化 YAML（回退或提位宽）的开发者与算法工程师。
...
| 可选指标 | 适用说明 | 算法说明 | 推荐 |
| --- | --- | --- | --- |
| `mse_layer_wise` | 按 Decoder 块内子模块输出 MSE 评估层敏感度，显存更友好 | 《[层级 MSE](../knowledge_base/quantization_algorithms/mse_layer_wise/term_mse_layer_wise.md)》 | 默认首选 |
| `mse_model_wise` | 按模型最终输出 MSE 评估层敏感度，显存占用更大 | 《[模型级 MSE](../knowledge_base/quantization_algorithms/mse_model_wise/term_mse_model_wise.md)》 | 需最终输出视角时改用 |
...
2. 检查 YAML 片段中的层名或通配符是否覆盖预期范围。若分析时通过 `--quant_modules` 指定了子模块（如 `"*mlp*"`），日志中 `Patterns analyzed` 会变为对应列表，但**排序与 YAML 片段仍按 Decoder 整层打印**，不会自动收窄到所选子结构；编写量化配置时请自行把层名补全为实际结构名称（如 `model.layers.N.mlp.*`），或用 `include`、`exclude` 圈定与 `--quant_modules` 一致的量化范围。
3. 将调整后的层名写入量化配置（通常写入 `linear_quant` 的 `exclude`，或收窄 `include`）；低比特方案下也可仅对高敏感块或模块单独提位宽。
4. 用更新后的 YAML 通过 `msmodelslim quant` 命令行执行模型量化，再按业务口径测评；未达标则扩大回退或更换 `${METRICS}` 后重跑本流程。

**输出**：已根据敏感层结果更新的量化配置及文件所在路径。

**通过条件**：YAML 中回退或提位宽层名与分析结果及实际量化范围一致，且可被量化配置加载。
...
| 显存不足或运行失败 | 缩小校准集、优先使用 `mse_layer_wise`，或换更大显存设备后，再执行分析命令；`mse_model_wise` 显存压力更大宜慎用 |
| 回退后仍无法部署或精度异常 | 核对通配符是否过宽或过窄，以及推理引擎对回退层数的限制；修正配置后重新量化并测评 |
...
| 接口或能力 | 简述 | 链接 |
| --- | --- | --- |
| `msmodelslim analyze layer` | Decoder 层级敏感层分析命令行入口 | 《[msmodelslim analyze 命令行 API](../api_reference/cli/msmodelslim_analyze.md)》；本指南 [命令行预览](#命令行预览) |
