...
| 输入 | 量化导出件 | 量化权重保存目录 | 导出件完整 | 可被伪量化推理功能加载 |
| 交付件 | 异常定位报告 | 定位结论归档目录 | 含伪量化推理定界结论、异常位置候选分布对比、首个生成 token 对比、逐层 cosine similarity 对比、异常层结论与处置建议 | 报告可复核：同数据可复现，结论有数据支撑 |

## 4. 流程总览

先用伪量化推理定界问题归属，再遵循**从宏观到微观、从外围到核心**的递进原则定位，每一层方法均可用于对比任意两组环境（如浮点 vs 量化、框架版本 X vs Y）。

```mermaid
flowchart TD
  P[问题：模型输出异常<br>截断/回答错误/重复/乱码] --> A[步骤1 定界精度问题<br>伪量化推理复现同批 badcase]
  A --> D0{伪量化推理是否<br>同样异常?}
  D0 -- 是 --> T[按《量化精度调优指南》<br>调整量化方案后重新量化]
  D0 -- 否 --> B[步骤2 理解输入输出格式<br>Chat Template + Tokenizer]
  B --> C[步骤3 logprobs 快速确认<br>异常位置取 top-N 候选分布]
  C --> D{输出是否基本可用?<br>能提取 reasoning/content}
  D -- 是 --> E[步骤4 Completion API 控制实验<br>手动构造 prompt 精确对比]
  D -- 否 --> F[步骤5 激活值逐层对比<br>dump + compare]
  E --> G{logits 差异是否集中在特定层?}
  G -- 是 --> F
  G -- 否 --> H[分析采样/beam search 等非确定性因素]
  F --> I[步骤6 定位结论与处置<br>异常层映射与针对性优化]
  H --> I
```

> **说明**：步骤1 用于先排除量化权重本身，避免在部署侧空转排查。步骤3~5 并非必须全部执行：输出基本可用时依次执行步骤3、步骤4、步骤5；输出严重重复、大量乱码时跳过步骤4，直接进入步骤5；步骤5 是所有场景（尤其乱码场景）最主要的定位手段。

## 5. 操作步骤

### 步骤1：定界精度问题

**目标**：在与部署框架无关的伪量化推理环境中复现同批 badcase，判断偏差归属——由量化权重本身引入，还是由推理框架、算子或编译优化触发。

**操作**：

1. **准备 badcase 样本**：从测评报告或业务反馈中提取 badcase 样本。
2. **执行伪量化推理**：参考《[伪量化推理使用指南](usage_fake_quant_evaluation.md)》使用量化导出件执行伪量化推理。
3. **按样本对齐两侧输出并定界**：
   - **伪量化推理结果正常**：精度问题归属部署侧，继续步骤2 定位；
   - **伪量化推理结果同样异常**：精度问题由量化权重引入，按《[量化精度调优指南](process_quantization_precision_tuning.md)》调整量化方案后重新量化，再回到本流程复验。

**输出**：同批 badcase 的伪量化推理结果，以及偏差归属结论（量化侧 / 部署侧）。

**通过条件**：badcase 在伪量化推理中得到可判读的逐样本结果，且偏差归属方向明确。

**审计记录**：伪量化推理的执行命令、伪量化推理结果、与部署侧输出的对比记录。
...
### 步骤4：Completion API 控制实验

**目标**：用 Completion API 手动构造精确 prompt，使模型的**首个生成 token** 就是待分析的目标 token，对位比较两组环境的 logits。

**适用条件**：模型输出**基本可用**（能从中提取 reasoning 和 content 构造 prompt）。若输出存在严重重复、大量乱码（局部片段虽有意义但整体不可用于 prompt 构造），跳过本步直接进入步骤5。
...
1. **映射模型结构**：将步骤5 定位的异常层对应到具体模块——Self-Attention 的 Q/K/V 投影、MLP 的 Gate/Up/Down 投影、RMS Norm 等。
...
- **长序列累积问题**：单步 logprobs/Completion 实验无法反映累积效应，直接以步骤5 激活值逐层对比为主，并关注跨层累积、位置编码影响与 Attention 模块。
...
| 接口或能力 | 简述 | 链接 |
| --- | --- | --- |
| `msmodelslim eval` | 伪量化推理验证命令行入口，用于定界偏差归属并验证量化权重 | 《[伪量化推理使用指南](usage_fake_quant_evaluation.md)》 |
