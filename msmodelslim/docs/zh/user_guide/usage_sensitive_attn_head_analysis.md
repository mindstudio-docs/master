...
本指南面向需要通过 `msmodelslim analyze attn_head` 命令行，按 **Attention Head** 粒度识别关键注意力头（induction heads 与 echo heads），并据此生成 `head.pt` 文件用于 RA Compress 压缩的开发者与算法工程师。
...
| 交付件 | head 筛选结果 | 标准输出（induction 与 echo head 列表）及可选 `head.pt` 文件 | `head.pt` 为 Python dict 序列化的 `.pt` 文件 | 能识别预期 layer 与 head 范围，`head.pt` 可被后续量化流程加载 |
...
| 可选指标 | 适用说明 | 算法说明 | 推荐 |
| --- | --- | --- | --- |
| `ra_compress` | 识别具有 prefix matching 和 copying matching 能力的注意力头，用于 KV Cache 压缩 | RA Compress 算法：对 Q@K^T 注意力分数在特定段偏移位置取均值，按 ratio 选 top heads | 当前唯一可选指标，直接选用 |
...
**目标**：读懂标准输出中的 induction 与 echo head 筛选结果，并将 `head.pt` 用于后续 RA Compress 量化流程。
...
**通过条件**：筛选结果中的 layer 与 head 索引在模型实际结构范围内；`head.pt` 可被后续量化流程正确加载。
...
| 现象 | 处理方向 |
| --- | --- |
| 校准集 token 长度不足（< 10000） | 使用工具内置 `calib_dummy.jsonl`，或确保自定义校准集 tokenize 后总长度 ≥ 10000 后，再执行分析命令 |
...
| 分析报缺少接口或不支持 | 按步骤 3 与接口文档补齐 `RaCompressAnalysisInterface`，或改用已支持该指标的 `model_type` 后，再执行分析命令 |
| 未识别到任何 induction 或 echo heads | 检查校准集 token 长度是否满足要求，以及模型投影层命名是否与适配器配置一致；修正后重新执行分析命令 |
