...
使用 [自动调优](../feature_guide/auto_precision_tuning/usage.md) 且策略需**自动生成回退候选**时（`standing_high` 始终；`standing_high_with_experience` 委托 Standing High 执行），模型适配器须实现 **`ModelSlimPipelineInterfaceV1`**：
