...
本指南面向需要使用 [DAOS 低比特量化方案](./term_daos.md) 的用户。DAOS 是面向多模态**生成**模型的低比特联合方案。它通过串联 `oasq`（离群感知平滑）与 `trainable_linear_quant`（TLQ 可训练线性量化），用于 W4A4 / MXFP 等极低比特场景；需要进一步加速 Attention 推理时，可再配置 `fa3_quant` 做 FA 量化。

本指南说明串联顺序、阶段目标与约束。各算法的推荐起步配置与字段调参，请分别查阅《[OASQ 使用指南](../oasq/usage_oasq.md)》、《[TLQ 使用指南](../trainable_linear_quant/usage_trainable_linear_quant.md)》、《[FA3 Quant 使用指南](../fa3_quant/usage_fa3_quant.md)》，不在此重复展开。
...
> **资源提示**：DAOS 含 OASQ 平滑与 TLQ 块级训练，量化耗时与显存显著高于一次标定算法；显存是否够用通常要在启动量化后才能确认，见相关步骤。
...
| 输入 | 模型适配器 | 用户适配代码，通过 `--model_type` 调用 | 实现 `PipelineInterface` 及 OASQ/TLQ（可选 FA3）对应专用接口 | 能被调度器（Runner）与处理器（Processor）正常驱动 |
| 输入 | 校准数据集 | 任务 `dataset`、校准集或模型实践配方 | JSONL 或 JSON 格式文本 Prompt，推荐 50 条 | 能代表真实输入分布，可被适配器 `handle_dataset` 成功编码为前向张量 |
| 输入 | 量化任务配置 | 用户 YAML 或 `lab_practice` 配方 | 符合 `multimodal_sd_modelslim_v1`（或对应任务协议），可在 `spec.process` 中串联写入 | 通过模式校验（Schema Validation），能被量化流程加载 |
...
- **确定量化模式**：固定 DAOS 要落地的量化模式，包括线性层 W/A 目标位宽与数值格式、是否对 Attention 做 FA 量化、TLQ 的 OP 路径倾向。
- **适配与编写配置**：按 OASQ → TLQ → 可选 FA3 完成子图适配，并在 `spec.process` 中写入可加载的 DAOS 串联配置。
- **启动量化**：启动整链量化并跑通；若出现显存不足，优先调整资源相关参数后重试。
- **测评与迭代调参**：完成端到端测评；不达标则按对应算法使用指南单变量迭代调参，直至满足目标或明确回退策略。
