...
MinMax 作为 [`linear_quant`](../linear_quant/linear_quant.md) 处理器中激活值或权重的量化方法，通过 `qconfig.act.method` 或 `qconfig.weight.method` 字段指定，YAML配置示例如下：
...
以下字段均属于 `linear_quant` 处理器配置，完整的 `linear_quant` 字段说明请参考[线性量化配置字段详解](../linear_quant/linear_quant.md#yaml配置字段详解)。
