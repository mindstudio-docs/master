...
Iterative Smooth 迭代平滑算法。Iterative Smooth 作为离群值抑制算法，通常作为量化前的预处理步骤，用于将激活离群值迁移到权重，提升低比特量化的精度。

本指南面向首次配置 Iterative Smooth 的用户，重点不是展开完整执行命令，而是说明推荐配置为什么适合作为起点、哪些参数真正需要调，以及出现精度或资源问题时应优先改哪一项。这类算法的主要作用是先改善待量化张量的数值分布，再交给后续量化步骤处理，本身通常不是最终的量化格式。因此判断配置是否合适时，不仅要看平滑或旋转后的张量范围，还要看与后续量化组合后的端到端精度；建议保持后续量化配置不变，只调整当前算法参数。
...
| 交付件 | Iterative Smooth 参数配置方案 | 用户量化 YAML 或任务配置 | 参数取值合法、作用范围明确；关键参数说明选择依据 | 可作为后续量化流程的算法配置输入，并可复现本指南中的基线选择 |
...
```mermaid
flowchart LR
    A[确定子图范围] --> B[统计激活/权重分布]
    B[统计激活/权重分布] --> C[迭代更新平滑尺度]
    C[迭代更新平滑尺度] --> D[融合尺度]
    D[融合尺度] --> E[进入后续量化]
```
...
上面的推荐值用于建立第一版可复现基线，其中最值得关注的配置包括 `type`, `alpha`, `scale_min`, `symmetric`。推荐值并不表示所有模型都只能使用该组合，而是优先选择仓库默认值、已验证实践或较稳健的中间取值，以降低第一次使用时同时遇到精度和兼容性问题的概率。如果目标模型已经有 `lab_practice` 配方，应优先复用该配方；只有在基线精度、显存或吞吐不满足目标时，再按照下一节的参数说明逐项调整。
...
| `alpha` | 平滑迁移强度，0~1，默认 `0.9`。越大通常把更多激活离群压力迁移到权重。仓库 13 个实践中最常见值是 0.9，少数模型使用 0.5。 | 无模型配方时从默认/主流实践 `0.9` 起步；已有实践直接沿用。 | 激活量化仍受极端值主导时可偏大；若权重侧误差明显增大或低比特权重更敏感，可向 0.5 等更保守方向降低。alpha 不是越大越好，必须和后续 W/A qconfig 一起评估。 |
| `scale_min` | 平滑 scale 的最小下限，默认 `1e-5`，用于避免极小尺度引起下溢或数值放大。仓库多数实践显式使用 `1e-5`。 | 保持 `1e-5`。 | 它主要是数值稳定保护，不应作为常规精度旋钮。只有确认某些通道 scale 被下限大量截断且影响结果时才研究调整，并同时检查是否由异常校准数据造成。 |
| `symmetric` | 决定后续量化假设以及平滑是否需要 shift。源码对非对称 IterSmooth 有结构限制：当前非对称路径主要支持 `norm-linear`，其他子图会关闭 shift。 | 常规 W8A8 实践优先 `true`；仓库多数实践也是 `true`。 | 只有明确采用非对称激活/权重量化且子图能力匹配时设 `false`。若想在多种子图上统一平滑，不建议为了少量偏移分布贸然切非对称。 |
| `enable_subgraph_type` | 决定哪些结构应用 IterSmooth。仓库最常见四类全开，也有模型只启用 `norm-linear` 或 `norm-linear + up-down`。 | 先使用目标模型实践；没有配方时从默认四类开始，并通过匹配/评测确认。 | 如果非对称模式、部署融合或模型结构限制某类子图，应先收窄对应类型；不要通过修改 alpha 来补偿错误的子图匹配。 |
...
IterSmooth 的主旋钮是 alpha；`scale_min` 主要保数值稳定，`symmetric` 与子图能力有关。先确定后续量化是否对称以及子图范围，再调 alpha。
