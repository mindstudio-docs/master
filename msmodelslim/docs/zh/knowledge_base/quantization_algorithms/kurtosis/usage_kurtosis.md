...
本流程适用于在 msModelSlim 中使用 Kurtosis 敏感层分析算法。Kurtosis 作为 `msmodelslim analyze linear` 的默认 metrics 指标，用于线性层粒度的敏感度排序，识别尖峰分布与尾部极端值的影响。
...
- `layer`/`attn` 范围分析（Kurtosis 仅用于 `linear` 范围）。
...
| 术语 | 简述 | 链接 |
| --- | --- | --- |
| Kurtosis | 通过估计激活峰度衡量层敏感度的分析算法 | [Kurtosis 词条](./term_kurtosis.md) |
| 超额峰度 | 刻画相对正态尖峭程度的统计量 | [Kurtosis 词条](./term_kurtosis.md) |
...
| 接口或能力 | 简述 | 链接 |
| --- | --- | --- |
| `msmodelslim analyze linear` | 线性层敏感度分析命令，`--metrics kurtosis` 启用本算法（默认 metrics） | [Kurtosis 词条](./term_kurtosis.md) |
