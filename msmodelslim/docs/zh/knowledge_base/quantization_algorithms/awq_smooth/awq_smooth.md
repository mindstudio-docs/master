...
| `weight_qconfig`       | 权重量化配置   | AWQ 搜索阶段使用的权重量化配置。字段定义与 [线性量化算法说明](../linear_quant/linear_quant.md#yaml配置字段详解) 中 `qconfig.weight` 保持一致。 |
...
| 特性         | AWQ                                   | [Flex AWQ SSZ](../flex_awq_ssz/flex_awq_ssz.md)       |
| ------------ | ------------------------------------- | ------------------------------------- |
