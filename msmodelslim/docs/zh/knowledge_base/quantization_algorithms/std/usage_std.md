...
本流程适用于在 msModelSlim 中使用 Std 敏感层分析算法。Std 作为 `msmodelslim analyze linear` 的 metrics 指标，用于线性层粒度的敏感度粗筛与排序。
...
- `layer`/`attn` 范围分析（Std 仅用于 `linear` 范围）。
...
| 术语 | 简述 | 链接 |
| --- | --- | --- |
| Std | 用动态范围与标准差比值衡量层敏感度的分析算法 | [Std 词条](./term_std.md) |
| 敏感度分数 | 用于层间排序的 score 值 | [Std 词条](./term_std.md) |
...
| 接口或能力 | 简述 | 链接 |
| --- | --- | --- |
| `msmodelslim analyze linear` | 线性层敏感度分析命令，`--metrics std` 启用本算法 | [Std 词条](./term_std.md) |
