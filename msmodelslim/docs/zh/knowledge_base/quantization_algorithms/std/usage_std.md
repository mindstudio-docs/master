...
Std 敏感层分析算法。Std 作为 `msmodelslim analyze linear` 的 metrics 指标，用于线性层粒度的敏感度粗筛与排序。

本指南面向首次配置 Std 的用户，重点不是展开完整执行命令，而是说明推荐配置为什么适合作为起点、哪些参数真正需要调，以及出现精度或资源问题时应优先改哪一项。这类算法的输出主要用于指导哪些层、模块或注意力头需要重点保护，而不是直接生成量化权重。配置时应先固定待分析模型、分析范围和指标口径；如果指标依赖校准数据或量化结果，还应保持数据集和量化基线一致。排序或分数用于形成候选集合，最终策略仍应结合实际量化或压缩效果验证。
...
```mermaid
flowchart LR
    A[确定线性层范围] --> B[固定 metrics=std]
    B[固定 metrics=std] --> C[统计动态范围与标准差]
    C[统计动态范围与标准差] --> D[按 score 排序]
