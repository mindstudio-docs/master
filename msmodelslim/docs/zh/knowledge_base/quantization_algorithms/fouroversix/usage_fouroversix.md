...
FouroverSix 自适应块缩放量化算法。FouroverSix 作为 `linear_quant` 处理器的权重量化方法，通过 per-block 在 Scale-to-6 与 Scale-to-4 间 MSE 择优，提升 mxFP4 权重量化精度。

本指南面向首次配置 FouroverSix 的用户，重点不是展开完整执行命令，而是说明推荐配置为什么适合作为起点、哪些参数真正需要调，以及出现精度或资源问题时应优先改哪一项。这类算法通常直接影响量化尺度、舍入方式、量化粒度或低比特表示，因此参数选择会同时影响精度、压缩率以及部署兼容性。第一次使用时建议先固定目标位宽、校准集和评测方式，只采用本指南给出的推荐起点；确认基线稳定后，再围绕真正影响算法行为的参数逐项调整。
...
```mermaid
flowchart LR
    A[确定 MXFP4 per-block 方案] --> B[逐块比较 Scale-to-4/6]
    B[逐块比较 Scale-to-4/6] --> C[选择误差更小尺度]
    C[选择误差更小尺度] --> D[量化权重]
    D[量化权重] --> E[对比误差]
```
