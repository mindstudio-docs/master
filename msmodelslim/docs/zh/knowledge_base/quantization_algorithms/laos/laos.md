...
- **核心思想**：核心思想是“旋转矩阵优化 + 基于舍入偏移参数训练的低比特量化”。先通过 [Adapt Rotation](../adapt_rotation/adapt_rotation.md) 在Qwen3稠密模型上进行旋转矩阵优化（分阶段执行），实现有效的离群值抑制，再通过 [AutoRound](../autoround/autoround.md) 进行低比特量化与舍入偏移参数优化，从而提升 W4A4 场景下的精度与稳定性。
...
配置字段来自 Adapt Rotation 与 AutoRound 处理器，详见 [Adapt Rotation YAML配置字段详解](../adapt_rotation/adapt_rotation.md#yaml-配置字段详解)、[AutoRound YAML配置字段详解](../autoround/autoround.md#yaml配置字段详解)。
