...
Standing High With Experience（基于专家经验的摸高算法）是在 《[Standing High 调优算法](../standing_high/standing_high.md)》 摸高算法基础上，结合**专家经验**的自动调优策略。该策略基于专家经验自动生成完整的摸高配置，用户只需指定**量化类型**（`quant_type`）和**模型结构配置**（`structure_configs`），无需提供完整的量化配置，即可启动摸高调优。

算法内部将生成的完整配置委托给 Standing High 策略执行，因此摸高流程（浮点预检、二分搜索最小回退级别、摸高过程、离群值抑制策略遍历等）与 Standing High 一致，区别仅在于**配置来源**由专家经验自动填充。若浮点预检发现全回退浮点基线即无法满足精度要求，同样会抛出 `SpecError`（错误码 302）退出并提示放宽精度预期。
...
3. **执行侧**：将生成的配置交给 `StandingHighStrategy` 执行摸高，后续与 《[Standing High 调优算法](../standing_high/standing_high.md)》 一致，迭代优化直至得到满足精度要求的最优配置。
...
- 敏感层分析不会在策略侧预先 `load_model`；详见《[自动调优配置协议说明](../../../api_reference/config/auto_precision_tuning.md#21-strategy---调优策略配置)》与《[LLM 大模型接入指南](../../ptq/llm/integration_guide_large_language_model_quantization.md)》。

## 功能介绍

### 使用说明

在自动调优流程中通过 `msmodelslim tune` 启动；在调优 YAML 的 `strategy` 字段将 `type` 设为 `standing_high_with_experience`，并按下文配置其他字段即可。完整调优配置与命令参数见 《[自动调优使用说明](../../../user_guide/usage_auto_precision_tuning.md)》。
