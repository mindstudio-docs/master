...
Standing High 算法的核心思想是“摸高”：算法首先进行浮点预检，评估全回退浮点基线精度，若浮点基线无法满足精度要求则直接报错退出；随后使用配置文件中的初始配置进行评估，如果满足精度要求则直接返回。否则，使用二分搜索快速定位满足精度要求的最小回退级别，然后通过摸高过程逐步减少回退层数，结合不同的离群值抑制策略，寻找最优配置。算法采用自适应步长机制，在找到满足要求的配置时加快搜索速度。
...
在进入量化搜索前，算法先构建一个“全回退浮点”基线配置：所有 `linear_quant` 处理器排除全部层（不量化任何 Linear 层），且不注入离群值抑制处理器，得到未经量化与权重变换的纯浮点模型，并对其评估精度。

- 若浮点基线满足精度要求，说明精度目标在浮点模型上可达，继续后续搜索；
- 若浮点基线不满足精度要求，说明精度目标设置过高（连浮点模型都无法达到），算法立即终止，抛出 `SpecError`（错误码 302）退出，并提示用户放宽精度预期。此举可避免在不可能达标的精度目标下继续消耗量化与评估资源。
...
5） **终止条件**：出现以下任一情况时终止并返回当前记录的最佳配置：初始配置已满足精度要求；摸高过程中回退级别已到 0 且所有离群值抑制策略均已尝试；或步长已为 1 且所有离群值抑制策略均不满足精度要求。此外，若浮点预检未通过，算法会在搜索开始前直接报错退出（见“浮点预检”）。
...
4. **浮点预检**：调优起始即评估全回退浮点基线，若精度目标连浮点模型都无法满足，则立即抛出 `SpecError`（错误码 302）退出，避免在不可能达标的精度目标下进行无效搜索。
...
**模型适配**：策略启动时会**自动运行敏感层分析**以生成回退候选。模型适配器须实现 **`ModelSlimPipelineInterfaceV1`**（即 `core/runner/pipeline_interface.py` 中的 `PipelineInterface`）。分析由 `PipelineAnalysisService` 经 `LayerWiseRunner` 调用适配器的 `init_model`、`handle_dataset` 及 visit/forward pipeline，**不会在策略侧预先调用 `load_model`**。与 CLI `msmodelslim analyze` 要求一致。详见 《[LLM 大模型接入指南](../../ptq/llm/integration_guide_large_language_model_quantization.md)》。

## 功能介绍

### 使用说明

在自动调优流程中通过 `msmodelslim tune` 启动；在调优 YAML 的 `strategy` 字段将 `type` 设为 `standing_high`，并按下文配置其他字段即可。完整调优配置与命令参数见 《[自动调优使用说明](../../../user_guide/usage_auto_precision_tuning.md)》。
...
**配置协议**: template 字段的配置协议与一键量化配置协议中的 `spec` 字段保持一致，详细配置说明请参考《[一键量化使用说明](../../../user_guide/usage_quick_quantization.md#5-量化配置协议详解)》。
