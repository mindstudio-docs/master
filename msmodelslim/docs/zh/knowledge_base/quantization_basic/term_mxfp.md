...
> **英文名称**：MXFP8 / MXFP4（Microscaling Formats）<br>
> **应用领域**：权重、激活量化，超低比特压缩<br>

## 1. 概述

**MX**（OCP Microscaling Formats，微缩放格式）是开放计算项目（Open Compute Project，OCP）定义的一类**块级共享指数**格式：每 **32个元素**为一块，整块共享一个 **E8M0 指数**（8位指数、0位尾数），元素只保存相对该指数的低位数据。块内元素共享同一数量级，从而以少量位宽覆盖大动态范围，缩放参数开销也摊薄到每32个元素一份。
...
- **结构**：块共享 E8M0 指数 + 元素为 **E4M3**（1个符号位 + 4个指数位 + 3个尾数位）。
- **特点**：8位位宽，访存量为 [FP16/BF16](term_fp16_bf16.md) 的一半，同时保留浮点动态范围；相比 INT8 均匀分档对离群值更耐受。
- **应用**：如 [W8A8 MX 动态量化](../quantization_mode/linear_layer_quantization/term_w8a8_mx_dynamic.md)。

### 2.2 MXFP4

- **结构**：块共享 E8M0 指数 + 元素为 **E2M1**（1个符号位 + 2个指数位 + 1个尾数位，emax=2、max 6）。
...
- 相比 [FP8（E4M3）](term_fp8.md)：MXFP8 采用块级共享指数，缩放参数开销从每元素一份摊薄到每32个元素一份；代价是块粒度较粗，需硬件 MX 支持。
- 相比 [INT4](term_int4.md)：MXFP4 用块级共享指数保留动态范围、对离群值更耐受；代价是依赖 MX 硬件，最后一维长度需能被 32 整除。
