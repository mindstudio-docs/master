...
> **英文名称**：FP16 / BF16（16-bit Floating Point）<br>
> **应用领域**：权重/激活的未量化基准、高精度保留场景<br>

## 1. 概述

**FP16** 与 **BF16** 都是 16位浮点格式，各占 2字节/元素，是模型权重与激活的**常见未量化格式**，也是各类量化"减半 / 1/4"压缩对比的**基准**——[INT8](term_int8.md) 的"访存减半"即相对它们而言。FP16 尾数精度高，但可表示范围远小于 FP32（最大有限值约 65504）；BF16 的指数范围与 FP32 完全相同，但尾数精度更低。量化模式常以它们作为"不量化"的高精度基线，如 [W8A16 静态量化](../quantization_mode/linear_layer_quantization/term_w8a16_static.md) 的激活保持 FP16。
...
- **FP16**：1符号 + 5指数 + 10尾数。可表示范围远小于 FP32（最大有限值约 65504），10位尾数保证较高的数值精度。
- **BF16**：1符号 + 8指数 + 7尾数。指数范围与 FP32 完全相同（可表示范围约 $3.4\times10^{38}$），但尾数仅 7位，局部精度低于 FP16；适合需要大动态范围、可牺牲局部精度的训练/推理场景。
...
- **未量化基线**：量化前的权重/激活默认为 16位浮点（FP16 或 BF16，均 2字节），各类量化的压缩比、带宽收益都以它为基准衡量。
