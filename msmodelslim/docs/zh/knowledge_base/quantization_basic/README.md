...
本目录集中介绍量化领域的基础术语，是阅读[量化模式](../quantization_mode/README.md)等知识词条的前置知识。建议先读[量化与反量化](term_quantization.md)理解量化机制，再按需查阅各数据类型词条；量化带来的计算收益（整数 GEMM）在[量化模式](../quantization_mode/README.md)中展开。
...
模式名常用 `WxAy` 记法表示[线性层量化](../quantization_mode/linear_layer_quantization/README.md)中**权重（W）与激活（A）的位宽组合**。`W` 与 `A` 后的数字是位宽，后缀可选：可以只写位宽（如 `W8A8`、`W4A4`），也可以追加数据类型或参数获取方式（如 `W8A8 静态`、`W4A4 MX 动态`）。完整说明见[量化模式](../quantization_mode/README.md)中[如何理解一个量化模式](../quantization_mode/README.md#how-to-read)一节。
