...
适用于在 DP 支持的基础上，进一步对计算密集型的算法任务做并行加速，从而进一步提升多卡量化效率。
...
2. **业务参数只经 `args` / `kwargs` 传入可序列化值**，不得作为 `submit` 的第二个位置参数。常用模式：
   - 传**整数索引**，执行时回查本 rank 上内容一致的任务表（判定模式见本指南步骤 3，实现见 [FlexAWQSSZProcessor](https://gitcode.com/Ascend/msmodelslim/blob/master/msmodelslim/processor/anti_outlier/flex_smooth/processor.py)）：`submit(fn=self._worker_fn, args=(idx,), ...)`，`_worker_fn` 内 `adapter_config = self.sorted_configs[idx - 1]`；
   - 传**模块名**（[LinearQuant 案例](https://gitcode.com/Ascend/msmodelslim/blob/master/msmodelslim/processor/quant/linear.py)）：`submit(fn=self._dts_calibrate_forward, args=(name,), ...)`，`_dts_calibrate_forward` 内 `self.model.get_submodule(module_name)` 后执行本地逻辑。
...
**输入**：步骤 2 的 `fn`；依赖模块路径；`DistHelper`（含 `local_only` 判定时）。
...
3. **判定 `parallel` 语义**（默认 `True`）：
   - `True`：多卡时子任务由不同 rank 分担执行（每个子任务通常只在一个 rank 上跑），`run()` 结束后经同步使结果等价于各 rank 都执行过；
   - `False`：多卡时**每个 rank 都完整执行**该任务（不分工），适用于子任务内包含非共享模块等场景。FlexAWQSSZ 的判定模式：
...
4. **配置同步策略**（可选，见步骤 5）：传 `sync_fn` 自定义任务级同步。

**输出**：完成全部 `submit` 的调度器上下文。

**通过条件**：全部 `submit` 无异常（依赖路径校验通过）；`run()` 前各 rank 提交的任务数量与语义一致（`run()` 内部会校验，见步骤 4）。
...
2. 了解执行机制（`WaveDTSBackend`）：
   - **语义一致性校验**：`run()` 先 `all_gather_object` 比对各 rank 的任务数与任务语义哈希（`task_semantic_hash` 由 fn / sync_fn / args / kwargs / dependencies / parallel 规约后 sha256），不一致直接 `RuntimeError` 并指出首个不一致任务索引——避免「各 rank 提交不同任务」导致的隐性错误；
   ...
   - **执行保护**：共享任务执行期间挂载 `_collective_op_guard`（见步骤 1），非法集合通信直接报错；
...
**目标**：使算法子任务在各 rank 产生的模型状态在 `run()` 后对齐，等价于「各 rank 都执行过完整任务链」。
...
| `DTSMixin.distributed_sync` | 依赖路径子树中某子模块类继承 `DTSMixin` 并实现 `distributed_sync(record, sync_ctx)`，`run()` 时自动调用 | 模型结构未变、但默认同步不满足（如需要同步派生量）；各 rank 调用时集合通信顺序须一致 |
...
**通过条件**：`run()` 后各 rank 模型状态一致（与未接入 DTS 的串行执行结果比对）。
...
| 案例 | 简述 | 链接 |
| --- | --- | --- |
| FlexAWQSSZ 接入 | 逐子图平滑任务化：`submit(args=(idx,))` 回查任务表，`parallel=not (is_non_fusion or has_non_shared_module)`（本指南步骤 3 含完整判定代码） | [FlexAWQSSZProcessor](https://gitcode.com/Ascend/msmodelslim/blob/master/msmodelslim/processor/anti_outlier/flex_smooth/processor.py) |
