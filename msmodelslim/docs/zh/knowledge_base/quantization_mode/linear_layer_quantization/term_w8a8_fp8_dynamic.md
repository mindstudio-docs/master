...
其中 $x$ 为待量化张量，$q$ 为量化后的 FP8 值（$\mathrm{round}_{FP8}$ 表示舍入到 E4M3 可表示的最近值，最大有限值 448），$\hat{x}$ 为反量化还原值，$s$ 为 scale，FP8（E4M3）格式可表示的最大值为 448，因此 $s$按 FP8 的表示上限确定。量化参数的获取方式与作用粒度见上文模式规格表。
