...
> **英文名称**：W8A16 Static Quantization<br>
> **应用领域**：大语言模型量化压缩、推理加速<br>
> **承载 IR 类**：`W8A16StaticPerChannelFakeQuantLinear` / `W8A16StaticPerGroupFakeQuantLinear`（[`msmodelslim/ir/w8a16_static.py`](../../../../../msmodelslim/ir/w8a16_static.py)）<br>

## 1. 概述

W8A16 静态量化是只把线性层的权重压到 [INT8](../../quantization_basic/term_int8.md)，激活保持 [FP16/BF16](../../quantization_basic/term_fp16_bf16.md) 不量化。它面向激活精度敏感、不宜量化激活的场景。权重是每 token 都要整体读取的静态数据，压到 [INT8](../../quantization_basic/term_int8.md) 使权重访存减半；激活不量化则完全规避激活量化误差，是精度最稳的线性层方案之一。代价是激活不量化意味着矩阵乘需把 [INT8](../../quantization_basic/term_int8.md) 权重反量化回浮点执行，没有[整数 GEMM](../term_gemm.md) 加速。
...
其中 $x$ 为待量化张量，$q$ 为量化后的整数值，$\hat{x}$ 为反量化还原值，$s$ 为 scale，$z$ 为零点 offset，$b$ 为位宽（INT8 取 $b=8$，INT4 取 $b=4$），且 $z = \mathrm{round}(-\min(x)/s)$。量化参数 $s$、$z$ 的获取方式（静态校准）与作用粒度（`per-channel`、`per-group`）见上文模式规格表。本模式仅权重走上述公式（$b=8$），激活保持 `FP16` 不量化。
...
- **与 [W4A8 动态量化](term_w4a8_dynamic.md)（位宽搭配不同）**
  ...
  - `W4A8`：优势是权重压到 4bit、访存降至 `FP16` 的 1/4；劣势是 INT4 有符号仅能表示 −8～7 共 16 个整数档位，存在精度风险，激活动态还会引入归约开销。

- **与 [浮点稀疏量化](term_float_sparse.md)（同为高精度基线，收益来源不同）**
  - 本模式：收益来自权重位宽压缩（访存减半）。
  - 浮点稀疏：权重与激活均不量化，收益来自数值稀疏（跳过零值元素），是去除位宽量化后的稀疏对照基线。
...
- [浮点稀疏量化](term_float_sparse.md)：同类模式，完全不量化的稀疏基线。
- [线性量化算法](../../quantization_algorithms/linear_quant/term_linear_quant.md)：配套术语，本模式的处理器实现。
- [MinMax 最小最大值量化算法](../../quantization_algorithms/minmax/term_minmax.md)：配套术语，静态量化常用的量化参数统计方法。
...
2. 《[MinMax 参数配置流程指南](../../quantization_algorithms/minmax/usage_minmax.md)》
3. 《[量化模式](../README.md)》
