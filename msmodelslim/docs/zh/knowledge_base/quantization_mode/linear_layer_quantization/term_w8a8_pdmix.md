...
> **英文名称**：W8A8 PD-Mix Quantization<br>
> **应用领域**：大语言模型量化压缩、推理加速<br>
> **承载 IR 类**：`W8A8PDMixFakeQuantLinear`（[`msmodelslim/ir/w8a8_pdmix.py`](../../../../../msmodelslim/ir/w8a8_pdmix.py)）<br>

## 1. 概述

W8A8 PD-Mix 量化是以 **Prefill / Decode 推理阶段**为界混合两种激活量化策略的 W8A8 方案：Prefill 阶段激活逐 token 动态量化（保精度），Decode 阶段激活整体静态量化（零在线开销）。它是 [W8A8 动态](term_w8a8_dynamic.md) 与 [W8A8 静态](term_w8a8_static.md) 的分阶段融合，PD-Mix即 Prefill-Decode Mix。
...
- **与 [W4A8 动态量化](term_w4a8_dynamic.md)（位宽不同）**
  ...
  - W4A8：优势是权重压缩至 FP16 的 1/4；劣势是 INT4 有符号仅能表示 −8～7 共 16 个整数档位，存在精度风险。
...
- [PDMIX 阶段间混合量化算法](../../quantization_algorithms/pdmix/term_pdmix.md)：配套术语，本模式对应的算法处理器文档。
...
2. 《[PDMIX 参数配置流程指南](../../quantization_algorithms/pdmix/usage_pdmix.md)》
3. 《[量化模式](../README.md)》
