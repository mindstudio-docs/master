...
> **英文名称**：FA INT8 Dynamic Quantization<br>
...
FA INT8 动态量化是对送入 Flash Attention 的 Q/K/V 激活统一按 token 逐行动态量化到 [INT8](../../quantization_basic/term_int8.md) 的整体组合模式。量化参数在线计算、免校准，每个 token 的激活分辨率贴合自身数值范围。它是 FA 量化中"动态免校准"一档的 INT8 模式，K/V 承接缓存压缩（[KVCache 量化](../kv_cache_quantization/README.md)），Q 一并量化后使注意力得分计算可走低精度矩阵运算。
...
上式中，$x$ 表示待量化的张量，$q$ 表示该张量量化后的数值，$\hat{x}$ 表示该张量经反量化还原后的数值，$s$ 表示 scale，$b=8$ 表示位宽。该模式采用对称量化，即只有 scale、没有 offset。在 per-token 粒度下，输入先被 reshape 为 (B\*H\*S, D)，随后逐行统计 min/max 并动态计算每个 token 的 scale。

### 2.3 与其他模式的关系

- **与 [FA INT8 PerHead 量化](term_fa_int8_perhead.md)（同为 INT8 统一模式，参数获取方式与粒度不同）**
...
- **与 [W8A8 动态量化](../linear_layer_quantization/term_w8a8_dynamic.md)（同类 per-token 动态思路）**
  - 本模式把同一思路应用于注意力 Q/K/V 激活；W8A8 动态应用于线性层权重与激活。二者正交、可组合。
...
- **长序列注意力加速**：配合 Flash Attention 的整数/低精度计算，降低带宽与计算开销。
...
- [FA INT8 PerHead 量化](term_fa_int8_perhead.md)：同类模式，per-head 静态的 INT8 统一模式。
