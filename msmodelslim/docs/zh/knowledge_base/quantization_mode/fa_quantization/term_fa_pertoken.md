...
> **英文名称**：FA Per-Token Quantization<br>
...
FA PerToken 量化是对送入 Flash Attention 的 Q/K/V 激活按 token 逐行动态量化（[INT8](../../quantization_basic/term_int8.md) 或 [FP8（E4M3）](../../quantization_basic/term_fp8.md)），量化参数在线计算、免校准。它是 [KVCache 量化](../kv_cache_quantization/README.md)的**进阶**：K/V 量化省缓存显存，Q 一并量化后使注意力得分计算可走低精度矩阵运算；per-token 粒度让每个 token 的激活分辨率贴合自身数值范围。
...
- **与 [FA PerBlock 量化](term_fa_perblock.md)（同为 FA 激活量化，格式与粒度不同）**
  - 本模式：INT8/FP8 按元素独立编码，scale 按 token 共享（张量每一行对应一个 token），无需特殊硬件格式支持；劣势是 scale 数量与 token 数同量级。
  - PerBlock：MX 格式块级共享指数（32元素一块），粒度更细（沿 head_dim）、对离群值更耐受；劣势是依赖硬件 MX 支持、head_dim 需按 32 对齐。

- **与 [W8A8 动态量化](../linear_layer_quantization/term_w8a8_dynamic.md)（同类 per-token 动态思路）**
  - 本模式把同一思路应用于注意力 Q/K/V 激活；W8A8 动态应用于线性层权重与激活。二者可正交组合：线性层走 W8A8，注意力路径走本模式，共同构成整体量化方案。
