...
详细安装方式请参见《[msMonitor工具安装指南](../getting_started/install_guide.md)》。
...
> [!NOTE] Note
> `--certs-dir NO_CERTS` 表示不使用证书验证，仅用于测试环境，后文同理。在生产环境中，建议使用证书验证，以确保数据传输的安全性。详情请参见 [dynolog_instruct](./dynolog_instruct.md)。
...
发送 nputrace 子命令，采集性能数据，详细参数说明请参考 [nputrace 子命令](./nputrace_instruct.md)。
...
发送 npu-monitor 子命令，采集性能数据，详细参数说明请参考 [npu-monitor 子命令](./npumonitor_instruct.md)。
