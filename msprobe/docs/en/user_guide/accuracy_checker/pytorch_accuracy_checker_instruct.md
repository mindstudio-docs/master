...
| `-csv_path` or `--result_csv_path` | No | Specifies the path to the `accuracy_checking_result_{timestamp}.csv` file that was generated when the run was interrupted. <br> To resume execution from the point of interruption when running `acc_check`, this parameter must be configured. <br/>It must point to the `accuracy_checking_result_{timestamp}.csv` file from the previous interruption. For details, see [Resumable Check](#resumable-check) |
...
#### config.yaml File Description
   ...
   - Modify the parameters in the `config.yaml` file.
      ...
      | quantization_api_list       | No     | List of quantization operator APIs with floating-point input and integer output. When configured, this indicates that for this type of API, the condition for `PASS` is that the absolute error must be ≤ 1.                         |

      Note: If both `white_list` and `black_list` are configured and the API lists configured for them do not overlap, the trustlist takes effect. If the API lists overlap, the APIs excluded by the trustlist and the overlapped APIs are not dumped.
...
#### API Output Post-Processing Configuration

To accommodate variable-length outputs of certain APIs in real-world scenarios (e.g., invalid values in tail padding areas), `acc_check` supports applying post-processing rules to CPU-side and device-side outputs separately before comparison. The post-processing configuration file is located at [api_output_postprocess.yaml](../../../../python/msprobe/core/common/output_postprocess/api_output_postprocess.yaml).

During execution, the framework selects the appropriate side rules through a unified post-processing entry point based on the `backend` (`cpu` or `device`). APIs that are not matched in the configuration will skip the corresponding side's post-processing logic.
...
| Field | Description |
| --- | --- |
| `acc_check_handlers.target` | Configures post-processing functions for the target side on a per-API basis. The value format is `python_file_path:function_name`. Supports relative paths (relative to the `output_postprocess` directory) or absolute paths. If not configured, set to `{}`. |
| `acc_check_handlers.golden` | Configures post-processing functions for the golden side on a per-API basis. The value format is `python_file_path:function_name`. Supports relative paths (relative to the `output_postprocess` directory) or absolute paths. If not configured, set to `{}`. |

The following APIs are pre-configured with default handlers:

- `npu_dequant_swiglu_quant`: Uses `group_index` as the basis for effective length.
- `npu_grouped_matmul`: Uses `group_list` as the basis for effective length.

When adding new processing rules, simply append additional `API -> handler` mappings to the corresponding side. Both default handlers and user-defined extensions share the same loading and execution logic.

**acc_check_handlers Configuration Example**
...
- The current configuration semantics are one API corresponds to one handler.
- If the same API requires multiple processing steps, it is recommended to chain them together inside that API's handler.
- Different APIs can share the same handler, or be configured with different handlers.
...
- The handler script must be placed under the [output_postprocess](../../../../python/msprobe/core/common/output_postprocess/) directory.
- If the handler execution fails or returns `None`, the framework will automatically fall back to the original `output`.
...
| Mean ULP error | Mean ULP error (absolute value) between NPU data and the benchmark data.                                                                                                                                                                                                                                                                                                                                                        |
| Proportion of ULP errors greater than the threshold     | Proportion of elements where the absolute ULP error between NPU data and benchmark data exceeds a threshold, relative to the total number of elements. The threshold is `1` for float16 or bfloat16 data types, and `32` for float32.                                                                                                                                                                                                                                                                    |
| Ratio of ULP error greater than the threshold | NPU-CPU ULP error exceedance rate/GPU-CPU ULP error exceedance rate                                                                                                                                                                                                                                                                                                                                  |
