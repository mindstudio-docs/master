...
- **Multiple dump modes**: Provide multiple collection modes, including statistics, tensor, acc_check, structure, and overflow_check, and nan_check.
...
- Loss or gnorm may change after msProbe is used. This can be caused by synchronization introduced by item operations in the tool or by the hook mechanisms of the PyTorch or MindSpore framework. For details, see [Cause Analysis of Model Calculation Result Changes](../../support/faq.md#model-computation-result-change).
...
| Lightweight deterministic difference localization | task="statistics"<br>summary_mode="xor"          | Only XOR checksum values are collected. To accelerate collection, install the tool package compiled with the `--include-mod=xor_checksum` flag. For installation instructions, see [Install the basic tool package and xor_checksum](../../install_guide/msprobe_install_guide.md#install-xor-checksum). The C++ acceleration operator will be used preferentially, delivering several-fold performance improvement. |
| NaN/Inf detection | task="nan_check"      | Detect NaN/Inf during API execution via register status. |
...
- For APIs supported by this tool for dumping (see [support_wrap_ops.yaml](../../../../python/msprobe/pytorch/dump/api_dump/support_wrap_ops.yaml)), filtering rules can be configured to ignore specific inputs or outputs of these APIs. For detailed instructions, refer to [API Input/Output Filtering](#api-inputoutput-filtering).
...
* `dump.json`: stores statistical information for forward/backward data of APIs or Modules. It includes the API or Module name of the dumped data, statistical information for each data tensor such as `dtype`, `shape`, `max`, `min`, `mean`, `L2norm` (L2 norm, square root), as well as checksum values output based on the `summary_mode` configuration (`md5` corresponds to the CRC-32 field md5; `xor` corresponds to the XOR checksum field md5). For details, see [dump.json Description](#dumpjson-description).
  * When the tensor data format is NZ, the `max`, `min`, `mean`, and `L2norm` (L2 norm, square root) statistics in `dump.json` are all null. Additionally, `mean` and `L2norm` statistics are calculated only when the tensor data type is one of `torch.float32`, `torch.float16`, or `torch.bfloat16`; for other data types, only `max` and `min` statistics are calculated.
  * When `summary_mode` is set to `xor`, `dump.json` outputs only the XOR checksum value, without outputting `max`, `min`, `mean`, or `L2norm` statistics. If the acceleration operator is unavailable, the tool automatically falls back to the generic implementation.
  * When `task` is set to `nan_check`, the data for each API in dump.json will include an `is_nan` field with a value of `0` or `1`, where `0` indicates no overflow status and `1` indicates an overflow status (statistics for API data are not saved in this mode).

* `dump_error_info.log`: Generated only when the dump tool encounters an error; used to record dump error logs.
...
- `Module.conv2.Conv2d.forward.0`: module forward data.
  **input_args** is the input data (position parameter), **input_kwargs** is the input data (keyword parameter), **output** is the output data of the module, and **parameters** is module parameters, including weight and bias.
...
### API Input/Output Filtering

**Function**

msProbe provides a fixed list of supported APIs. For details, see [support_wrap_ops.yaml](../../../../python/msprobe/pytorch/dump/api_dump/support_wrap_ops.yaml). If you do not need to collect certain inputs or outputs of these APIs, you can configure built-in ignore rules in [builtin_ignore_ops.yaml](../../../../python/msprobe/pytorch/dump/api_dump/builtin_ignore_ops.yaml).

When an ignore rule is matched, the tool retains the API call record but masks the API's inputs or outputs, setting them to `Null` in the dump results. This feature is commonly used for parameters that do not require participation in accuracy comparison, such as receive buffers of communication operators or in-place write outputs.

If your goal is to completely exclude an API from collection, modify `support_wrap_ops.yaml`. If your goal is to retain the API call record but mask certain inputs or outputs, modify `builtin_ignore_ops.yaml`.

**Notes**

- Currently, only ignore rules for API forward call inputs and outputs are supported; backward calls are not supported. Note that "API forward call" here refers to the forward method call of the API, which is not equivalent to the forward pass in the training script.
...
<a id="top-level-category-mapping-table"></a>Top-Level Category Mapping Table

| support_wrap_ops.yaml | builtin_ignore_ops.yaml |
| --- | --- |
...
| `npu_distributed` | `Distributed` |
| `mindspeed` | `MindSpeed` |

**Example**

For `torch.distributed.recv(tensor, src=None, group=None, tag=0)`,  to ignore the 0-th input parameter `tensor`:
...
For `torch.distributed.isend(tensor, dst=None, group=None, tag=0)`, to ignore all outputs:
...
For `torch_npu.npu_fusion_attention`, to ignore the 4th and 5th outputs:
...
After modifying this file, restart the training or inference process to reload the configuration.
...
### Batch Registration of User-Defined APIs

**Function**

In addition to registering custom APIs one by one using [register_custom_api](#register_custom_api) in code, the tool also supports batch registration via a YAML configuration file. This is particularly useful for scenarios where a large number of custom operators need to be registered.

**Notes**

Modify the `custom_wrap_ops.yaml` file located in the `pytorch/dump/api_dump/` directory under the msProbe installation path. Do not rename the file. After modifying this file, restart the training or inference process to reload the configuration.
...
| Field | Description |
| --- | --- |
| module_path | The Python module path to which the API belongs. Supports multi-level submodules, e.g., `torch.nn.functional`, `module_a.submodule`. Must be a valid Python module path (composed of letters, digits, underscores, and dots, starting with a letter or underscore). Defaults to the prefix of the API name in [dump.json](#dumpjson-description). |
| api-name | The attribute name of the API to be registered within the module, e.g., `relu`, `compute`. Must be a valid Python identifier (composed of letters, digits, and underscores, starting with a letter or underscore). |

**Example**

```yaml
# Register APIs in the main module
__main__:
  - add_and_mul
```
...
>     """计算 (a + b) * c"""
>     return (a + b) * c
> # After enabling the tool here, add_and_mul will be dumped by default.
> ```
...
- **model**: model whose module-level data needs to be collected. The input can be of the **torch.nn.Module**, **list[torch.nn.Module]**, or **Tuple[torch.nn.Module]** type. By default, this parameter is not configured.
  ...
  This parameter must be set in this API when **level** is set to **L0** or **mix**, or **token_range** is not **None**.

  For a complex model, if only part of the model needs to be monitored (for example, **model.A** and **model.A extends torch.nn.Module**), pass the part to be monitored (**model.A**).
  
  Note: The input layer is not dumped. The tool dumps only the sub-layers of the input layer. For example, if **model.A** is passed, **A** is not dumped, but **A**.*x* or **A**.*x.xx* is dumped.
