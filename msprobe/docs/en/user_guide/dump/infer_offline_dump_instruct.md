...
Collecting data for ONNX models depends on the `onnx` and `onnxruntime` packages, while collecting data for OM models depends on the `aisbench` and `aclruntime` packages. You can install the required dependency packages using the following commands before use. If the dependency packages are already installed, you may skip this step.

```bash
msprobe install_deps -m offline [--no_check]
```

**Note**: The **no_check** parameter skips the check of the certificate information of the target website of `aisbench` and `aclruntime` packages, which poses security risks. Exercise caution when using this parameter and you shall bear the consequences.
