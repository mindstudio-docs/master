...
* **Ascend Transformer Boost (ATB)**: An efficient and reliable acceleration library designed for Transformer models based on the Ascend AI Processor. For details, see [ATB User Guide](https://www.hiascend.com/document/detail/en/CANNCommunityEdition/latest/acce/ascendtb/ascendtb_0001.html).
...
**Note**: Currently, msProbe with the ATB dump module can be [compiled and installed from source code](../../install_guide/msprobe_install_guide.md#23-installation-from-source). During compilation, you need to use the `--include-mod` parameter to specify the **atb_probe** module.
...
```ColdFusion
├── outputPath  # Output path used when executing the module loading script
│   ├── atb_dump_data  # Fixed directory automatically created by the tool
│   |   ├── data  # Fixed directory automatically created by the tool, storing op input/output data
...
|   |   |   |    |   |   |   |   ├── after  # Stores output tensors
|   |   |   |    |   |   |   |   |   |── outtensor0.bin
|   |   |   |    |   |   |   |   ├── before  # Store input tensors
|   |   |   |    |   |   |   |   |   |── intensor0.bin
|   |   |   |    |   |   |   |   |   |── intensor1.bin
|   |   |   |    |   |   |   ├── after
|   |   |   |    |   |   |   |   |── outtensor0_output.bin  # May be outtensor0.bin depending on CANN version
|   |   |   |    |   |   |   ├── before
|   |   |   |    |   |   |   |   |── intensor0_x.bin  # May be intensor0.bin depending on CANN version
|   |   |   |    |   |   |   |   |── intensor1_indices.bin  # May be intensor1.bin depending on CANN version
...
|   |   |   |    |   |   └── ...
|   |   |   |    |   ...
|   |   |   |    |   ├── statistic.csv  # Statistics data for op input/output tensors
...
|   |   |   |   └── ...
|   |   |   ...
│   |   ├── info  # Fixed directory automatically created by the tool, storing structure information
│   |   |   ├── layer  # Fixed directory automatically created by the tool, storing layer-level Operation structure information
│   |   │   |   ├── 0_39943  # Device ID and process ID
│   |   │   |   |   ├── WordEmbedding_0.json
|   |   |   |   |   ...
|   |   |   |   ...
│   |   |   ├── model  # Fixed directory automatically created by the tool, storing model structure information
│   |   │   |   ├── 0_39943  # Device ID and process ID
...
| Index           | Tensor index (may contain the parameter name), for example, **0,2_value**.|
