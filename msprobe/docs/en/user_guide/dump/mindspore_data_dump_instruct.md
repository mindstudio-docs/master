...
In static graph mode with `ms_backend(jit_level=O0/O1)` set, msProbe supports data collection at L0 and L2 levels, and does not support `ge_backend(jit_level=O)`.
...
> **Note**: In dynamic graph mode, the part decorated by `mindspore.jit` is actually executed in static graph mode. In this case, the kernel-level (L2) data collection method and scope are the same as those used in the static graph mode.
...
```python
import mindspore as ms
ms.set_context(mode=ms.PYNATIVE_MODE, device_target="Ascend", jit_config={"jit_level": "O0"})
...
- With the exception of intermediate files like `kernel_kbyk_dump.json` and `kernel_graph_exception_dump.json`, which can generally be ignored, the directory structure for L2-level dumps is documented in the official MindSpore documentation. For details, see [Dump in Ascend O0/O1 Mode >Introduction to Data Object Directory and Data File](https://www.mindspore.cn/docs/en/r2.5.0/model_train/debug/dump.html).
