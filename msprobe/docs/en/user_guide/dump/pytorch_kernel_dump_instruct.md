...
3. If the problem persists, use the **torch_npu.npu** API provided by the [TorchNPU](https://gitcode.com/Ascend/pytorch) to collect kernel-level data. The kernel dump function of the tool is implemented based on the **init_dump**, **set_dump**, and **finalize_dump** sub-interfaces. For details about the **torch_npu.npu** API, see [torch_npu.npu](https://gitcode.com/Ascend/op-plugin/blob/master/docs/en/custom_APIs/torch_npu-npu/torch_npu-npu_list.md).
