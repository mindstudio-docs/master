...
| summary_mode | Optional     | Dump file output mode. PyTorch, MSAdapter, and MindSpore dynamic/static graphs, jit_level=O0/O1 are supported. For details, see [summary_mode Configuration](#summary_mode-configuration).|
...
> [!NOTE]
>
> If both **white_list** and **black_list** are configured, the APIs that actually take effect are those in the trustlist excluding those in the blocklist.
...
**Notes**
 ...
 - Before use, ensure that the msProbe tool package is compiled with the `--include-mod=nan_check` flag.
 - This task runs on Ascend NPU devices and requires CANN version 8.5.0 or higher.
 - The `level` in the collection `config` must be set to `L1`.
 - The environment variable `INF_NAN_MODE_FORCE_DISABLE=1` must be set before use.
 - Since detection results are obtained by reading register status, there may be cases where the tensor output appears normal but the `nan_check` mode detects an overflow, indicating that the API experienced intermediate overflow (which is considered normal behavior).
 - Since the hardware does not natively support the int64 type (it is implemented in software), the register status obtained by this interface cannot guarantee accurate results for int64 types.

**Parameters**

`nan_check` detects whether NaN or Inf occurs during API runtime by reading the NPU register status.

| Parameter | Required/Optional | Description |
| ---- | --------- | ---- |
| scope | Optional | Dump scope for PyTorch scenarios, of the type `list[str]`. By default, it is not specified (if neither scope nor list is set, all APIs are dumped). For details, see [scope Configuration](#scope-configuration). |
| list  | Optional | Custom list of operators to collect, of the type `list[str]`. By default, it is not specified (if neither scope nor list is set, all APIs are dumped). For details, see [list Configuration](#list-configuration). |
| tensor_list | Optional | Keyword list of APIs that require additional `npu_nan_test` triggering, of the type `list[str]`. For details, see [tensor_list Configuration](#tensor_list-configuration). |
| data_mode | Optional | Dump data filter, of the type `list[str]`. For details, see [data_mode Configuration](#data_mode-configuration).<br/><br/>**Note**: In `nan_check` task scenarios, when `input` is specified in data_mode, `output` must also be configured. |
...
- In the MindSpore static graph O0/O1 scenario, `kernel_name` can be set to an operator name list, an operator type, or a regular expression of the operator name (when the string is in the "name-regex(xxx)" format).
...
- General behavior
  ...
  - In PyTorch, MSAdapter, and MindSpore dynamic graph scenarios, specifying a certain type of API or module will dump statistics and full tensor data for the inputs and outputs of that API or module.

  - Example: `"tensor_list": ["relu"]`

- Special notes for `nan_check` tasks: `nan_check` detects NaN/Inf overflow for all APIs by default. When further localization of the exact overflow location within a specific API is required, you can specify the API for in-depth analysis via `tensor_list`. When overflow occurs, the tool will raise an `Exception` and generate additional detailed data files for these APIs, saving them to the `extra-info` subdirectory under the current directory. These files can be used with the [Tensor Parsing Tool](https://www.hiascend.com/document/detail/en/CANNCommunityEdition/latest/maintenref/troubleshooting/troubleshooting_0532.html) for element-level analysis.
  - Matching mode: Substring matching, case-insensitive. For example, when [truediv] is configured, all APIs whose names contain `truediv` (e.g., `truediv`, `__truediv__`) will be matched.

  - Usage recommendations
    - `nan_check` already covers basic overflow detection for all APIs by default. It is recommended to configure `tensor_list` only when deeper investigation of a specific API's overflow root cause is required, in order to control additional overhead.
    - When configuring, it is recommended to use more specific keywords (e.g., `truediv`, `softmax`) to avoid unintended matches caused by overly short strings. If further scope restriction is needed, you can combine with `scope` or `list` to narrow down the target set.
...
- MindSpore static graph O0/O1 scenario: The L0 dump supports only **all**, **forward**, and **backward**. The L2 dump supports only **all**, **input**, and **output**. In addition, the options can be configured only separately and cannot be combined freely.
...
- PyTorch, MSAdapter, and MindSpore dynamic graph
  ...
  Example: "summary_mode": "md5". To use lightweight XOR checksum, set "summary_mode": "xor".

To reduce collection overhead in large-tensor scenarios, users are advised to use the msProbe package compiled with the `--include-mod=xor_checksum` flag. For details, see [Install the basic tool package and xor_checksum](../../install_guide/msprobe_install_guide.md#install-xor-checksum). When `summary_mode` is set to `xor`, the tool will first attempt to use the compiled C++ acceleration operator for device-side XOR checksum computation, which can significantly improve performance. In cases where the acceleration operator is unavailable, fails to load, or encounters runtime errors, the tool will automatically fall back to the generic implementation without disrupting the collection workflow.

- MindSpore static graph

  The value is of the string or list[str] type.

  - L2 (jit_level=O0/O1): In addition to **md5** and **statistics**, the statistical item list can be configured. The optional statistical items are **max**, **min**, **mean**, **l2norm**, **count**, **negative zero count**, **zero count, positive zero count**, **nan count**, **negative inf count**, **positive inf count**, **hash**, and **md5**. You can select any combination of them. **hash** calculates the MD5 value in MindSpore 2.7.0 and earlier versions, and calculates the SHA1 value in later versions.
...
**Note**: In the PyTorch, MSAdapter, and MindSpore dynamic graph scenarios, when **summary_mode** is set to **md5**, the CRC-32 algorithm is used. In the MindSpore static graph scenario, when **summary_mode** is set to **md5**, the MD5 algorithm is used.
