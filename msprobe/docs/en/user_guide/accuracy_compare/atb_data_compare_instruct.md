...
You need to collect the benchmark precision data and the precision data to be compared (data with precision issues) of an ATB model by referring to [Precision Data Collection in ATB](../dump/atb_data_dump_instruct.md). Then, run the following command to compare data precision.
