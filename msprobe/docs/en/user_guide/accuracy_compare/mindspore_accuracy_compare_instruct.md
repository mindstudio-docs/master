...
| ----------------- |-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| -------- |
| `-tp` or `--target_path` | Required | Path to the `dump.json` file (single-rank scenario) or dump directory (multi-rank scenario) in the NPU environment. Type: `str`. |
| `-gp` or `--golden_path` | Required | Path to the `dump.json` file (single-rank scenario) or dump directory (multi-rank scenario) in the CPU, GPU, or NPU environment. Type: `str`. |
| `-o` or `--output_path` | Optional | Specifies the output directory for comparison result files. The default is to create an `output` directory in the current path. Type: `str`. The default output format is CSV, with filenames automatically generated based on timestamps in the following formats:<br/>• `compare_result_{timestamp}.csv`<br/>• `compare_result_{rank_id}_{step_id}_{timestamp}.csv` (only supported in [full kernel comparison across different versions](#full-kernel-comparison-across-different-versions)).<br/>When `--xlsx` is specified, the output format is XLSX.<br/>**Note:** Files in the `output` directory with the same name as the result file will be overwritten. |
| `--xlsx` | Optional     | Outputs comparison results in XLSX format. If not specified, the default output format is CSV |
| `-fm` or `--fuzzy_match` | Optional     | Enables fuzzy matching. When enabled, APIs at the same hierarchical level with identical names but different invocation counts can be matched and compared. This parameter is directly enabled by configuration; if not specified, fuzzy matching is disabled.                                                                                                                                                                           |
| `-am` or `--api_mapping` | Optional     | Cross-framework comparison. When this parameter is specified, the cross-framework API comparison feature is enabled. A custom mapping file `*.yaml` can be specified. If no mapping file is provided, the default mapping relationship defined by msProbe is used. For the custom mapping file format, see [Custom Mapping File (api_mapping)](#api-mapping). This parameter is required only for [cross-framework API comparison](#cross-framework-api-comparison).                                                                          |
| `-cm` or `--cell_mapping` | Optional     | Cross-framework comparison. When this parameter is specified, the cross-framework cell module comparison feature is enabled. A custom mapping file `*.yaml` can be specified. If no mapping file is provided, the default mapping relationship defined by msProbe is used. For the custom mapping file format, see [Custom Mapping File (cell_mapping)](#cell-mapping). This parameter is required only for [cross-framework cell module comparison](#cross-framework-cell-comparison).                                                               |
| `-dm` or `--data_mapping` | Optional | Same-framework or cross-framework comparison. Specifies the correspondence between two specific parameters via a mapping file. Can be used in L0, L1, or mix collection scenarios. This parameter must be used together with a custom mapping file `*.yaml`. For the custom mapping file format, see [Custom Mapping File (data_mapping)](#data-mapping). |
| `-lm` or `--layer_mapping` | Optional | Cross-framework comparison. When this parameter is specified, the cross-framework Layer-level comparison feature is enabled. After specifying the Layer layer in the model code, the corresponding modules or APIs in the dump data can be identified. A custom mapping file `*.yaml` must be specified. For the custom mapping file format, see [Custom Mapping File (layer_mapping)](#layer-mapping). This parameter is required only for [cross-framework Layer-level comparison](#cross-framework-layer-comparison). |
| `-da` or `--diff_analyze` | Optional | Automatically identifies the first divergent node in the network. Supports dump data such as md5 and statistics. Supports both single-rank and multi-rank scenarios. This parameter is directly enabled by configuration; if not specified, automatic diff analysis is disabled. |
| `--rank` | Optional | Specifies the Rank ID(s) for comparison. Used only for kernel comparison. Type: `int`. Dump files under the `target_path` and `golden_path` directories must contain data for the corresponding ranks. Default is empty, which means all ranks are compared. One or more rank IDs can be configured, separated by commas, e.g., `1,2,3`. |
| `--step` | Optional | Specifies the Step ID(s) for comparison. Used only for kernel comparison. Type: `int`. Dump files under the `target_path` and `golden_path` directories must contain data for the corresponding steps. Default is empty, which means all steps are compared. One or more step IDs can be configured, separated by commas, e.g., `1,2,3`. |
| `-tensor_log` or `--is_print_compare_log` | Optional | Enables log printing for individual modules or APIs. Only supports tensor data dumped by the msProbe tool. This parameter is directly enabled by configuration; if not specified, log printing is disabled. |
...
2. Run the following example command to perform the comparison.
   ...
   In this scenario, only `-tp`, `-gp`, `-o`, `--xlsx`, `--rank`, and `--step` of the `compare` command are supported.
...
3. Check data comparison results at L0/Mix level in dynamic/static graph scenarios. By default, `.csv` files are output. If `--xlsx` is specified, `.xlsx` files are output.
...
Full kernel comparison scenario: A `.csv` file is generated in the configured output path. The file name is automatically generated based on the timestamp in the format of `compare_result_{rank_id}_{step_id}_{timestamp}.csv`. If `--xlsx` is specified, a `.xlsx` file is generated in the format of `compare_result_{rank_id}_{step_id}_{timestamp}.xlsx`.
...
Single-point data comparison in dynamic graph scenarios: A `.csv` file is generated in the configured output path. The file name is automatically generated based on the timestamp in the format of `debug_compare_result_(rank_id/proc_id)_{timestamp}.csv`. If `--xlsx` is specified, a `.xlsx` file is generated in the format of `debug_compare_result_(rank_id/proc_id)_{timestamp}.xlsx`.
...
- Only comparison results in XLSX format are supported as input; CSV format comparison results are not supported.
...
The 1st item is followed by `dict`. The keys include `y`, `y_mask`, `x_mask`, `loss_mask`, and `data_info`. According to the naming rules, the following names can be obtained:
...
`y` is followed by `Tensor`, and the naming ends. `y_mask` is followed by `Tensor`, and the naming ends. `x_mask` is followed by `Tensor`, and the naming ends. `loss_mask` is followed by `Tensor`, and the naming ends. `data_info` is followed by `dict`, the key is followed by `img_hw`, and the following name can be obtained:
