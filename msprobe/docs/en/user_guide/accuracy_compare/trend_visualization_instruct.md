...
**Note**: Replace `ubuntu` with the actual server address. For example, if the actual server address is `10.123.12.12`, enter [http://10.123.12.12:6008](http://10.123.12.12:6008/) in the address box of the browser.
...
1. Manually set a proxy for the local computer network. For example, in Windows 10, add the server address (for example, `10.123.12.12`) in the manual proxy settings.
   ...
   Finally, enter `http://10.123.156.78:6008` in the browser's address bar.

   **Note**: If the firewall is enabled on the server, this method will not work. In this case, disable the firewall or try the following methods.
...
3. Transfer the image composition result from the server to the local computer and install msProbe on the local computer to view the image composition result.
...
This plugin supports scanning and switching between multiple `.trend.db` database files.

##### Features

- **Auto Scanning**: The plugin automatically scans all .trend.db files in the `logdir` directory upon startup.
- **Dynamic Switching**: You can switch between different database files in real time using the frontend dropdown menu.
- `Auto Refresh`: The page automatically refreshes after switching databases to display data from the newly selected database.

##### Usage

1. Prepare multiple database files.
   
   Place multiple .trend.db files in the TensorBoard `logdir` directory, for example:

   ```bash
   logdir/
   ├── experiment1.trend.db
   ├── experiment2.trend.db
   └── experiment3.trend.db
   ```

2. Select a database file.
   
   - Open your browser and navigate to `http://localhost:6006`.
   - Select the "Trend Analyzer" tab from the top navigation bar.
   - In the left control panel, locate the "Select Database File" dropdown menu.
   - Choose the database file you wish to view from the dropdown list.
   - The page will automatically refresh and display data from the selected database.

![switch_db](../../figures/trend_analyzer/switch_db.png)

##### Important Notes

- Database files must have the `.trend.db` suffix.
- Switching databases triggers a page refresh, and the currently selected metrics, dimensions, and other configurations will be reset.
- If no `.trend.db` files are found, the plugin will not be activated.
...
|Label|By setting tags, the data range displayed in heatmaps and line charts can be restricted to data that satisfies the tag criteria (e.g., specific layers, specific module types). The following tag types are supported<br/>&#8226; `default`: Indicate that the tensor target belongs to network layer data, such as `Module` or `Cell`. These are specific to dump data scenarios and distinguish between network layer data and operator API data.<br/>&#8226; `layer`: Labels extracted from network layer names in the format `xxx.N`, where `xxx` is a string and `N` is an integer representing the network layer index, e.g., `layers.0`.<br/>&#8226; `index`: Indicate the input/output position to which a tensor belongs. For example, the 0-th input tensor is tagged as `input.0`.<br/>&#8226; `module`: String tags extracted from network layer names that represent the network layer type, e.g `TransformerLayer`.<br/>&#8226; `function`: Operator API names for which no layer tag could be extracted.<br/>Examples: <br/>&#8226; For a monitored object with the full name `module.decoder.layers.0.input_layernorm.weight` collected in a monitor data scenario, the following labels will be extracted: module labels: `module`, `decoder`, `input_layernorm`, `weight`; `layer` label: `layers.0`.<br/>&#8226; For a monitored object with the full name `Module.blocks.0.fsdp_wrapped_module.norm.1.WanLayerNorm` collected in a dump data scenario, if it contains one input and one output tensor, the following labels will be extracted: `default` label: `Module`; module labels: `fsdp_wrapped_module`, `WanLayerNorm`; `layer` labels: `blocks.0`, `norm.1`; `index` labels: `input.0`, `output.0`.|
