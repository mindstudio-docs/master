...
```sh
# Use a version earlier than TorchNPU 7.1.0.
msprobe compare --target_path ${dump_path}/msprobe_ge_dump --golden_path data_dump --mode torchair
```

**Note**: When a version earlier than TorchNPU 7.1.0 is used, the name of the token ID directory in the dump result in FX mode is one greater than the actual token ID. Therefore, during comparison, the directory name is decremented by one to obtain the correct token ID.
...
**Note**: This section applies only to specific scenarios.
...
Execute this script in the background during the dump process to convert the dump data into info, thereby reducing disk space usage.
...
```bash
# If TorchNPU 7.1.0 or later is used, convert the FX dump data in msprobe_fx_dump into info.
python3 convert.py msprobe_fx_dump
```

```bash
# If a TorchNPU version earlier than 7.1.0 is used, convert the FX dump data in data_dump into info.
