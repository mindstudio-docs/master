...
- For details about supported PyTorch versions, see [Release Notes](https://gitcode.com/Ascend/msprobe/releases).
...
| `--tp`                          | No (mandatory only in the graph merging scenario)| TP size. The value is of the list[int] type. In the actual training script, `--tensor-model-parallel-size T` needs to be specified, where `T` indicates the TP size, that is, the `tp` parameter required for graph merging (`tp=T`).                                                                                                 |
| `--pp`                          | No (mandatory only in the graph merging scenario)| Number of pipeline parallel stages. The value is of the list[int] type. In the actual training script, `--pipeline-model-parallel-size P` needs to be specified, where `P` indicates the number of pipeline parallel stages, that is, the `pp` parameter required for graph merging (`pp=P`).                                                                                           |
| `--vpp`                         | No| Number of virtual pipeline parallel stages. The value is of the list[int] type. VPP depends on pipeline parallelism. In the actual training script, you need to specify `--num-layers-per-virtual-pipeline-stage V`, where `V` indicates the number of layers in each virtual pipeline stage, and specify `--num-layers L`, where `L` indicates the total number of model layers. For graph merging, `vpp=L/V/P` is required. The `vpp` parameter is optional. The default value is `1` (`vpp=1`), indicating that VPP is disabled.|
| `--order`                       | No| Sorting order of model parallelism policies. The value is of the list[str] type. The default value for Megatron is `tp-cp-ep-dp-pp`. If msProbe is used to dump data and the specified level is `L0`, and `order` in the actual training script is not the default value (for example, `--use-tp-pp-dp-mapping` is specified in the actual training script), pass the modified `order`. If the specified level for data dumping is `mix`, no modification is required.                        |
| `--file_type`                   | No | Output file type, which is a string. The value can be `db` (default) and `json`. <br>1. For single-graph build, two-graph comparison, and graph merge comparison scenarios: only db format is supported; json format is not supported.<br>2. For graph merging build scenarios: both db and json formats are supported. Selecting json outputs the merged JSON structure file, which can be used for accuracy comparison or hierarchical visualization.
...
`target_path`: eight ranks (`tp=8`); not specifying `golden_path`
...
`target_path`: eight ranks (`pp=8`, `vpp=2`); not specifying `golden_path`
...
According to [Matching Description](#matching-description), nodes that do not meet the matching rules are considered unmatched and are marked in gray. This function is applicable to scenarios where model structure differences need to be checked.
...
> **Note**: After generating `compare_{timestamp}.vis.db` using the real data mode (`task="tensor"`), do not move, rename, or delete the collected tensor data files. Otherwise, the "manual node matching" feature will fail during comparison due to the inability to locate the tensor files.
...
1. Default matching
   ...
   - The node levels are the same (all parent nodes are the same).
