...
`ASCEND_LAUNCH_BLOCKING` is an environment variable that controls the operator execution mode in PyTorch training or online inference scenarios. If it is set to `1`, operators run in synchronous mode. If the computation result changes after the tool is used, you can set `ASCEND_LAUNCH_BLOCKING` to `1`. If the result still changes, the result change is caused by a synchronization operation. In this case, you need to reproduce the problem and locate the fault. You are advised to use the asynchronous dump function of msProbe by referring to the `async_dump` field in [Configuration](../user_guide/dump/config_json_introduct.md).
...
Currently, data collection supports the input and output of fusion operators. To use this function, add fusion operators to [support_wrap_ops.yaml](../../../python/msprobe/pytorch/dump/api_dump/support_wrap_ops.yaml) directory. In the following example, the softmax fusion operator is called.
...
4. `NameError: name 'torch_npu' is not defined` is displayed when the kernel-level data of a specified backward API is dumped.
   ...
   For an NPU environment, install TorchNPU; for a GPU environment, the kernel-level data of a specified API cannot be dumped.
