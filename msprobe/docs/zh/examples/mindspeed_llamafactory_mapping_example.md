...
数据采集前需要准备一个json文件，本案例命名为config.json，其内容包含了数据采集所需配置。

本案例使用的配置内容如下，详细介绍请参见[配置文件介绍](../user_guide/dump/config_json_introduct.md)。
...
本案例使用的工具采集接口配置如下，更多配置和接口介绍请参考[PyTorch场景精度数据采集](../user_guide/dump/pytorch_data_dump_instruct.md)。

#### LLamaFactory数据采集

LLamaFactory依赖Transformers的底层能力，msProbe工具采集功能将添加到Transformers中。
...
具体的参数说明请点击查看[分级可视化构图比对-分级可视化功能介绍](../user_guide/accuracy_compare/pytorch_visualization_instruct.md#分级可视化功能介绍)。

在基于MindSpeed和LLamaFactory框架的模型比对场景中，**-lm参数是必填的**，-lm参数所需的layer_mapping.yaml如何配置将在下面的章节进行介绍。

模型分级可视化比对完成后，可通过TensorBoard启动端口，在浏览器页面查看模型结构和精度比对结果，请参考[分级可视化构图比对-启动TensorBoard](../user_guide/accuracy_compare/pytorch_visualization_instruct.md#启动tensorboard)和[分级可视化构图比对-浏览器查看](../user_guide/accuracy_compare/pytorch_visualization_instruct.md#浏览器查看)。
...
请参考[分级可视化构图比对-手动选择节点匹配](../user_guide/accuracy_compare/pytorch_visualization_instruct.md#手动选择节点匹配)。
