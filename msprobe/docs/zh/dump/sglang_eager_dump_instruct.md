...
* 本文档仅适用于**SGLang版本<0.5.11**的数据采集，需通过侵入式修改SGLang源码的方式开启msProbe工具能力。若当前SGLang版本>=0.5.11，此版本及更高版本已原生内置msProbe工具，可直接指定参数`--msprobe-dump-config`进行精度数据采集，具体操作请参考文档《[SGLang精度数据采集](./sglang_eager_dump_instruct_new.md)》。
