...
1. 安装并正确配置 Ascend Extension for PyTorch（torch_npu）。
2. 安装 msProbe 工具，详见《[msProbe 安装指南](../msprobe_install_guide.md)》。
...
- 仅支持 PyTorch 框架。
- 构建 `aclgraph_dump` 需要 torch_npu 参与编译；若未包含该模块，将无法正常使用该功能。
