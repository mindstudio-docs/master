...
大模型场景下，通常不是简单地利用自动迁移能力实现从GPU到NPU的训练脚本迁移，而是会对NPU网络进行一系列针对性的适配，因此，常常会造成迁移后的NPU模型存在部分子结构不能与GPU原始模型完全对应。模型结构不一致导致API调用类型及数量不一致，若直接按照API粒度进行精度数据dump和比对，则无法完全比对所有的API。
...
>[!NOTE]
>
>`dump_enable`仅在需要动态开关dump时配置。运行中可将`dump_enable`从`false`改为`true`（或反向修改）实现动态开关，json中其他字段修改也能生效。
...
- MindSpore静态图O0/O1场景配置kernel_name，可以是算子的名称列表，也可以指定算子类型，还可以配置算子名称的正则表达式（当字符串符合“name-regex(xxx)”格式时），后台则会将其作为正则表达式。
...
- MindSpore静态图O0/O1场景：L0级别dump仅支持"all"、"forward"和"backward"参数；L2级别dump仅支持"all"、"input"和"output"参数。且各参数只能单独配置，不支持自由组合。
...
- PyTorch
  ...
  - 对tensor进行切片，未配置时默认值为空列表[]，表示不切片。
  - dict里的key可选值为["dim", "size", "begin", "end"]，value必须是整数。
  - `dim`：待切片的目标维度索引。取值≥0，未配置时默认值为0。
...
| `begin` + `end` | `"slice": [{"begin": 10, "end": 20}]` | 针对第0维，保留索引区间[10, 20)的数据（共10个）   |
| `dim` + `begin` + `end` | `"slice": [{"dim": 1, "begin": 10, "end": 20}]` | 针对第1维，保留索引区间[10, 20)的数据（共10个）   |
| `size` + `begin` + `end` | `"slice": [{"size": 50, "begin": 10, "end": 20}]` | 当第0维长度等于50时，保留索引区间[10, 20)的数据（共10个） |
| `dim` + `size` + `begin` + `end` | `"slice": [{"dim": 1, "size": 50, "begin": 10, "end": 20}]` | 当第1维长度等于50时，保留索引区间[10, 20)的数据（共10个） |
