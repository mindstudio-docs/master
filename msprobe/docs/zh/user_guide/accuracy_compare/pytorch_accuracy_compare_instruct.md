...
- 两个 API 的名称相同，API 命名规则：`{api_type}.{api_name}.{api调用次数}.{前向反向}.{输入输出}.{index}`，如：Functional.conv2d.1.backward.input.0。
...
| 参数名                                | 可选/必选 | 说明                                                                                                                                                                               |
|------------------------------------| -------- |----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| -tp或--target_path                  | 必选     | NPU环境下的dump.json路径（单卡场景）或dump目录（多卡场景），str 类型。                                                                                                                                    |
| -gp或--golden_path                  | 必选     | CPU、GPU或NPU环境下的dump.json路径（单卡场景）或dump目录（多卡场景），str 类型。                                                                                                                            |
| -o或--output_path                   | 可选     | 配置比对结果文件存盘目录，str 类型，默认在当前目录创建output目录。默认输出csv文件，文件名称基于时间戳自动生成，格式为：`compare_result_{timestamp}.csv`。传入`--xlsx`时输出xlsx文件。<br>提示：output目录下与结果文件同名的文件将被删除覆盖。                                                      |
| --xlsx                              | 可选     | 配置比对结果输出为xlsx格式。默认不配置，表示输出csv格式。                                                                                                                                                |
| -fm或--fuzzy_match                  | 可选     | 模糊匹配。开启后，对于网络中同一层级且命名相同仅调用次数不同的 API，可匹配并进行比对。通过直接配置该参数开启，默认未配置，表示关闭。                                                                                                             |
...
支持单卡和多卡，可同时比对多卡的 dump 数据。多机场景需要每个设备单独执行比对操作。
   ...
   > - 新增规则时，直接在对应侧追加 `API -> handler` 映射即可。
...
首差异算子节点识别场景是指：CPU或GPU与NPU环境的网络中通过`msprobe dump`保存数据的数据分析，找到网络精度问题中出现的首个差异算子节点。
...
单点数据比对场景：CPU 或 GPU 与 NPU 环境的网络中单点保存的数据比对。

单点数据比对支持单卡比对和多卡比对。多机场景需要每个设备单独执行比对操作。

1. 参见《[单点保存工具](../dump/debugger_save_instruct.md)》完成 CPU 或 GPU 与 NPU 的动态图场景单点数据采集。
...
- 单卡场景：默认在配置的输出路径中生成.csv后缀的文件，文件名称基于时间戳自动生成，格式为：compare_result\_{timestamp}.csv。若配置`--xlsx`，则生成.xlsx后缀文件，格式为：compare_result\_{timestamp}.xlsx。

- 多卡场景：默认在配置的输出路径中生成多个.csv后缀的文件，文件名称基于时间戳自动生成，格式为：compare_result_rank{rank_id}_{timestamp}.csv。若配置`--xlsx`，则生成多个.xlsx后缀文件，格式为：compare_result_rank{rank_id}_{timestamp}.xlsx。
...
- 动态图单点数据比对场景：
   ...
   默认在配置的输出路径中生成.csv后缀的文件，文件名称基于时间戳自动生成，格式为：debug_compare_result_(rank_id/proc_id)_{timestamp}.csv。若配置`--xlsx`，则生成.xlsx后缀的文件，格式为：debug_compare_result_(rank_id/proc_id)_{timestamp}.xlsx。
...
默认输出的 `compare_result_{timestamp}.csv` 文件列出了所有执行精度比对的 API 详细信息和比对结果；配置`--xlsx`时，输出文件为 `compare_result_{timestamp}.xlsx`。示例如下：
...
**提示**：在比对结果文件中，可以通过：Result（比对结果）、Err_Message（错误信息提示）定位可疑算子，但鉴于每种指标都有对应的判定标准，还需要结合实际情况进行判断。
...
MD5模式：数据采集时配置config.json中task字段为"statistics"，summary_mode字段为"md5"或"xor"。配置为"md5"时，仅落盘统计量和md5值；配置为"xor"时，仅落盘XOR校验值。
...
2. EucDist：通过计算两个向量的欧氏距离来判断其相似度，定义为多维空间中两个点之间的绝对距离。数值越接近0，张量越相似，数值越大，差异越大。
...
| -o或--output_dir     | 必选     | 数据提取汇总结果存盘目录，str类型。文件名称基于时间戳自动生成，格式为：`multi_ranks_compare_merge_{timestamp}.xlsx`。<br>提示：output目录下与结果文件同名的文件将被删除覆盖。 |
...
以Distributed.all_reduce.0.forward为例。这个API将多卡数据规约操作输出为一个group内的规约结果，同一个group内的输出保持一致。<br>这个API中，rank0-3为一个group，Distributed.all_reduce.0.forward.input.group展示为tp-0-1-2-3，rank0-3输出一致；rank4-7为一个group，展示为tp-4-5-6-7，rank4-7输出一致。<br>group除了这种形式，还有如[0, 1, 2, 3]的呈现形式。
...
API和模块名称请从《[PyTorch场景精度数据采集](../dump/pytorch_data_dump_instruct.md)》中的dump.json文件获取。
