...
- 仅支持PyTorch框架。
- 支持PyTorch版本：请参考[版本说明](https://gitcode.com/Ascend/msprobe/releases)。
...
可通过浏览器界面，鼠标选择两个待匹配的灰色节点进行匹配。

> **须知**：使用真实数据模式（task="tensor"）生成 compare_{timestamp}.vis.db 后，**请勿移动、重命名或删除采集的tensor数据文件**，否则比对时会因找不到tensor文件导致“手动选择节点匹配”失败。
