...
| 参数            | 可选/必选 | 说明|
|---------------|-------|----------------------------------------------|
| -d或--dump   | 必选  | 数据采集模式，可选择是否传入启动训练的用户shell脚本路径或yaml配置文件路径。支持同时传入shell脚本和yaml配置文件，表示采集训练超参、环境变量和第三方库版本，默认都不传入，表示只采集环境变量和第三方库版本。shell脚本和yaml配置文件均可采集训练超参，取决于训练超参所在文件的位置。 |
...
以上六项分别对应环境变量、第三方库版本、数据集、权重、训练超参、随机操作检查。
