...
<table>
    ...
    <tr>
        ...
        <td>采集支持device md5、支持stack/construct开关、支持采集黑名单；统计值采集/溢出节点检测/比对性能优化；可视化支持tensor级别点点匹配、匹配功能增强。 </td>
    </tr>
    <tr>
        <td>2</td>
        <td>pytorch图模式场景精度定位能力解决方案</td>
        <td>支持Inductor（triton后端）、mlir、akg、dvm、eager->aot eager的自动比对，支持aclgraph整网数据采集</td>
    </tr>
    <tr>
        <td>3</td>
        <td>强化学习精度定位能力增强</td>
...
- 强化学习精度定位：进一步扩展对强化学习中最新推理场景的支持度，加强配置自动检查以及定界能力
...
- 采集支持device md5
  ...
  - 需求内容：通过异或等手段实现device侧md5计算，集成算子侧提供的device md5融合算子

- 支持stack/construct开关
  - 需求背景： 网络训练时，默认每一步都会采集dump.json、stack.json和construct.json。其中stack.json和construct.json在不同步之间存在大量重复，不建议每步都保存
...
2. 精度比对工具支持mlir、akg、dvm、编译多阶段（eager->aot eager）
...
- 精度比对工具支持mlir、akg、dvm、支持编译多阶段（eager->aot eager）的自动精度比对
  - 需求背景：图模式支持度欠缺
  - 需求目标：为图模式mlir、akg后端、dvm、编译多阶段（eager->aot eager）支持精度工具
- 支持aclgraph场景整网数据采集
