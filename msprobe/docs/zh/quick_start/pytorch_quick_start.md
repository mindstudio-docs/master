...
使用msProbe工具在模型精度调试中主要执行如下操作：
   ...
   1. 训练前配置检查
      ...
      识别两个环境影响精度的配置差异，具体请参见《[训练前配置检查](../user_guide/config_check_instruct.md)》。

   2. 训练状态监测

      监测训练过程中计算、通信、优化器等部分出现的异常情况，具体请参见《[训练状态监测](../user_guide/monitor_instruct.md)》。

   3. 精度数据采集

      采集训练过程中的API或Module层级前反向输入输出数据，具体请参见《[数据采集](../user_guide/dump/pytorch_data_dump_instruct.md)》。

   4. 精度预检

      扫描API数据，找出存在精度问题的API，具体请参见《[精度预检](../user_guide/accuracy_checker/pytorch_accuracy_checker_instruct.md)》。

   5. 精度比对

      对比NPU侧和标杆环境的API数据，快速定位精度问题，具体请参见《[精度比对](../user_guide/accuracy_compare/pytorch_accuracy_compare_instruct.md)》。
...
3. 分别在GPU和昇腾NPU环境下的训练脚本（pytorch_main.py文件）中添加工具，如下所示。
   ...
   **说明**：[PyTorch精度数据采集代码样例](#pytorch精度数据采集代码样例)中的完整代码已添加工具，下列仅为说明工具接口在脚本中添加的位置。
   ...
   334         debugger.start(model)  # 开启数据dump
   ...
   356
   ...
   **说明**：精度数据会占据一定的磁盘空间，可能存在磁盘写满导致服务器不可用的风险。精度数据所需空间跟模型的参数、采集开关配置、采集的迭代数量有较大关系，须用户自行保证落盘目录下的可用磁盘空间。
...
├── step0
    └── proc3209296                  # 单卡训练中，训练进程没有rank信息，此时数据保存在proc{pid}，多卡场景下为rank{id}
        ├── construct.json           # 保存Module的层级关系信息
...
2. 执行比对。
   ...
   命令如下，具体参数使用参见[参数说明](../user_guide/accuracy_compare/pytorch_accuracy_compare_instruct.md#参数说明)：
...
3. 比对结果文件分析。
   ...
   更多比对结果分析请参见“[精度比对结果分析](../user_guide/accuracy_compare/pytorch_accuracy_compare_instruct.md#精度比对结果分析)”。
...
2. 执行图构建比对, 具体参数使用参见[双图比对](../user_guide/accuracy_compare/pytorch_visualization_instruct.md#双图比对)的参数说明。
