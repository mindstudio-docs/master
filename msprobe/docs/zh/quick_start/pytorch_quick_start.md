...
使用msProbe工具在模型精度调试中主要执行如下操作：
   ...
   1. 训练前配置检查
      ...
      识别两个环境影响精度的配置差异，具体请参见《[训练前配置检查](../config_check_instruct.md)》。

   2. 训练状态监测

      监测训练过程中计算、通信、优化器等部分出现的异常情况，具体请参见《[训练状态监测](../monitor_instruct.md)》。

   3. 精度数据采集

      采集训练过程中的API或Module层级前反向输入输出数据，具体请参见《[数据采集](../dump/pytorch_data_dump_instruct.md)》。

   4. 精度预检

      扫描API数据，找出存在精度问题的API，具体请参见《[精度预检](../accuracy_checker/pytorch_accuracy_checker_instruct.md)》。

   5. 精度比对

      对比NPU侧和标杆环境的API数据，快速定位精度问题，具体请参见《[精度比对](../accuracy_compare/pytorch_accuracy_compare_instruct.md)》。
...
2. 执行比对。
   ...
   命令如下, 具体参数使用参见[参数说明](../accuracy_compare/pytorch_accuracy_compare_instruct.md#参数说明)：
...
2. 执行图构建比对, 具体参数使用参见[双图比对](../accuracy_compare/pytorch_visualization_instruct.md#双图比对)的参数说明。。
