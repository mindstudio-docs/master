...
使用msProbe工具在模型精度调试中主要执行如下操作：
   ...
   3. 精度数据采集
      ...
      采集训练过程中的API或Module层级前反向输入输出数据，具体请参见《[数据采集](../user_guide/dump/mindspore_data_dump_instruct.md)》。

   4. 精度预检

      扫描API数据，找出存在精度问题的API，具体请参见《[精度预检](../user_guide/accuracy_checker/mindspore_accuracy_checker_instruct.md)》。

   5. 精度比对

      对比NPU侧和标杆环境的API数据，快速定位精度问题，具体请参见《[精度比对](../user_guide/accuracy_compare/mindspore_accuracy_compare_instruct.md)》。
...
3. 比对结果文件分析。
   ...
   更多比对结果分析请参见“[输出结果文件说明](../user_guide/accuracy_compare/mindspore_accuracy_compare_instruct.md#输出结果文件说明)”。
