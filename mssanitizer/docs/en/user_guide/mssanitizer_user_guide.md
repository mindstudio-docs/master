...
| `synccheck` | Synchronization failure caused by incorrect use of synchronization instructions| `mssanitizer --tool=synccheck ./application` |

### Application Scenario

| Development Scenario| Reference Section|
| --- | --- |
...
| Integrating operators in the PyTorch framework (TorchAir graph mode, and so on)| [4.3 PyTorch Framework Adaptation Scenario](#43-pytorch-framework-adaptation-scenario)|
...
2. In PyTorch graph mode (TorchAir), the Ascend IR graph execution mode and aclgraph graph execution mode are supported. For details, see "PyTorch Graph Mode (TorchAir)" > "Reduce-Overhead Mode" > "Configuring the Reduce-Overhead Mode" in the [TorchNPU](https://www.hiascend.com/document/detail/en/Pytorch/720/modthirdparty/torchairuseguide/torchair_00015.html).
3. For details about the PyTorch framework calling scenario, see "PyTorch Framework Feature Guide" > "Custom Operator Adaptation Development" > "OpPlugin-based Operator Adaptation Development" in [TorchNPU](https://www.hiascend.com/document/detail/en/Pytorch/2610/devguide/fwfeatures/docs/en/framework_feature_guide_pytorch/adaptation_overview_opplugin.md). For details, see "Checking Operators Called by PyTorch APIs" in [example](../best_practices/mssanitizer_basic_cases.md).
...
> - This scenario applies only to the operator kernel launch scenario.
...
> [!NOTE]
>
> Exception report level description.
>
...
| [GM Memory Safe Area Out-of-Bounds Write](#61310-gm-memory-safe-area-out-of-bounds-write)| When the aicpu tiling downshift function is used, the out-of-bounds write behavior of the tiling function may occur.| Host | GM |
...
| Function| Command|
| --- | --- |
| Explicitly specifies memory check. By default, illegal read/write, multi-core corruption, unaligned access, and illegal release checks are enabled.| `mssanitizer --tool=memcheck application`|
| Manually enables memory leak check in addition to the memcheck check items.| `mssanitizer --tool=memcheck --leak-check=yes application` |
| Manually enables the check for unused allocated memory in addition to the memcheck check items.| `mssanitizer --tool=memcheck --check-unused-memory=yes application`|
...
```text
====== WARNING: Unused memory of 1000 byte(s)     // Basic error information, indicating that an allocated memory unused exception is detected
======    at 0x1240c0016000 on GM                    // Memory location where the exception occurs, including the address space and memory address
======    code in add_custom.cpp:2 (serialNo:69)   // Code location where the exception occurs, including the file name, line number, and sequence number of the API call behavior
====== SUMMARY: 1100byte(s) unused memory in 2 allocation(s) // Summary of unused memory, including the number of unused memory blocks and bytes
```

##### 6.1.3.7 Ascend 950 SIMT Unit Exception

In the SIMT architecture, the location of the thread where the exception occurs is also provided. The thread ID starts from 0. For example, the following exception occurs at the thread whose ID X=1, ID Y=0, and ID Z=0. When the SIMT unit of the Ascend 950 products is abnormal, the error information is displayed as follows:
...
##### 6.1.3.10 GM Memory Safe Area Out-of-Bounds Write

When you develop a custom operator that uses the aicpu tiling downshift function, out-of-bounds write behavior may occur in the tiling function. When the check tool launches the operator, it determines whether an out-of-bounds write occurs by checking the buffer content, and reports an alarm if an out-of-bounds exception occurs. Currently, the alarm displays the memory address and the out-of-bounds length.

```text
====== ERROR: illegal write of size 16 byte(s)
======    Access to 0x12c0c0017010 on GM is 16 byte(s) after the nearest allocation at 0x12c0c0016000 of size 4096 byte(s)
======
    in MyAicpuKernel on device 0
```

> [!NOTE]
>
> GM memory safe area out-of-bounds write does not support detection in memory pool scenarios.
> Currently, only Atlas A2 training products/Atlas A2 inference products, Atlas A3 training products/Atlas A3 inference products, and Ascend 950 products are supported.
...
When running the msSanitizer tool, run the following command to enable race check. In the command, `application` indicates the user program:

```shell
mssanitizer --tool=racecheck application
```

Run the following command to additionally enable cross-NPU race check:
...
- The following example indicates a write-write race risk on the GM in the operator program. The PIPE_MTE3 pipelines of core 0 on device 0 and device 1 both write to the 0x12c0c0017000 address. This operation corresponds to line 39 in the operator implementation file `kernel.cpp`.

    ```text
    ====== ERROR: Potential WAW hazard detected at GM in kernel:
    ======    PIPE_MTE3 Write at WAW()+0x12c0c0017000 in block 0 (aiv) on device 1 at pc current 0x1038 (serialNo:16)
    ======    #0 cross_npu_check/race_when_lack_of_cross_core_sync/kernel.cpp:39:9
    ======    PIPE_MTE3 Write at WAW()+0x12c0c0017000 in block 0 (aiv) on device 0 at pc current 0x1038 (serialNo:16)
    ======    #0 cross_npu_check/race_when_lack_of_cross_core_sync/kernel.cpp:39:9
    ```
...
**Table 3** Uninitialization exception types
...
> - Accurate uninitialization check results require that no data race exists. Because initialization is essentially a write-before-read operation, the uninitialization check results may be inaccurate if a memory read/write race exists. Therefore, you are advised to resolve race problems before performing the uninitialization check.
> - Currently, only Atlas A2 training products/Atlas A2 inference products, Atlas A3 training products/Atlas A3 inference products, and Ascend 950 products are supported.
...
The following example shows an uninitialized read of 224 bytes on the kernel side of the operator program. The read occurs on the GM and corresponds to line 18 in the code `add_custom.cpp`.
...
For Ascend 950 products, the SIMT unit also supports the inter-thread synchronization instruction `__syncthreads()`, which synchronizes all threads in a block so that subsequent instructions are executed only after all threads reach the synchronization point. Therefore, you must not specify that only some threads execute `__syncthreads()`. The synchronization check function identifies such behavior and reports an error to remind you that a synchronization exception exists.
...
**Table 4** Synchronization exception types
...
| Operator hang check| The operator hangs because synchronization instructions such as `wait_flag`/`wait_flag_dev` are incorrectly used. Operator hang check displays the instruction at the location of the stuck pipeline and the call stack.| Kernel |
| Thread synchronization check| The `__syncthreads()` synchronization instruction is incorrectly used in the operator, failing to synchronize all threads.| Kernel |
...
##### 6.4.3.2 Redundancy Check

If redundant `SetFlag` instructions exist, all subsequent operator synchronization instructions fail to pair, leading to exceptions.
...
##### 6.4.3.3 Operator Hang Check

- When you use Ctrl-C to terminate the operator, the tool prompts that the operator has been terminated, and you can press Ctrl-C again to terminate the tool.

    ```text
    Ctrl-C received. Running kernel will be killed, and you can press Ctrl-C again to force quit.
    ```

- When the operator terminated by Ctrl-C stops running, the tool prints the operator process exit information.
...
##### 6.4.3.4 Thread Synchronization Check

If the SIMT unit of the Ascend 950 products incorrectly uses the `__syncthreads()` synchronization instruction, a thread synchronization check alarm is triggered, and the tool prints information about all threads that have not reached the synchronization point.

```text
====== ERROR: Sync error detected. Divergent thread(s) in vec_add
======    by thread(0,0,0) in block aiv(0) on device 0
======    code in pc current 0x1d8 (serialNo:16)
======    #0 /data/xxx/kernel.cpp:82:9
```
...
1. `options`: CLI options of the tool. For details about the options and their default arguments, see [7.2 Options](#72-options).
2. `user_program`: user operator program.
3. `user_options`: CLI options of the user program.
4. `--`: If the program to be loaded contains CLI options, use `--` to separate the tool from the user program before the program, for example, `mssanitizer -- application parameter1 parameter2 ...`.
...
| --padding | Specifies the length of the GM memory safe area.| The value is an integer ranging from 32 to 1024, in bytes.<br>Defaults to 32.| No|
...
**Table 3: Race check options**

| Option| Description| Value| Mandatory (Yes or No)|
| --- | --- | --- | --- |
| --check-cross-npu-races | Enables cross-NPU race check.| `yes`/`no` (default)| No|
...
### 7.4 CLI Parameter Auto-Completion

The exception check tool provides CLI parameter auto-completion. Before using this function, ensure that the `bash-completion` package is installed on the system to support auto-completion in bash.

Run the following command to activate auto-completion in the current shell session:

```shell
source ${ASCEND_HOME_PATH}/tools/mssanitizer/bin/mssanitizer-bash-completion
```

Copy the auto-completion script to the user-level bash completion directory. Auto-completion is activated automatically when you log in to a shell session:

```shell
# Ensure that the bash completion directory is created
mkdir -p ~/.local/share/bash-completion/completions
# Copy the mssanitizer auto-completion script
...
After activation, press `<TAB>` once to complete a unique candidate, or press `<TAB>` twice to list all candidates:
...
5. msSanitizer currently supports the check of operators whose `blockDim` is less than 100.
