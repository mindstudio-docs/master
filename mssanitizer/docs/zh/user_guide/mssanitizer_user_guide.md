...
| `synccheck` | 未正确使用同步指令导致的同步失败 | `mssanitizer --tool=synccheck ./application` |
...
2. PyTorch图模式（TorchAir）下，支持Ascend IR图执行模式和aclgraph图执行模式，具体请参见《[TorchNPU](https://www.hiascend.com/document/detail/zh/Pytorch/720/modthirdparty/torchairuseguide/torchair_00015.html)》中“PyTorch图模式使用(TorchAir)>reduce-overhead模式功能>reduce-overhead模式配置”章节。
3. PyTorch框架调用场景，详细信息可参考《[TorchNPU](https://www.hiascend.com/document/detail/zh/Pytorch/latest/ptmoddevg/Frameworkfeatures/docs/zh/framework_feature_guide_pytorch/adaptation_overview_opplugin.md)》中“PyTorch框架特性指南>自定义算子适配开发>基于OpPlugin算子适配开发”章节。具体操作请参见《[基础案例](../best_practices/mssanitizer_basic_cases.md)》中的“检测PyTorch接口调用的算子”。
...
| [Ascend 950PR&950DT系列产品SIMT单元异常信息](#6137-ascend-950pr950dt系列产品simt单元异常信息) | SIMT架构下展示异常信息发生的线程位置。 | Kernel | GM |
...
> - msSanitizer工具也支持对Atlas A2系列产品的AllReduce、AllGather、ReduceScatter、AlltoAll接口及Atlas A3系列产品的AllGather、ReduceScatter、AlltoAllV接口进行非法读写的检测，具体介绍请参见《[Ascend C算子开发接口](https://www.hiascend.com/document/redirect/CannCommercialAscendCApi)》中的“高阶API > Hccl >  Hccl Kernel侧接口”章节。
> - msSanitizer工具也支持对通算融合类算子的非法读写检测。
> - 当前Ascend 950PR&950DT系列产品支持内存检测中GM/UB/L1/L0A/L0B/L0C，其他暂不支持。
...
======    at 0x1240c0016000 on GM                    // 异常发生的内存位置信息,包含发生的地址空间与内存地址
...
##### 6.1.3.7 Ascend 950PR&950DT系列产品SIMT单元异常信息

SIMT架构下异常信息的展示会额外提供异常信息发生的线程位置，线程id从0开始计数，例如下方异常信息发生在线程idX=1 idY=0，idZ=0处。Ascend 950PR&950DT系列产品SIMT单元异常时，错误信息展示如下：
...
>
> 当前仅内存检测支持寄存器告警。
> --trace-non-default-spr-reg=vector功能当前仅支持Atlas A2系列产品、Atlas A3系列产品。

##### 6.1.3.10 GM内存安全区越界写入

开发自定义算子使用aicpu tiling下沉功能时，tiling函数中可能出现越界写行为，使用检测工具拉起算子时通过检查缓冲区的内容判断是否出现越界，并在出现越界异常时以告警的形式进行提示。当前告警信息展示内存地址和越界长度。
...
> 当前仅支持Atlas A2系列产品、Atlas A3系列产品以及Ascend 950PR&950DT系列产品。
...
运行msSanitizer工具时，执行如下命令，启用竞争检测功能（racecheck）。其中 application 为用户程序：

```shell
mssanitizer --tool=racecheck application
```

执行如下命令，可以额外开启卡间竞争检测功能：
...
- 以下示例表示了算子程序中存在对 GM 内存的写写竞争风险。其中 0 卡和 1 卡的 0 核的 PIPE\_MTE3 流水均存在对 0x12c0c0017000 地址的写入，该操作对应算子实现文件 kernel.cpp 的第 39 行。
...
> - 当前仅支持Atlas A2系列产品、Atlas A3系列产品以及Ascend 950PR&950DT系列产品。
> - Ascend 950PR&950DT系列产品不支持UB地址空间未初始化检测。
...
以下示例表示了算子程序中kernel侧存在224字节的未初始化读，发生在GM上，出现在代码add_custom.cpp的第18行。
...
对于Ascend 950PR&950DT系列产品，SIMT单元还存在线程间同步指令__syncthreads()，作用是同步block内的所有线程，到达同步点后再执行后续指令。因此禁止用户指定部分线程执行__syncthreads()，同步检测功能会识别此类行为并上报错误信息，提醒用户存在同步异常。
...
| 线程同步检测 | 算子中错误使用__syncthreads()同步指令，未能实现所有线程之间的同步。 | Kernel |
...
若Ascend 950PR&950DT系列产品SIMT单元未正确使用__syncthreads()同步指令，将触发线程同步检测告警，工具会打印出所有未达到同步点的线程信息。
...
| --padding | 指定GM内存安全区长度 | 可设定范围为 32~1024 之间的整数，单位为字节。<br>默认值为32。| 否 |
| -h，--help | 输出帮助信息。 | - | 否 |
