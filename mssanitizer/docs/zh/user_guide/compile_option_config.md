...
2. 进行算子代码编译，需添加以下编译选项：
    ...
    增加--cce-enable-sanitizer或--sanitizer选项代表使能异常检测，两个选项完全等价，可随意使用其一。
    ...
    > - 目前Ascend 950 系列产品不支持--cce-enable-sanitizer或--sanitizer以及-O0编译选项。如需在kernel侧添加-g编译选项，添加后若算子中存在异常信息，则工具会展示异常信息对应的调用栈信息。
...
3. 链接阶段需增加target_link_options选项。
    ...
    - 编辑样例工程目录下的“cmake/Modules/CMakeCCEInformation.cmake”文件，增加${_CMAKE_COMPILE_AS_CCE_FLAG}链接选项。
...
1. 重新编译算子，确认编译过程无报错。
2. 使用 msSanitizer 工具拉起算子进行检测，具体用法请参见《[MindStudio Sanitizer 使用指南](./mssanitizer_user_guide.md)》。
3. 若检测报告中的文件名和行号显示异常，请参见《[MindStudio Sanitizer 常见问题](../support/mssanitizer_faq.md)》。
