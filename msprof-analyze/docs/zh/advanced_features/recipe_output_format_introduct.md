# 解析结果和cluster_analysis.db交付件表结构说明
...
设置Level_none时，由于msprof_{时间戳}.db文件里没有COMMUNICATION_OP，该分析能力会将通信内置打点数据转换成通信算子，并且可以在MindStudio Insight中呈现。
