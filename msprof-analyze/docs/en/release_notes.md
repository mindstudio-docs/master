...
- MindStudio Profiler Analyze (`msprof-analyze`) supports AscendPyTorch 1.11.0 and later. For version mapping between PyTorch, CANN, and Python, see [Ascend Extension for PyTorch](https://gitcode.com/Ascend/pytorch).
- `msprof-analyze` supports MindSpore 2.4.0 and later. For version mapping between MindSpore, CANN, and Python, see [MindSpore Release List](https://www.mindspore.cn/versions/en).
- The firmware and driver versions supported by `msprof-analyze` are identical to those supported by the corresponding CANN software. Developers can obtain matching firmware and drivers based on the product model and CANN version from the [Firmware and Drivers](https://www.hiascend.com/hardware/firmware-drivers/community?product=2&model=28&cann=8.0.RC3.alpha003&driver=1.0.25.alpha) page on the Ascend Community website.

## Release Package Download Links
