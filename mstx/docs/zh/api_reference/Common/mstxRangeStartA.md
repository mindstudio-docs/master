...
|stream|输入|stream表示使用mark的线程。<br>C/C++中数据类型：aclrtStream。<br>Python中stream是aclrtStream对象。默认None。<br>配置为nullptr时，只标记Host侧的瞬时事件。<br>配置为有效的stream时，标记Host侧和对应Device侧的瞬时事件。|
