...
根据调优目标，您可以参考以下路径选择阅读重点：

- **判断算子瓶颈类型**：先阅读[详情（Details）](#详情details)中的Roofline瓶颈分析，判断是计算瓶颈还是内存瓶颈。
- **分析计算瓶颈**：阅读[详情（Details）](#详情details)中的计算工作负载分析和核间负载分析。
- **分析内存瓶颈**：阅读[详情（Details）](#详情details)中的内存负载分析和[缓存（Cache）](#缓存cache)。
- **定位高耗时指令**：阅读[时间线（Timeline）](#时间线timeline)。
- **关联指令到源码**：阅读[源码（Source）](#源码source)。
- **分析Triton算子内存溢出**：阅读[片上内存（On-Chip Memory）](#片上内存on-chip-memory)。
- **对比优化前后效果**：阅读[详情（Details）](#详情details)中的“支持算子间性能对比”。
...
算子调优场景的数据由msOpProf工具采集生成，请根据实际需求选择采集模式：

|采集模式|适用场景|采集命令示例|生成文件|
|--|--|--|--|
|上板采集（msopprof）|需要真实硬件性能数据，分析实际运行耗时、内存带宽、Cache行为等。|`msopprof --output=./output_dir ./op_executable`|visualize_data.bin|
|仿真采集（msopprof simulator）|无真实硬件或需要指令级详细分析，如指令流水追踪、代码热点定位等。|`msopprof simulator --soc-version=Ascendxxxyy --output=./output_dir ./op_executable`|trace.json、visualize_data.bin|
...
> - 更多采集参数和用法请参见[msopprof用户指南](https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/user_guide/msopprof_user_guide.md)和[msopprof simulator用户指南](https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/user_guide/msopprof_simulator_user_guide.md)。

**数据导入**

采集完成后，将生成的性能数据文件导入MindStudio Insight工具，具体数据说明请参见[数据说明](#数据说明)，数据导入操作请参见[导入数据](./basic_operations.md#导入数据)。
...
|显示界面|文件名|说明|文件获取方式|
|--|--|--|--|
|时间线（Timeline）|trace.json|算子仿真指令流水图文件。|仅仿真模式。参见[msopprof simulator用户指南](https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/user_guide/msopprof_simulator_user_guide.md)。|
|时间线（Timeline）|visualize_data.bin|指令流水图可视化呈现文件。|上板+仿真模式。参见[msopprof用户指南](https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/user_guide/msopprof_user_guide.md)和[msopprof simulator用户指南](https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/user_guide/msopprof_simulator_user_guide.md)。|
|详情（Details）|visualize_data.bin|算子基础信息、计算单元负载和Roofline瓶颈分析等信息的可视化呈现文件。|仅上板模式。参见[msopprof用户指南](https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/user_guide/msopprof_user_guide.md)。|
|源码（Source）|visualize_data.bin|仿真热点函数等信息可视化呈现文件。|上板+仿真模式。参见[msopprof用户指南](https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/user_guide/msopprof_user_guide.md)和[msopprof simulator用户指南](https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/user_guide/msopprof_simulator_user_guide.md)。|
|缓存（Cache）|visualize_data.bin|用户程序Kernel函数内的L2 Cache访问信息可视化呈现文件。|仅上板模式。参见[msopprof用户指南](https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/user_guide/msopprof_user_guide.md)。|
...
- MindStudio Insight在算子调优场景下，支持导入的json文件中，需要在文件内容中第一个中括号之前有"profilingType":"op"字段标识，才可被导入。由msOpProf工具生成的json文件默认包含此字段；若导入失败，请检查json文件内容是否包含此字段。
...
- 仅<term>Atlas 350 加速卡</term>支持通过“msopprof”方式采集指令流水图可视化呈现文件，数据展示在时间线（Timeline）界面。其他硬件型号可通过“msopprof simulator”仿真模式采集trace.json文件，在时间线界面查看指令流水。

**术语说明**

|术语|全称|说明|
|--|--|--|
|HBM|High Bandwidth Memory|高带宽存储器，AI处理器的外部主存储器，容量大但访问延迟较高。|
|GM|Global Memory|全局内存，即HBM，算子可访问的全局存储空间。|
|L2|L2 Cache|二级缓存，位于HBM和AI Core之间，用于缓存频繁访问的数据以降低访问延迟。|
|L1|L1 Buffer|一级缓存，AI Core内部的存储空间，供Cube和Vector计算单元共享。|
|L0A/L0B|L0A/L0B Buffer|Cube计算单元的输入缓冲区，分别存储矩阵乘的左矩阵和右矩阵。|
|L0C|L0C Buffer|Cube计算单元的输出缓冲区，存储矩阵乘的结果。|
|UB|Unified Buffer|统一缓冲区，Vector计算单元的专用存储空间，用于向量运算的数据输入输出。|
|MTE1|Memory Transfer Engine 1|数据搬运引擎1，搬运方向：L1 → {L0A, L0B, UB}。|
|MTE2|Memory Transfer Engine 2|数据搬运引擎2，搬运方向：{DDR/GM, L2} → {L1, L0A/B, UB}。|
|MTE3|Memory Transfer Engine 3|数据搬运引擎3，搬运方向：UB → {DDR/GM, L2, L1}、L1 → {DDR/L2}。|
|FIXP|Fix Pipeline|数据搬运流水，搬运方向：L0C → OUT/L1。|
|Pipe|Pipeline|指令流水线，AI Core中不同类型指令的执行通道。|
|Core|AI Core|AI处理器中的计算核心，包含Cube、Vector等计算单元。|
|Block|Sub Block|算子执行时的并行分块，每个Block在一个AI Core上运行。|
...
|SCALAR|标量运算单元。|
|FLOWCTRL|控制流指令。|
|MTE1|数据搬运流水，数据搬运方向为：L1 ->{L0A/L0B, UB}。|
|CUBE|矩阵乘运算单元。|
|FIXP|数据搬运流水，数据搬运方向为：FIXPIPE L0C -> OUT/L1。<br> 仅<term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>导出的性能数据支持展示。|
|MTE2|数据搬运流水，数据搬运方向为：{DDR/GM, L2} ->{L1, L0A/B, UB}。|
|VECTOR|向量运算单元。|
|MTE3|数据搬运流水，数据搬运方向为：UB -> {DDR/GM, L2, L1}、L1->{DDR/L2}。|
|CACHEMISS|未命中ICACHE。|
|USEMASK|自定义打点范围。|
...
详情（Details）界面用于展示算子基础信息、计算负载分析、核间负载分析、Roofline瓶颈分析、Top Warp Stall Season和内存负载分析，并以图形和数据窗格呈现方式展示分析结果。

### 界面介绍

详情（Details）界面包含如下六个区域：

- 区域一：基础信息（Base Info），如[**图1** 基础信息](#基础信息)所示。
- 区域二：核间负载分析（Core Occupancy），如[**图2** 核间负载分析](#核间负载分析)所示。
- 区域三：Roofline瓶颈分析（Roofline），如[**图3** Roofline瓶颈分析-内存单元](#内存单元)所示。
- 区域四：计算工作负载分析（Compute Workload Analysis），如[**图4** 计算工作负载分析](#计算工作负载分析)所示。
- 区域五：Top Warp Stall Season，如[**图5** Top Warp Stall Season](#TopWarpStallSeason)所示。
- 区域六：内存负载分析（Memory Workload Analysis），如[**图6** 内存负载分析](#内存负载分析)所示。

**基础信息（Base Info）**

区域一是基础信息（Base Info），可查看算子基础信息，包含名称、时长、类型等内容，详细说明如[**表 1**  基础信息字段说明](#基础信息字段说明)所示。

**图 1**  基础信息<a id="基础信息"></a>
![](./figures/operator_tuning/basic_info_1.png "基础信息")

**表 1**  基础信息字段说明<a id="基础信息字段说明"></a>
...
**Top Warp Stall Season**

区域五是Top Warp Stall Season图，主要展示指令在执行过程中，由于资源冲突、数据依赖等原因而产生的等待次数，包含active，如[**图 8**  Top Warp Stall Season](#TopWarpStallSeason)所示，参数解释见[**表 9**  Top Warp Stall Season参数说明](#TopWarpStallSeason参数说明)。

**图 8**  Top Warp Stall Season<a id="TopWarpStallSeason"></a>
![](./figures/operator_tuning/top_warp_stall_season_1.png "Top Warp Stall Season")

**表 9**  Top Warp Stall Season参数说明<a id="TopWarpStallSeason参数说明"></a>

|中文参数|英文参数|说明|
|--|--|--|
|Nop阻塞|Nop_Cycles|空操作周期。|
|Divergence栈溢出阻塞|Divergence_Stack_Spill|分支发散栈溢出。在执行分支指令时，由于栈空间不足或分支路径导致的预测与实际路径不一致，从而引发的流水线阻塞周期。|
|IBuf空阻塞|IBuf_Empty|指令缓冲为空。指IBuf为空时，因无法获取下一条指令而导致的流水线阻塞周期。|
|寄存器冲突阻塞|Register_bank_conflict|寄存器Bank冲突。由于指令访问同一组寄存器时发生冲突，导致流水线暂停以避免数据错误的阻塞周期。|
|资源冲突阻塞|Resource_conflict|资源冲突。由于多个指令同时竞争同一硬件资源而无法并行执行，导致流水线暂停的阻塞周期。|
|ScoreBoard未准备阻塞|ScoreBoard_Not_Ready|记分牌未就绪。由于ScoreBoard状态尚未就绪（如前序指令仍在占用相关资源或寄存器），当前指令无法继续执行而导致的流水线阻塞。|
|Warp Level同步阻塞|Warp_Level_Sync|Warp级同步等待。在多线程执行中，为确保同Warp内线程的同步与一致性，在未满足同步条件时对指令执行进行的阻塞。|
|active|active|无阻塞成功发射指令的次数。|
|其他阻塞|Others|指除已明确分类的阻塞原因外，由其他异常或控制逻辑触发的流水线暂停周期。|

**内存负载分析（Memory Workload Analysis）**

区域六是内存负载分析（Memory Workload Analysis），以内存热力图和数据窗格呈现方式查看相应信息，如[**图 9**  内存负载分析](#内存负载分析)所示，参数配置如[**表 10**  参数配置说明](#参数配置说明)所示，内存热力图和数据窗格的参数是依据实际采集的数据进行展示。热力图右侧的“峰值\(%\)”为箭头颜色，值为峰值带占比（最大带宽占比）。图标![](./figures/operator_tuning/zh-cn_image_0000002532040351.png)中的内容为各个Block的内存负载分析结果。

**图 9**  内存负载分析<a id="内存负载分析"></a>
![](./figures/operator_tuning/memory_load_analysis_1.png "内存负载分析")

**表 10**  参数配置说明<a id="参数配置说明"></a>
...
在算子对比模式下，详情（Details）界面分别从基础信息（Base Info）、核间负载分析（Core Occupancy）、计算工作负载分析（Compute Workload Analysis）、内存负载分析（Memory Workload Analysis）方面呈现对比数据。算子对比功能仅支持同类型的算子进行对比。
...
片上内存（On-Chip Memory）界面支持展示内存分布情况，便于用户了解内存分布情况以及对应的内存分布详情，分析UB（Unified Buffer）内存出现溢出的问题。

### 界面介绍

导入指定目录下的`memory_info.json`文件，出现片上内存（On-Chip Memory）界面，展示内存分布情况，如[**图1**  片上内存-内存块图](#片上内存-内存块图)所示，点击内存块图中的色块，则会展现该内存块的内存详情以及内存分布，如[**图2**  片上内存-选中详情](#片上内存-选中详情)以及[**图3**  片上内存-内存分布](#片上内存-内存分布)所示。

 - [**图1**  片上内存-内存块图](#片上内存-内存块图)，该图反映了随时间变化内存的使用情况，整体运行过程中内存的峰值以及碎片情况。横轴代表虚拟的执行顺序，每个时间单位相当于 IR 文件中的一行，纵轴代表片上内存的地址。图中每一个色块代表一块 tensor 内存块。
 - [**图2**  片上内存-选中详情](#片上内存-选中详情)，其中包括内存块大小、生命周期、申请位置以及是否为临时变量。
 - [**图3**  片上内存-内存分布](#片上内存-内存分布)，反映内存在物理上的排布，便于用户进行内存调优，减少 bank 冲突。该图的布局理解，需要先了解昇腾芯片的内存布局，可以参考《Ascend C算子开发》文档中“[避免Unified Buffer的bank冲突](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/900/programug/Ascendcopdevg/atlas_ascendc_best_practices_10_00019.html)”章节的内容介绍。

### 使用说明

导入指定目录下的`memory_info.json`文件，界面出现内存块图，如[**图1**  片上内存-内存块图](#片上内存-内存块图)所示。基本信息将会在界面最上方展示，包括算子名称、内存类型、编译状态信息。可以通过切换内存类型查看对应的片上内存情况和编译状态。若编译失败，则会展示编译失败的错误信息，如[**图4**  片上内存-基本信息](#片上内存-基本信息)所示。

点击[**图1**  片上内存-内存块图](#片上内存-内存块图)中任意的内存块，则会展现该内存块的内存详情以及内存分布，如[**图2**  片上内存-选中详情](#片上内存-选中详情)以及[**图3**  片上内存-内存分布](#片上内存-内存分布)所示。

**图1**  片上内存-内存块图<a id="片上内存-内存块图"></a>
![](./figures/operator_tuning/memory_block_graph_1.png "片上内存-内存块图")

**图2**  片上内存-选中详情<a id="片上内存-选中详情"></a>
![](./figures/operator_tuning/slice_detail_1.png "片上内存-选中详情")

**图3**  片上内存-内存分布<a id="片上内存-内存分布"></a>
![](./figures/operator_tuning/memory_layout_1.png "片上内存-内存分布")

**图4**  片上内存-基本信息<a id="片上内存-基本信息"></a>
![](./figures/operator_tuning/basic_information_1.png "片上内存-基本信息")
