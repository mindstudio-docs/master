...
| 数据源 | 适用场景 | 优势 | 局限 |
| --- | --- | --- | --- |
| msMemScope | 需要分析全部内存事件（含框架外内存）、内存泄漏检测、内存拆解分析 | 数据完整，覆盖所有内存事件；支持内存块生命周期图、内存详情拆解图、调用栈火焰图 | 采集对运行性能有一定影响 |
| PyTorch Snapshot | 仅需分析PyTorch框架管理的内存池、内存碎片化分析、内存峰值优化 | 采集性能开销小；包含内存池状态数据 | 仅覆盖PyTorch框架管理的内存；大规模数据分析依赖可视化工具 |
...
安装msMemScope工具，安装前请确保已满足以下条件：

- 已安装CANN工具包（7.0及以上版本）并配置环境变量
- Python版本 >= 3.8
...
内存调优场景的数据由msMemScope工具采集生成，支持以下采集方式：
...
采集完成后，将生成的db格式文件导入MindStudio Insight工具，数据导入操作请参见[导入数据](./basic_operations.md#导入数据)。
...
|核心API参数|说明|
|--|--|
|torch_npu.npu.memory._record_memory_history() 函数参数|- `enabled`：控制记录的内容范围。`None`禁用；`state`仅记录当前分配；`all`记录所有内存分配/释放事件（默认）<br>- `context`：控制记录的堆栈信息范围。`None`不记录；`state`仅记录当前分配；`alloc`额外记录分配操作堆栈；`all`额外记录释放操作堆栈（默认）<br>- `stacks`：控制堆栈深度。`python`包含Python/TorchScript/inductor；`all`额外包含C++（默认）<br>- `max_entries`：限制记录的内存事件数量，默认9223372036854775807（无实际限制）<br>- `device`：可选参数，指定要记录内存历史的设备|
|torch_npu.npu.memory._dump_snapshot(file_path) 函数参数|- `file_path`：保存内存快照的文件路径，文件格式为 `pickle`|
...
内存详情（MemScope）界面包含调用栈火焰图（区域一）、内存块生命周期图（区域二）、内存详情拆解图（区域三）和内存详情表（区域四）。

- 区域一：调用栈火焰图，通过选择线程ID，展示对应的Python调用栈图；在“搜索”输入框中输入要搜索的函数名，或单击下拉框选择函数名，可选多个函数名进行搜索，调用栈图中会高亮显示搜索的函数，如[**图 1** 调用栈火焰图](#调用栈火焰图)所示。

  > [!NOTE] 说明
  > 该区域视图默认勾选“允许压缩”，该状态下，工具在不影响整体数据的情况下，压缩数据展示，增加工具使用流畅度。

  **图 1**  调用栈火焰图<a id="调用栈火焰图"></a>  
  ![](./figures/memory_tuning/call_stack_flame_1.png "调用栈火焰图")
...
- 区域三：内存详情拆解图，默认不显示，当鼠标置于“调用栈火焰图”或者“内存块生命周期图”中，会显示一条时间线，在“内存块生命周期图”区域，单击时间线，展示对应时间点的内存详情拆解图。当出现内存详情拆解图后，可以通过左上角按键隐藏/展示内存详情拆解图，如[**图 3** 内存详情拆解图](#内存详情拆解图)所示。

  **图 3**  内存详情拆解图<a id="内存详情拆解图"></a>  
  ![](./figures/memory_tuning/memory_breakdown_1.png "内存详情拆解图")

- 区域四：内存详情表，分为“内存块视图”和“内存事件视图”，可选择相应视图查看详情表，如[**图 4** 内存详情表](#内存详情表)所示，具体使用说明请参见[内存详情展示](#内存详情展示)。

  **图 4**  内存详情表<a id="内存详情表"></a>  
  ![](./figures/memory_tuning/memscope_memory_select_detail_1.png "内存详情表")

#### 使用说明

**调用栈火焰图和内存块生命周期图支持查看指定时间范围内数据**

MindStudio Insight 调用栈火焰图和内存块生命周期图通过控制下方趋势图缩放条选择区域展示指定范围内数据，如[**图 1**  下方趋势图缩放条](#趋势图缩放条)所示。

**图 1**  趋势图缩放条<a id="趋势图缩放条"> </a> 
![](./figures/memory_tuning/memscope_memory_block_timeline_graph_1.png "趋势图缩放条")

**趋势图缩放条支持趋势图可视化以及时间域选择多样化操作**

1. 趋势图可视化：

    MindStudio Insight 的趋势图缩放条所在背景呈现的是在整体时间范围下内存使用（Operator Allocated）情况的趋势图，直观地呈现选定时间范围内整个内存的使用情况趋势。
...
**内存块生命周期图和内存详情拆解图支持拖拽移动和缩放**

 - 内存块生命周期图和内存详情拆解图支持通过拖拽实现视图的移动，通过滚轮实现视图的缩放。
 - 内存块生命周期图支持通过视图右上角按键选择缩放模式，有横向缩放以及等比缩放两种模式，默认为等比缩放模式：

   - 横向缩放：只缩放横向时间轴，选择后可通过`Shift`键+`↑`键/`↓`键进行横向缩放。
   - 等比缩放：全视图等比例缩放，选择后可通过`Ctrl`键+`↑`键/`↓`键进行等比缩放。
...
内存快照详情（PyTorch Snapshot）界面包含内存块生命周期图（区域一）、内存池状态图（区域二）和内存详情表（区域三）。

- 区域一：如[**图 1**  内存快照内存块生命周期图](#内存快照内存块生命周期图)所示。内存块生命周期图，展示reserved折线图、内存申请/释放折线图和内存块图，选择内存块图上的色块，展示该内存块的详情，并统计缩放条选中区间内的**区间未释放总量**、**区间未释放最大块**以及**区间未释放最小块**，以上三个统计量含义如下：

  - 区间未释放总量：在缩放条区间内**申请，但未释放**的内存块大小总和。
  - 区间未释放最大块：在缩放条区间内**申请，但未释放**的**最大**内存块大小。
  - 区间未释放最小块：在缩放条区间内**申请，但未释放**的**最小**内存块大小。

  > [!NOTE] 说明
  >reserved折线图：PTA缓存池总大小（Operator Reserved）情况的趋势图，表示PTA Caching缓存池总大小的变化趋势，直观呈现实际PTA从驱动处申请及管理的缓存池大小，为任意时刻所有segment大小的总和。

  **图 1**  内存快照内存块生命周期图<a id="内存快照内存块生命周期图"></a>  
  ![](./figures/memory_tuning/memory_pytorch_snapshot_memory_block_timeline_graph_1.png "内存快照内存块生命周期图")

- 区域二：如[**图 2**  内存快照内存池状态图](#内存快照内存池状态图)所示。内存池状态图，当鼠标置于“内存块生命周期图”中，会显示一条时间线，在“内存块生命周期图”区域，单击时间线，会展示对应时间点的内存池状态图，具体使用说明请参见[内存池状态图展示](#内存池状态图展示)。

  **图 2**  内存快照内存池状态图<a id="内存快照内存池状态图"></a>  
  ![](./figures/memory_tuning/memory_pytorch_snapshot_memory_pool_state_diagram_1.png "内存快照内存池状态图")

- 区域三：如[**图 3**  内存快照详情数据](#内存快照详情数据)所示。内存快照详情表，分为“内存块视图”和“内存事件视图”，可选择相应视图查看详情表，具体使用说明请参见[内存快照详情展示](#内存快照详情展示)。

  **图 3**  内存快照详情数据<a id="内存快照详情数据"></a>  
  ![](./figures/memory_tuning/memory_pytorch_snapshot_detail_1.png "内存快照详情数据")

#### 使用说明

**内存块生命周期图支持查看指定时间范围内数据**

MindStudio Insight 内存块生命周期图通过控制下方内存快照趋势图缩放条选择区域展示指定范围内数据，如[**图 1**  下方内存快照趋势图缩放条](#内存快照趋势图缩放条)所示。
...
1. 趋势图可视化：
    ...
    MindStudio Insight 的内存快照趋势图缩放条所在背景呈现的是在整体时间范围下内存使用（Operator Allocated）情况的趋势图，直观地呈现选定时间范围内整个内存的使用情况趋势。
...
**内存块生命周期图和内存池状态图支持拖拽移动和滚轮缩放**
 ...
 - 内存块生命周期图和内存池状态图支持通过视图右上角按键选择缩放模式，有横向缩放以及等比缩放两种模式，默认为等比缩放模式：
...
当鼠标置于“内存块生命周期图”中，在“内存块生命周期图”区域，单击内存色块，则会在“内存块生命周期图”下方展示对应时间点的事件总览以及内存池状态图。也可以通过搜索对应的地址精准定位事件详情，支持按照具体事件类型筛选数据，如[**图 2** 内存池状态图](#内存池状态图)所示。
...
- 内存块视图：展示缩放条选中区间内存块的详细信息，如[**图 3**  快照内存块视图](#快照内存块视图)所示，字段解释如[**表 1**  快照内存块视图字段说明](#快照内存块视图字段说明)所示。鼠标悬浮在视图中，右上角会浮现![](./figures/system_tuning/zh-cn_image_0000002500040346.png)按钮，单击复制表中当前所展示的内容。
    ...
    **表 1**  快照内存块视图字段说明<a id="快照内存块视图字段说明"></a>
    ...
    |Free Event ID|可选|内存块释放完成事件ID。|整数|2|内存块释放完成事件ID，唯一标识内存块的释放事件。**值为-1代表该内存块的释放完成事件在内存快照采集生命周期中未被记录**。可点击内存块释放完成事件ID跳转至内存池状态图对应的事件并高亮。|
    
  内存块视图提供“**自动筛选区间未释放**”功能，该功能的筛选逻辑如下：

    - 未勾选该功能，展示所有申请释放区间与缩放条选中区间有交集的内存块详情，具体示意图如[图4 自动筛选区间未释放展示逻辑示意图](#自动筛选区间未释放展示逻辑示意图)中“开启前”所示，实际效果图如[图5 未勾选自动筛选区间未释放内存块视图](#未勾选自动筛选区间未释放内存块视图)。

    - 勾选该功能，展示所有在缩放条选中区间内存块申请但未释放的内存块详情；用于识别潜在的泄露 tensor 占用内存块，具体示意图如[图4 自动筛选区间未释放展示逻辑示意图](#自动筛选区间未释放展示逻辑示意图)中“开启后”所示，实际效果图如[图6 勾选自动筛选区间未释放内存块视图](#勾选自动筛选区间未释放内存块视图)。

      **图4**  自动筛选区间未释放展示逻辑示意图<a id="自动筛选区间未释放展示逻辑示意图"></a>
      ![](./figures/memory_tuning/potential_leak_tensor_filter_rule.png "自动筛选区间未释放展示逻辑示意图")

      **图5**  未勾选自动筛选区间未释放内存块视图<a id="未勾选自动筛选区间未释放内存块视图"></a>
      ![](./figures/memory_tuning/memory_pytorch_snapshot_block_view_unselected_2.png "未勾选筛选内存块视图")

      **图6**  勾选自动筛选区间未释放内存块视图<a id="勾选自动筛选区间未释放内存块视图"></a>
      ![](./figures/memory_tuning/memory_pytorch_snapshot_block_view_selected_2.png "勾选自动筛选区间未释放内存块视图")

- 内存事件视图：展示缩放条选中区域内存事件的详细信息，如[**图 4**  快照内存事件视图](#快照内存事件视图)所示，字段解释如[**表 2**  快照内存事件视图字段说明](#快照内存事件视图字段说明)所示。鼠标悬浮在视图中，右上角会浮现![](./figures/system_tuning/zh-cn_image_0000002500040346.png)按钮，单击复制表中当前所展示的内容。
...
- 选中详情：支持查看多个详情信息，可通过点击内存块生命周期图中的色块，内存池状态图左侧事件列表的事件以及内存池状态图中的色块查看详情信息。详情信息主要分为以下三类，包括：
  - 事件类型选中详情，如[**图 5**  事件类型选中详情](#事件类型选中详情)所示，字段解释如[**表 3**  事件类型选中详情字段说明](#事件类型选中详情字段说明)所示，基础事件信息在左侧区域呈现，具体事件信息可切换事件类型查看。

    当在“内存块生命周期图”中单击任意时间点的视图，快照选中详情的展示信息也会随之更新。

    **图 5**  事件类型选中详情<a id="事件类型选中详情"></a>  
    ![](./figures/memory_tuning/memory_pytorch_snapshot_select_detail_1.png "事件类型选中详情")

    **表 3**  事件类型选中详情字段说明<a id="事件类型选中详情字段说明"></a> 
...
3. 使用内存工具（msMemScope）采集对应异常范围的内存详情与内存拆解数据，导入MindStudio Insight；
4. 查看内存详情（MemScope）界面，结合“调用栈火焰图”、“内存块生命周期图”、“内存详情表”进行内存占用拆解分析。
