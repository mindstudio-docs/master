...
**集群数据采集与预处理流程**<a id="集群数据采集与预处理流程"></a>
...
<tbody>
  ...
  <tr>
    ...
    <td class="tg-0pky">Thread</td>
    <td class="tg-0pky">在MindSpore场景下，展示当前Thread下运行的阶段耗时，用于分析网络结构性能。</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-0pky">Python GC</td>
    <td class="tg-0pky">在PyTorch场景下，若开启GC检测功能且采集周期内发生GC事件，则会在Python GC泳道中显示。</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-0pky">HBM内存读写速率，单位为MB/s。</td>
  </tr>
  <tr>
    <td class="tg-0pky">DDR</td>
    <td class="tg-0pky">DDR内存读写速率</td>
  </tr>
  <tr>
    <td class="tg-0pky">LLC</td>
    <td class="tg-0pky">具体说明见<a href="#NPU Metrics泳道信息">表 2 NPU Metrics泳道信息</a>中“LLC”泳道说明</td>
  </tr>
  <tr>
    <td class="tg-0pky">NPU_MEM</td>
    <td class="tg-0pky">具体说明见<a href="#NPU Metrics泳道信息">表 2 NPU Metrics泳道信息</a>中“NPU_MEM”泳道说明</td>
  </tr>
  <tr>
    <td class="tg-0pky">Stars Soc Info</td>
    <td class="tg-0pky">具体说明见<a href="#NPU Metrics泳道信息">表 2 NPU Metrics泳道信息</a>中“Stars Soc Info”泳道说明</td>
  </tr>
  <tr>
    <td class="tg-0pky">acc_pmu</td>
    <td class="tg-0pky">具体说明见<a href="#NPU Metrics泳道信息">表 2 NPU Metrics泳道信息</a>中“acc_pmu”泳道说明</td>
  </tr>
  <tr>
    <td class="tg-0pky">AI Core Utilization</td>
    <td class="tg-0pky">具体说明见<a href="#NPU Metrics泳道信息">表 2 NPU Metrics泳道信息</a>中“AI Core Utilization”泳道说明</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-0pky">具体说明见<a href="#NPU Metrics泳道信息">表 2 NPU Metrics泳道信息</a>中“SIO”泳道说明</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-0pky">具体说明见<a href="#NPU Metrics泳道信息">表 2 NPU Metrics泳道信息</a>中“PCIe”泳道说明</td>
  </tr>
  <tr>
  ...
  <tr>
    <td class="tg-0pky">biu_group</td>
    <td class="tg-0pky">具体说明见<a href="#NPU Metrics泳道信息">表 2 NPU Metrics泳道信息</a>中“biu_group”泳道说明</td>
  </tr>
  <tr>
    <td class="tg-0pky">aic_core_group</td>
    <td class="tg-0pky">具体说明见<a href="#NPU Metrics泳道信息">表 2 NPU Metrics泳道信息</a>中“aic_core_group”泳道说明</td>
  </tr>
  <tr>
    <td class="tg-0pky">aiv_core_group</td>
    <td class="tg-0pky">具体说明见<a href="#NPU Metrics泳道信息">表 2 NPU Metrics泳道信息</a>中“aiv_core_group”泳道说明</td>
  </tr>
  <tr>
    <td class="tg-0pky">Stars Chip Trans</td>
...
**表 2** NPU Metrics泳道信息<div id="NPU Metrics泳道信息"></div>
...
<tr>
  ...
  <tr>
    ...
    <td class="tg-0pky">Meta带宽等级信息。</td>
  </tr>
  <tr>
...
**表 3** CPU Metrics泳道信息<a id="CPU Metrics泳道信息"></a>
...
时间线（Timeline）界面支持通过鼠标功能栏调整视图，界面如[图1 悬浮工具栏](#悬浮工具栏)所示，主要功能如下：

1. 默认模式：鼠标可灵活点击，自由框选泳道。
2. 拖拽模式：自由横向拖拽时序图，灵活查看定位算子。
3. 泳道高度自适应：会根据窗口变化，自适应压缩空白无效区域，使视图展示更清晰。
4. 悬浮工具指引：对悬浮工具栏功能进行图文介绍。
5. 悬浮工具栏可以通过按键隐藏和展示。
...
- 单击界面左上方工具栏中的![](./figures/system_tuning/zh-cn_image_0000002500040294.png)，在弹出输入框中输入需要搜索的内容，然后按回车键，则会匹配对应的算子或API，搜索结果匹配算子和API总数，在界面中也会高亮显示匹配的算子或API，如[**图 1**  搜索算子](#搜索算子)所示。

    单击搜索框前方的下拉菜单可选择查看不同的卡，也可选择“ALL”查看所有卡，系统默认选择第一张卡，如[**图2** 选卡功能](#选卡功能)。

    单击搜索框后方的切换按钮，可以查看上一个或者下一个匹配的算子或API，也可以在输入框后方输入具体的数字搜索其对应的算子或API，该算子或API将会被选中。

    **图 1**  搜索算子<a id="搜索算子"></a>
    ![](./figures/system_tuning/search_operators_2.jpeg "搜索算子")
...
- 单击界面左上方工具栏中的![](./figures/system_tuning/zh-cn_image_0000002500040294.png)，可在搜索弹出输入框左侧分别单击![](./figures/system_tuning/zh-cn_image_0000002531920185.png)和![](./figures/system_tuning/zh-cn_image_0000002500040296.png)，开启大小写匹配和全词匹配功能。
    ...
    - 单击搜索框前方的下拉菜单可选择查看不同的卡，也可选择“ALL”查看所有卡，系统默认选择第一张卡，如[**图2** 选卡功能](#选卡功能)。
      ...
      **图 2**  选卡功能<a id="选卡功能"></a>
      ![](./figures/system_tuning/search_operators_select_2.jpeg "选卡功能")

    - 单击![](./figures/system_tuning/zh-cn_image_0000002531920185.png)开启大小写匹配，在弹出输入框中输入需要搜索的内容，然后按回车键，则会匹配名称中包含搜索项的算子或API，如[**图 3**  开启大小写匹配或全词匹配](#开启大小写匹配或全词匹配)所示。

      **图 3**  开启大小写匹配或全词匹配<a id="开启大小写匹配或全词匹配"></a>
      ![](./figures/system_tuning/enable_case_or_whole_word_matching_1.jpeg "开启大小写匹配或全词匹配")

    - 单击![](./figures/system_tuning/zh-cn_image_0000002500040296.png)开启全词匹配，在弹出输入框中输入需要搜索的内容，然后按回车键，则会匹配名称为搜索项的算子或API，但是会忽略大小写，如[**图 3**  开启大小写匹配或全词匹配](#开启大小写匹配或全词匹配)所示。
    ...
    - 单击搜索框后方的“在查找窗口打开”，可跳转至页面下方的“查找”页签，展示搜索项相关信息，如[**图 4**  在查找窗口打开](#在查找窗口打开)所示，字段解释如[**表 2**  查找页签字段说明](#查找页签字段说明)所示。单击“点击跳转Timeline”列的“点击”可跳转到算子或API在时间线视图上的具体位置。

      **图 4**  在查找窗口打开<a id="在查找窗口打开"></a>
      ![](./figures/system_tuning/open_in_search_window_1.jpeg "在查找窗口打开")
      ...
      **表 2**  查找页签字段说明<a id="查找页签字段说明"></a>
      ...
      |名称|Name|算子名称。单击名称后面的筛选图标可通过名称筛选数据。|
...
- MindStudio Insight支持一键置顶同一通信域的子级泳道。

  选择需要置顶的同名泳道，右键单击同名的任意泳道，选择“置顶同名泳道（泳道名称）”，则相同泳道名称的泳道会一并置顶，便于同名泳道的查看对比分析。以下为该功能的操作示例：

    在父泳道下的子泳道上单击鼠标右键，选择“置顶同名泳道（泳道名称）”，将置顶该通信域（组）下的所有同名泳道，便于查看对比，如[**图 7**  置顶同名泳道](#置顶同名泳道)所示，如[**图 8**  置顶同名泳道效果](#置顶同名泳道效果)所示。

    **图 7**  置顶同名泳道<a id="置顶同名泳道"></a>
    ![](./figures/system_tuning/pin_samename_lane_to_top_1.png "置顶通信泳道")

     **图 8**  置顶同名泳道效果<a id="置顶同名泳道效果"></a>
    ![](./figures/system_tuning/pin_samename_lane_to_top_2.png "置顶通信泳道效果")

    在已置顶的泳道上单击鼠标右键，可选择“取消置顶同名泳道（泳道名称）”或者“取消置顶（全部）”，取消泳道置顶，如[**图 9**  取消置顶同名泳道](#取消置顶同名泳道)所示。“取消置顶同名泳道（泳道名称）”即取消该通信域（组）下的所有泳道，“取消置顶（全部）”即取消所有置顶泳道。

    **图 9**  取消置顶同名泳道<a id="取消置顶同名泳道"></a>
    ![](./figures/system_tuning/cancel_pin_to_top_1.png "取消置顶同名泳道")
...
> - 若泳道设置了偏移量，且偏移量不为0，则会在泳道名称后方显示高亮蓝点。
...
- 手动设置对齐到起始位置
    ...
    如[**图 10**  初始位置偏移量](#初始位置偏移量)所示，0卡中最左侧线程数据与时间轴初始位置的偏移量为7293500ns。
...
- 手动设置偏移量
    ...
    对于多机多卡场景，由于机器上时间不准，可能造成多卡间时间线（Timeline）相对位置不准确，MindStudio Insight支持单卡维度的时间校准，如[**图 11**  单卡时间调整](#单卡时间调整)所示，通过设置偏移量，可以将单卡的时间线（Timeline）左右移动，从而达到时间“校准”的目的。偏移量的单位为ns，负值为右移，正值为左移。

    **图 11**  单卡时间调整<a id="单卡时间调整"></a>
    ![](./figures/system_tuning/single_card_time_adjustment_1.png "单卡时间调整")

    同时，为了更灵活的校准时间，MindStudio Insight还支持以泳道维度进行时间校准，如[**图 12**  泳道时间调整](#泳道时间调整)所示。在时间线（Timeline）界面，展开卡，单击所需二级泳道名称后面的“偏移量”，在输入框输入值，单击回车键，进行泳道时间调整。db场景下，需要首先展开机器名称，分别在host和各卡下的二级泳道上执行时间调整操作。

    **图 12**  泳道时间调整<a id="泳道时间调整"></a>
    ![](./figures/system_tuning/lane_time_adjustment_1.png "泳道时间调整")

**支持时间对齐**

为方便数据查看对比更直观，支持多泳道算子的时间对齐，选择想要对齐的泳道算子，单击右键，在功能菜单中选择“**时间对齐**”功能右侧的“**左对齐**”或者“**右对齐**”即可实现。如[**图1 时间对齐**](#时间对齐)所示。

**图 1**  时间对齐<a id="时间对齐"></a>
![](./figures/system_tuning/timeline_align_1.png "时间对齐")

- 选择“左对齐”后，效果如[**图2 时间左对齐**](#时间左对齐)所示。

  **图 2**  时间左对齐<a id="时间左对齐"></a>
  ![](./figures/system_tuning/timeline_left_align_1.png "时间左对齐")

- 选择“右对齐”后，效果如[**图3 时间右对齐**](#时间右对齐)所示。

  **图 3**  时间右对齐<a id="时间右对齐"></a>
  ![](./figures/system_tuning/timeline_right_align_1.png "时间右对齐")

**支持多机多卡展示**

当导入多机多卡数据时，MindStudio Insight支持以机器维度展示数据，如[**图 4**  多机多卡展示](#多机多卡展示)所示。
...
- 区域标记
    ...
    在时间线（Timeline）界面选中某个区域后，单击![](./figures/system_tuning/2023-08-10_175758.png)或敲击键盘K键将选中区域进行标记并保存，如[**图 5**  区域标记](#区域标记)所示。

    **图 5**  区域标记<a id="区域标记"></a>
    ![](./figures/system_tuning/region_marker_1.png "区域标记")

    左键双击任一标记，可以设置该标记对的属性，支持修改标记对名称、颜色以及删除该标记对，如[**图 6**  修改标记对属性](#修改标记对属性)所示。

    **图 6**  修改标记对属性<a id="修改标记对属性"></a>
    ![](./figures/system_tuning/modify_marker_pair_attributes_1.png "修改标记对属性")

- 单点标记

    在最上方空泳道的任意位置，单击鼠标左键或敲击键盘K键，将生成一个单点标记，如[**图 7**  单点标记](#单点标记)所示。

    **图 7**  单点标记<a id="单点标记"></a>
...
- 标记管理
    ...
    单击左上方工具栏中的![](./figures/system_tuning/zh-cn_image_0000002500040314.png)，将显示所有标记信息，如[**图 8**  查看标记信息](#查看标记信息)所示。

    **图 8**  查看标记信息<a id="查看标记信息"></a>
...
- MindStudio Insight支持算子连线关系展示，单击有连线的算子，即可显示该算子关联的连线，即使折叠连线起点或者终点的进程，连线也不会消失。并且支持定位到该算子关联连线的其它算子，并显示其详情。
    1. 单击有连线的算子，显示连线关系。
    2. 单击鼠标右键，选择“跳转到连线算子”，在算子列表中单击“来源”或“目标”下的具体算子名称或连线方式，即可跳转至对应算子，并自动高亮显示该算子，同时在“选中详情”页签中显示该算子的详细信息。如[**图 9**  算子连线关系](#算子连线关系)所示。

        **图 9**  算子连线关系<a id="算子连线关系"></a>
...
- MindStudio Insight支持全量连线的功能，单击界面左上方工具栏中的![](./figures/system_tuning/zh-cn_image_0000002500200298.png)，在弹框中选择一个或多个连线类型，也可在搜索框中搜索某一连线类型的关键字，勾选相应的连线类型，则在图形化窗格展示对应类型的所有连线，如[**图 10**  全量连线](#全量连线)所示。

    > [!NOTE]
    >
    > 最多支持选择10个连线类型。

    **图 10**  全量连线<a id="全量连线"></a>
...
- 一键全部解析：在时间线（Timeline）界面，单击“开始全局解析”，将开始解析所有卡的数据，如[**图 11**  全局解析](#全局解析)所示。当所有卡的数据解析完成后，“开始全局解析”按钮消失。

    **图 11**  全局解析<a id="全局解析"></a>
    ![](./figures/system_tuning/global_parsing_1.png "全局解析")

- 部分解析：当只需要解析部分卡的数据时，可逐个单击对应卡序号后面的![](./figures/system_tuning/zh-cn_image_0000002500040320.png)，解析所选卡的数据，如[**图 12**  单卡解析](#单卡解析)所示。当对应卡数据解析完成后，按钮消失，如图中0卡和1卡所示。

    **图 12**  单卡解析<a id="单卡解析"></a>
    ![](./figures/system_tuning/single_card_parsing_1.png "单卡解析")

    如果导入的卡数量较多，可通过卡过滤功能定位所需解析数据的卡，进行数据解析操作。在时间线（Timeline）界面的工具栏中，单击![](./figures/system_tuning/zh-cn_image_0000002532040213.png)，选择“卡过滤”，然后单击后方输入框，在下拉框选择所需呈现的卡，即可在时间线（Timeline）界面展示对应信息，单击卡序号后面的![](./figures/system_tuning/zh-cn_image_0000002500040320.png)，进行数据解析，如[**图 13**  过滤展示并解析](#过滤展示并解析)所示，解析2、5、7卡数据。

    **图 13**  过滤展示并解析<a id="过滤展示并解析"></a>
...
- 相同通信域的卡解析：当解析完一个卡后，在通信的Group子泳道上，单击鼠标右键，选择“解析相关通信域的卡”，和该泳道通信域相关的卡都被解析，如[**图 14**  解析相关通信域的卡](#解析相关通信域的卡)，解析完成后，该鼠标右键菜单变为“已解析全部相关通信域的卡”并置灰。

    **图 14**  解析相关通信域的卡<a id="解析相关通信域的卡"></a>
...
- **算子时间对齐**
    1. 在时间线（Timeline）界面，选中任意一个算子，单击鼠标右键，选择“设置基准算子”，将选中算子设置为基准算子，如[**图 15** 设置基准算子](#设置基准算子)所示。

        **图 15** 设置基准算子<a id="设置基准算子"></a>
        ![](./figures/system_tuning/set_baseline_operator_1.png "设置基准算子")

    2. 选中与基准算子不同的二级泳道中的算子。
    3. 使用键盘快捷键L（左对齐），将选中的算子与基准算子左边界对齐，如[**图 16** 算子左边界对齐](#算子左边界对齐)所示。

        **图 16** 算子左边界对齐<a id="算子左边界对齐"></a>
        ![](./figures/system_tuning/align_operator_left_boundary_1.png "算子左边界对齐")

        使用键盘快捷键R（右对齐），则选中的算子与基准算子会右边界对齐，如[**图 17** 算子右边界对齐](#算子右边界对齐)所示。

        **图 17** 算子右边界对齐<a id="算子右边界对齐"></a>
...
- **取消基准算子**
    ...
    在泳道任意位置，单击鼠标右键，选择“消除基准算子”，则取消基准算子，如[**图 18** 取消基准算子](#取消基准算子)所示。

    **图 18** 取消基准算子<a id="取消基准算子"></a>
    ![](./figures/system_tuning/cancel_baseline_operator_1.png "取消基准算子")

- **恢复默认偏移量**

    如果已经执行了算子时间对齐操作，可在泳道任意位置，单击鼠标右键，选择“恢复所有卡的默认偏移量”，恢复默认的偏移量，如[**图 19** 恢复所有卡的默认偏移量](#恢复所有卡的默认偏移量)所示。

    **图 19** 恢复所有卡的默认偏移量<a id="恢复所有卡的默认偏移量"></a>
...
为方便详细定位问题代码位置，Python调用栈独立泳道呈现，如[**图3 Python调用栈](#Python调用栈)所示。

**图 3** Python调用栈<a id="Python调用栈"></a><br>
![](./figures/system_tuning/python_call_stack_1.png "Python调用栈")

**支持展开全部泳道**

在时间线（Timeline）界面，可使用鼠标右键菜单展开全部泳道或收起全部泳道。

- 展开全部泳道：在需要展开的泳道上，单击鼠标右键，在弹出菜单中选择“展开全部子项”，将展开所选泳道下的全部子泳道，如[**图 4** 展开全部子项](#展开全部子项)所示，展开0卡的全部泳道。

    当所选泳道及子泳道已处于全部展开状态时，鼠标右键不显示“展开全部子项”选项。

    **图 4** 展开全部子项<a id="展开全部子项"></a><br>
    ![](./figures/system_tuning/expand_all_children_1.png "展开全部子项")

- 收起全部泳道：在已展开的泳道上，单击鼠标右键，在弹出菜单中选择“收起全部子项”，将收起所选泳道下的全部子泳道，如[**图 5** 收起全部子项](#收起全部子项)所示，收起0卡的所有泳道。

    **图 5** 收起全部子项<a id="收起全部子项"></a><br>
...
- 开启泳道高度自适应：在展开的泳道上，单击鼠标右键，在弹出菜单中选择“开启泳道高度自适应”，可自动调整泳道高度，以适配当前页面显示，如[**图 6** 开启泳道高度自适应](#开启泳道高度自适应)所示。
    ...
    **图 6** 开启泳道高度自适应<a id="开启泳道高度自适应"></a><br>
...
- 关闭泳道高度自适应：在已开启泳道高度自适应功能的泳道上，单击鼠标右键，在弹出菜单中选择“关闭泳道高度自适应”，关闭泳道高度自适应功能，泳道恢复初始高度，如[**图 7** 关闭泳道高度自适应](#关闭泳道高度自适应)所示。
    ...
    **图 7** 关闭泳道高度自适应<a id="关闭泳道高度自适应"></a><br>
...
- 锁定框选区域
    ...
    在时间线（Timeline）界面，使用鼠标左键在泳道上框选部分算子区域后，单击鼠标右键，选择“锁定框选区域”，可在框选区域搜索相关算子，并展示连线起始点或终止点任意一个在框选区域的算子连线，如[**图 8** 锁定框选区域](#锁定框选区域)所示。也支持在单卡层级下跨多个泳道框选区域进行锁定。

    **图 8** 锁定框选区域<a id="锁定框选区域"></a>
    ![](./figures/system_tuning/lock_selected_region_1.png "锁定框选区域")

- 解锁框选区域

    如果需要取消框选，可单击鼠标右键，选择“解锁框选区域”，便可取消框选锁定，如[**图 9** 解锁框选区域](#解锁框选区域)所示。

    **图 9** 解锁框选区域<a id="解锁框选区域"></a>
...
>[!NOTE]
>
> - 当导入的数据为非Ftrace类型数据时，统计系统视图将展示：综合指标、内存拷贝总览、算子下发耗时统计、算子详情以及算子类型（包括Python API 汇总、CANN API 汇总、Ascend Hardware Task 汇总、通信汇总、覆盖分析）数据。
> - 当导入的数据为Ftrace类型数据时，统计系统视图仅展示：Ftrace耗时统计、Ftrace中断统计和Ftrace上下文切换统计。
...
- 算子下发耗时统计
  ...
  **表 11** 算子下发耗时统计字段说明<a id="算子下发耗时统计字段说明"></a>

  |中文字段|英文字段|说明|
  |--|--|--|
  |算子|Operator|算子名称。|
  |路径|Path|算子下发路径类型：ACLOP 或 ACLNN。|
  |准备时间|Prepare Time|PythonCall 父项：所有叶子项中最早的入队开始时间 - Python API 开始时间；CANN API 父项/完整链路叶子项：入队开始时间 - Python API 开始时间；Launch 叶子项：空。|
  |Python API时间|Python API Time|PythonCall 父项/CANN API 父项/完整链路叶子项：Python API 结束时间 - Python API 开始时间；Launch 叶子项：空。|
  |入队耗时|Enqueue Time|PythonCall 父项：汇总所有叶子项入队耗时；CANN API 父项/完整链路叶子项：入队结束时间 - 入队开始时间；Launch 叶子项：空。|
  |排队耗时|Queue Time|PythonCall 父项：汇总所有叶子项排队耗时；CANN API 父项/完整链路叶子项：出队开始时间 - 入队结束时间；Launch 叶子项：空。|
  |二级流水耗时|Pipeline2 Time|PythonCall 父项：汇总所有叶子项二级流水耗时；CANN API 父项：汇总子叶子项二级流水耗时；完整链路叶子项/Launch 叶子项：Launch 结束时间 - 出队开始时间（Launch 结束时间不存在时，用出队结束时间兜底。）。|
  |Launch 耗时|Launch Time|PythonCall 父项：汇总所有叶子项 Launch 耗时；CANN API 父项：汇总子叶子项 Launch 耗时；完整链路叶子项/Launch 叶子项：Launch 结束时间 - Launch 开始时间（无 Launch 则为空）。|
  |端到端耗时|E2E Time|PythonCall 父项：所有叶子项端到端耗时最大值；CANN API 父项：子叶子项端到端耗时最大值；完整链路叶子项/Launch 叶子项：Launch 结束时间 - Python API 开始时间（Launch 结束时间不存在时，用出队结束时间计算。）。|
...
6种专家建议系统包括亲和 API（Affinity API）、亲和优化器（Affinity Optimizer）、AICPU 算子（AICPU Operators）、ACLNN 算子（ACLNN Operators）、算子融合（Operators Fusion）和算子下发（Operators Dispatch），如[**图 13** 专家系统视图](#专家系统视图)所示，字段解释如[**表 12** 专家系统视图字段说明](#专家系统视图字段说明)所示。

选择任一专家建议系统，右侧区域会显示该类专家建议系统的详细信息，单击“点击跳转Timeline”列中的“点击”，会跳转到算子在时间线视图中的具体位置，区域四（数据窗格）“选中详情”页签将会展示该算子的具体信息。

**图 13** 专家系统视图<a id="专家系统视图"></a>
![](./figures/system_tuning/expert_system_view_1.png "专家系统视图")

**表 12** 专家系统视图字段说明<a id="专家系统视图字段说明"></a>
...
在时间线（Timeline）界面，选择所需泳道，单击鼠标右键，单击“在事件视图中显示”菜单，跳转至“系统视图”页签，左侧区域选项默认选择“事件视图”，右侧区域显示该泳道所有算子详情，如[**图 14** 事件视图](#事件视图)所示，字段解释如[**表 13** 事件视图字段说明](#事件视图字段说明)所示。

**图 14** 事件视图<a id="事件视图"></a>
![](./figures/system_tuning/event_view_1.png "事件视图")

**表 13** 事件视图字段说明<a id="事件视图字段说明"></a>
...
> [!NOTE] 说明
> 如果采集配置中设置了`aic_metrics=torch_npu.profiler.AiCMetrics.PipeUtilization`且profiler_level为Level1及以上，计算算子详情中会展示AI Core和AI Vector Core性能指标字段，详细字段说明请参见[AI Core/AI Vector Core性能指标字段说明](#ai-core性能指标字段说明)。
...
## 通信（Communication）<a id="communication"></a>
...
**通信链路类型说明**<a id="通信链路类型说明"></a>

> 以下内容来源于《性能调优工具指南》通信链路类型章节，如有更新请以源文档为准。

在通信分析中，数据传输通过不同的链路类型完成，各链路类型的说明如["表 19"  通信链路类型说明](#通信链路类型说明)所示。

**"表 19"  通信链路类型说明**<a id="通信链路类型说明-table"></a>

|链路类型|全称|说明|适用场景|经验带宽参考值(GB/s)|
|--|--|--|--|--|
|SDMA|Shared Direct Memory Access|节点内Device间通信链路，即同一服务器内不同NPU之间的数据传输。|单机多卡场景，同一服务器内的NPU间通信。|-|
|HCCS|Huawei Cache Coherence System|SDMA的子类型，基于华为缓存一致性系统的节点内通信链路。|同一服务器内通过HCCS互联的NPU间通信。|18|
|PCIe|Peripheral Component Interconnect Express|SDMA的子类型，基于PCIe总线的节点内通信链路。|同一服务器内通过PCIe连接的NPU间通信。|20|
|SIO|Serial Input/Output|SDMA的子类型，基于串行输入输出的节点内通信链路。|特定硬件拓扑下的节点内通信。|-|
|RDMA|Remote Direct Memory Access|跨节点Device间通信链路，即不同服务器间NPU的直接内存访问。|多机多卡场景，跨服务器的NPU间通信。|12.5|

> [!NOTE] 说明
>
> - SDMA包含HCCS、PCIe和SIO三种子类型，MindStudio Insight工具会根据实际硬件拓扑自动识别使用的子类型。
> - 经验带宽参考值为理论峰值带宽的典型值，实际带宽受硬件配置、网络拓扑和通信模式等因素影响，仅供参考。
> - 当实际带宽远低于经验带宽参考值时，可能存在链路瓶颈，建议检查网络配置和通信模式。
> - 带宽计算方式：带宽 = 通信数据量 / 通信耗时。在带宽分析页面中，可通过对比实际带宽与经验带宽参考值来识别通信瓶颈。
...
<a id="host-bound-analysis-case"></a>
