...
> 工具的南向主要涉及runtime、driver、PyTorch、mindspore、atb等组件，对其内存行为进行trace，对获取的数据进行合法化处理后向客户暴露。
...
输出文件是会至少按照文件夹750、文件640、只读文件400权限落盘。如果落盘失败，工具会输出日志提示当前异常。
...
1. 测试工程设计
   UT目录test和源码csrc目录平级，内部子目录和功能代码对称。
...
3. 运行环境
   ...
   ST需要运行在昇腾服务器上，根据脚本的不同，最多需要8张 NPU卡，对芯片型号没有要求。
4. 测试替身设计
   白盒测试中需要对dlopen/dlsym等函数进行mock，否则在没有昇腾环境的通用linux环境下，会返回失败导致无法覆盖大部分用例场景。
