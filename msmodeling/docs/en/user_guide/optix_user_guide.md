...
**Measured serving optimization** (`msmodeling optix`) is a feature for measured optimization of serving parameters. It provides two optimization modes: automatic optimization based on the Particle Swarm Optimization (PSO) algorithm and closed-loop tuning based on an AI agent. It supports automatic search on real serving frameworks such as vLLM and MindIE to obtain the best throughput parameter combination that meets latency requirements. For the differences between the two modes, see [Optimization Modes: PSO and Agent](#9-optimization-modes-pso-and-agent).

## 2. Intended Audience and Reading Path
...
- **Parameter optimization module**: automatically generates serving parameter combinations and continuously approaches the optimal solution. In PSO mode, the combinations are generated by the built-in PSO particle swarm optimization algorithm; in Agent mode, they are generated round by round by an AI agent. In addition, the Early Rejection algorithm performs early evaluation of serving parameters based on theoretical modeling, tuning experience, and partial measured data.
...
## 3. Supported Products<a name="ZH-CN_TOPIC_0000002479925980"></a>
...
For the complete steps, see [Recommended Practice: Environment and Deployment Stack](#6-recommended-practice-environment-and-deployment-stack). For common installation on the simulation side, see [Environment Setup Guide](../install_guide/msmodeling_install_guide.md#61-optix-and-simulation-environment-separation).
...
> [!WARNING]
> Do not install deployment packages such as `vllm` and `mindie_llm` in the msModeling virtual environment. Do not install msModeling in the system Python without creating a venv. For details, see [Recommended Practice: Environment and Deployment Stack](#6-recommended-practice-environment-and-deployment-stack).

## 6. Recommended Practice: Environment and Deployment Stack
...
1. Complete the requirements described in [Preparations](#4-preparations) and [Recommended Practice: Environment and Deployment Stack](#6-recommended-practice-environment-and-deployment-stack).
...
3. Start optimization. After completing the preceding steps, run the following command to start automatic optimization with one click:
    ...
    By default, the tool optimizes vLLM serving parameters with `AISBench` as the benchmark tool in PSO mode. To switch to Agent mode, see [Optimization Modes: PSO and Agent](#9-optimization-modes-pso-and-agent).

4. View the results. The optimization time is determined by the model size and dataset size, typically 4 to 8 hours. After completion, the `data_storage_*.csv` file is generated and saved in the `result/store` subdirectory of the current directory. It records the performance of each parameter set. For details, see [Output File Description](#11-output-file-description).

## 9. Optimization Modes: PSO and Agent

Measured serving optimization provides two optimization modes, selected by the top-level `optimizer_strategy` field in the configuration file. The default value is `pso`:

|Mode|Value|How it works|Characteristics|Applicable scenario|
|---|---|---|---|---|
|PSO mode (default)|`optimizer_strategy = "pso"`|The built-in PSO particle swarm optimization algorithm automatically generates parameter combinations and iterates towards the optimum.|Fully automatic operation without manual intervention. The search scale is directly controlled by `n_particles` and `iters`.|Routine automatic optimization and unattended long-time optimization.|
|Agent mode|`optimizer_strategy = "agent"`|An AI agent generates candidate parameters round by round. The tool validates candidates, executes the measurements, and writes back results. The agent decides the next-round direction based on the results of each round.|Each candidate carries an explainable rationale. Historical experience and failure information can be absorbed. Manual confirmation or modification of candidates is supported.|Scenarios that require experience-driven direction decisions or human review of candidates.|

Both modes share the optimization parameters, benchmark tool parameters, and serving parameters described in [Configuration File Description](#configuration-file-description). The differences are mainly in how the search is controlled:

- PSO mode controls the search scale with `n_particles` and `iters`. Agent mode uses the `[agent_optimizer]` configuration section to control the number of rounds, the number of trials per round, and the time budget.
- Agent mode requires `use_request_rate_calibration = false`. In this case, `CONCURRENCY` participates in the search and `REQUESTRATE` is fixed at the maximum-pressure rate, so that the relationship between concurrency and latency can be observed trial by trial. With the default `true`, the `CONCURRENCY` of candidates is pinned to `max` and the concurrency search dimension becomes ineffective.

### How Agent Mode Works

In Agent mode, the tool does not run PSO internally. Instead, it reads the candidate file `candidates.round-N.json` generated by the AI agent from the run directory resolved by `[agent_optimizer].run_dir`, executes the validated candidates one by one, and writes back `results.round-N.json` and `progress.round-N.json`. A round exits after completion only when the candidate file of the next round has not been generated yet. Convergence exit requires both the trend condition (improvement below 1% for `convergence_rounds` consecutive rounds) and the coverage condition (at least `min_rounds` rounds and at least `min_trials` trials completed), subject to the `max_rounds` limit, with the trial execution of each round limited by `time_limit_minutes`.

Fields of the `[agent_optimizer]` configuration section (all optional):

|Parameter|Default|Description|
|---|---|---|
|`run_id`|`"default"`|Run identifier. Candidates and results are organized under this identifier in the run directory.|
|`run_dir`|Empty|Run directory. If left empty, it is automatically resolved to `.agent_optimizer/runs/{run_id}`.|
|`max_rounds`|`32`|Maximum number of optimization rounds. The run terminates naturally when the limit is reached.|
|`candidates_per_round`|`0`|Number of candidates per round. `0` means the agent decides the count dynamically based on the previous round. A value greater than 0 enforces a fixed count.|
|`max_trials`|`12`|Maximum number of trials executed per round. The excess candidates are not executed.|
|`time_limit_minutes`|`1440`|Time limit for executing trials within a single round, in minutes. The default value is 1440. The global budget is accumulated across rounds by the agent.|
|`convergence_rounds`|`6`|Number of consecutive rounds without significant improvement required to trigger convergence detection.|
|`min_rounds`|`4`|Minimum number of rounds that must be completed before convergence.|
|`min_trials`|`12`|Minimum number of trials that must be completed before convergence.|

It is recommended to use Agent mode through the **`optix-assistant` skill**. The skill provides supporting scripts such as configuration preflight (`config_preflight`), context collection (`collect_context`), experience injection (`experience_injector`), and best-configuration export (`export_best_config`), and follows a unified behavioral contract for candidate generation, failure analysis, and adjudication. The typical workflow is as follows:

1. Complete the installation described in [Preparations](#4-preparations) and [Tool Installation](#5-tool-installation), and confirm that the deployment stack is available.
2. Enter Agent mode through the `optix-assistant` skill and choose fully automatic or interactive execution as prompted.
3. After the configuration preflight, the agent generates candidates, executes trials, and summarizes results round by round. After convergence, the best configuration and the final report are exported.
...
After automatic optimization is complete, a result file in CSV format is generated and stored in the `result/store` folder in the current directory. For details, see [Output File Description](#11-output-file-description).

## 11. Output File Description
...
|optimizer_strategy|No|Optimization mode, either `pso` (default) or `agent`. See [Optimization Modes: PSO and Agent](#9-optimization-modes-pso-and-agent).|
...
`REQUESTRATE=0` means unlimited traffic. Therefore, the maximum-pressure rate is `0` when the configured range contains `0`; otherwise, it is the numeric `max`. In Agent mode, this switch must be `false`. See [Optimization Modes: PSO and Agent](#9-optimization-modes-pso-and-agent).
