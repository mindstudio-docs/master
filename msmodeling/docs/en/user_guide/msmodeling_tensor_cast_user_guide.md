...
| --moe-tp-size                        | Parallelism Options | Optional | Specifies the TP size for experts, overriding `--tp-size`.<br>1. Type: Int.<br>2. Value range: positive integer.<br>3. Default: `None`.<br>4. Note: when the sequence parallel or dispatch_ffn_combine compilation options are enabled, expert parallelism requires this to be `1` (i.e. EP × moe_dp == tp × dp), otherwise config resolution fails. |
