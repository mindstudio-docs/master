...
2. 可选 `result/trial_logs/` 下的服务配置、benchmark 输出和日志备份。
...
| 备份目录 | `./result/trial_logs` | `--backup` 开启后创建 |
...
| `--backup` | 可选 | `False` | 是否将服务配置、benchmark 输出和日志备份到 `output/trial_logs` |
...
4. 若启用 `--backup`，可从 `output/trial_logs` 查看每轮变更前后的日志和配置。
