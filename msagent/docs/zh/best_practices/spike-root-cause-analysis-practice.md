...
![描述目标和数据](../figures/best_practices/spike_prompt.png)

#### 3.2.2 讨论确定分析步骤

和 agent 一起理清分析思路：先确认数据能否对比，再找可疑位置、锁定根因，最后逐层追查分叉点。

![讨论确定分析步骤](../figures/best_practices/spike_plan.png)

#### 3.2.3 找出候选尖刺位置（Phase 1）

从监控数据中圈出所有可疑的尖刺点，就像先在地图上标出异常区域。

![找出候选尖刺位置](../figures/best_practices/spike_phase1.png)

#### 3.2.4 锁定根因并和标杆设备对比（Phase 2）

从可疑点中确定最像根因的位置，再和 GPU 标杆对比，判断该异常属于设备特异还是双端共有。

![锁定根因并和标杆设备对比](../figures/best_practices/spike_phase2.png)

#### 3.2.5 追溯前反向分叉点（Phase 3）

逐层对比异常与正常数据，找到前向、反向计算最早开始出现差异的环节。

![追溯前反向分叉点](../figures/best_practices/spike_phase3.png)

#### 3.2.6 得到分析报告

agent 把结论、疑问和后续建议整理成一份完整报告，方便跟进处理。梯度尖刺定位的主要功能，是帮助使用者找到最早与标杆设备产生梯度差异的可疑分叉点，从而界定根因范围。

![得到分析报告](../figures/best_practices/spike_report.png)
