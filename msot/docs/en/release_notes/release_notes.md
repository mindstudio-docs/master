...
5. Enhanced software security by changing the permissions and ownership of build artifacts and on-drive files.
...
1. Fixed the issue where simt `ldk`/`stk` instructions were not written to the drive.
...
4. Fixed the issue where the CLI option name for call stack traceback did not conform to industry conventions, and updated the CLI option name.
