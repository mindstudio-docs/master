...
1. The tools in this code repository are designed for installation and use with low privileges. From the perspective of security and the principle of least privilege, none of the tools should be used with high-privilege accounts such as root. You are advised to install and execute them with standard user permissions.
...
3. Before using any tool in this code repository, you are advised to set `umask` to `0027` or higher to ensure that generated files meet the minimum privilege security requirements.

4. This tool is intended for development. It does not restrict the owner or permissions used for installation, nor does it restrict the owner or permissions of the file objects processed by the tool. Users must assign appropriate ownership and permissions based on the usage scenario and ensure that the content of files processed by the tool is secure and trustworthy.

5. When providing input to the tool, users must ensure that the content is secure and trustworthy, avoid using symbolic links, and convert every file to a real absolute path before providing it to the tool.

## File Permission Control

 1. When users provide input files to the tool, it is recommended that the file owner be consistent with the tool process owner, and the file permissions should not be modifiable by others (including group and others). By default, files written to drive by the tool are not writable by users in the same group or other users. Users can control permissions on the generated files as needed.

 2. Users need to properly control permissions during installation and usage. Users are advised to configure permissions by referring to the file permission requirements.
...
Please refer to the [MindStudio Vulnerability Mechanism Description](./vulnerability_handling_procedure.md).
...
3. During tool operation, the tool loads `.so` files from `LD_LIBRARY_PATH`. Before using the tool, users must ensure that the `LD_LIBRARY_PATH` environment variable contains secure and trustworthy content, that the referenced paths do not contain symbolic links, and that the permissions and ownership of those paths meet security expectations and cannot be tampered with by third parties. Otherwise, arbitrary code injection may occur.
...
This project is developed in Python and the source code is released. You are advised to directly use the public interfaces described in the documentation. Directly calling the source code of interfaces that are not explicitly made public is not recommended.

## Usage of Safe Functions

Insecure functions are not forcibly disabled, but you are advised to use the _s secure versions that explicitly pass the buffer length as a parameter, such as `memset_s`, `memcpy_s`, and so on.
