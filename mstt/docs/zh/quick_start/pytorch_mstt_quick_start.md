...
1. 完成[环境准备](#环境准备)。
2. 本样例选用ResNet50模型，以“pytorch_main.py”命名为例，创建训练脚本文件，脚本内容直接拷贝[PyTorch GPU环境训练脚本样例](#51-pytorch-gpu环境训练脚本样例)，确保该脚本在GPU环境下正常运行。
...
2. 在训练脚本中导入自动迁移的库代码。
   ...
   在[PyTorch GPU环境训练脚本样例](#51-pytorch-gpu环境训练脚本样例)中的24、25行添加自动迁移的库代码，即可在昇腾NPU环境下直接执行训练，如下所示。
   ...
   也可直接拷贝添加完成后的完整代码[PyTorch昇腾NPU环境训练脚本样例](#52-pytorch昇腾npu环境训练脚本样例)。
...
1. 获取两个环境的zip包（包含影响训练精度的环境配置：环境变量、第三方库版本、权重、数据集、随机函数等）。
   ...
   > [!NOTE]
   >
   > GPU和NPU环境可分别拷贝[PyTorch GPU训练前配置检查代码样例](#53-pytorch-gpu训练前配置检查代码样例)和[PyTorch昇腾NPU训练前配置检查代码样例](#54-pytorch昇腾npu训练前配置检查代码样例)中的完整代码执行。
...
2. 在训练脚本中添加工具，如下所示。
   ...
   > [!NOTE]
   >
   > 可直接拷贝[PyTorch训练状态监测代码样例](#55-pytorch训练状态监测代码样例)中的完整代码执行，下列仅为说明脚本中需要添加的工具接口。
...
4. 查看结果。
   ...
   **图1** 结果文件内容
   ...
   输出结果详细介绍请参见“[输出结果](https://gitcode.com/Ascend/msprobe/blob/master/docs/zh/user_guide/monitor_v2_instruct.md#%E8%BE%93%E5%87%BA%E7%BB%93%E6%9E%9C)”。
...
2. 分别在GPU和昇腾NPU环境下的训练脚本（pytorch_main.py文件）中添加工具，如下所示。
   ...
   > [!NOTE]
   >
   > GPU和昇腾NPU环境可直接拷贝[PyTorch精度数据采集代码样例](#56-pytorch精度数据采集代码样例)中的完整代码执行，其中GPU环境下执行训练时，脚本中不需要添加如下24、25行。
...
- 完成[环境准备](#环境准备)。
- 完成[精度数据采集](#33-精度数据采集)，得到PyTorch训练场景昇腾NPU环境的精度数据。
...
2. 启动预检。
   ...
   分别在GPU和昇腾NPU环境下使用**acc_check**命令执行预检操作。（预检场景GPU环境需要使用昇腾NPU环境拷贝的精度数据）

   ```bash
   msprobe acc_check -api_info /home/dump/step0/proc3209297/dump.json -o /home/checker_result/gpu
   ```

   ```bash
   msprobe acc_check -api_info /home/dump/step0/proc3209296/dump.json -o /home/checker_result/npu
   ```
...
4. 预检结果分析。
   ...
   - api_precision_compare_details_{timestamp}.csv文件会标明每个API是否通过测试，示例如下。
     ...
     **图3** api_precision_compare_details_2
     ...
     更多比对结果字段含义请参见“[预检结果比对](https://gitcode.com/Ascend/msprobe/blob/master/docs/zh/user_guide/accuracy_checker/pytorch_accuracy_checker_instruct.md#%E9%A2%84%E6%A3%80%E7%BB%93%E6%9E%9C%E6%AF%94%E5%AF%B9)”。
...
- 完成[环境准备](#环境准备)。
- 完成[精度数据采集](#33-精度数据采集)，得到GPU和昇腾NPU环境的精度数据。
...
1. 数据准备。
   ...
   dump_data_gpu目录下dump.json路径为`/home/dump/dump_data_gpu/step0/proc3209297/dump.json`。

   dump_data_npu目录下dump.json路径为`/home/dump/dump_data_npu/step0/proc3209296/dump.json`。
...
2. 执行比对。
   ...
   ```bash
   msprobe compare -tp /home/dump/dump_data_npu/step0/proc3209296/dump.json -gp /home/dump/dump_data_gpu/step0/proc3209297/dump.json -o /home/accuracy_compare
   ```
   ...
   ```ColdFusion
   ...
   The result file path is: /home/accuracy_compare/compare_result_{timestamp}.csv
   ************************************************************************************
...
3. 比对结果文件分析。
   ...
   更多比对结果分析请参见“[精度比对结果分析](https://gitcode.com/Ascend/msprobe/blob/master/docs/zh/user_guide/accuracy_compare/pytorch_accuracy_compare_instruct.md#%E7%B2%BE%E5%BA%A6%E6%AF%94%E5%AF%B9%E7%BB%93%E6%9E%9C%E5%88%86%E6%9E%90)”。
...
- 完成[精度数据采集](#33-精度数据采集)，得到GPU和昇腾NPU环境的精度数据。
  ...
  由于本样例在dump数据时"level"配置为"L1"，故采集到模型结构数据为空，分级可视化构图时需要将"level"配置为"mix"，并将训练脚本中的debugger.start()改为debugger.start(model)。
...
- 完成[环境准备](#环境准备)。
- 根据[模型开发&迁移](#2-模型开发迁移)中的PyTorch部分，得到可正常执行训练任务的GPU和昇腾NPU环境。

> [!NOTE]
>
> 在执行性能数据采集前请先将训练脚本（pytorch_main.py文件）中的[精度数据采集](#33-精度数据采集)相关接口删除，因为精度数据采集和性能数据采集不可同时执行。
...
1. 在昇腾NPU环境下的训练脚本（pytorch_main.py文件）中添加Ascend PyTorch Profiler接口工具，如下所示。
   ...
   > [!NOTE]
   >
   > 可直接拷贝[Ascend PyTorch Profiler接口采集性能数据代码样例](#57-ascend-pytorch-profiler接口采集性能数据代码样例)中的完整代码执行，下列仅为说明脚本中需要添加的工具接口。
...
3. 查看采集到的PyTorch训练性能数据结果文件。
   ...
   Ascend PyTorch Profiler接口采集的性能数据可以使用msTT的msprof-analyze工具进行辅助分析，也可以直接使用MindStudio Insight工具进行可视化分析，详细操作请参见[使用msprof-analyze工具分析性能数据](#421-使用msprof-analyze工具分析性能数据)和[使用MindStudio Insight工具可视化性能数据](#422-使用mindstudio-insight工具可视化性能数据)。
...
msprof-analyze的advisor功能是将Profiling采集并解析出的性能数据进行分析，并输出性能调优建议。
...
1. 安装MindStudio Insight。
   ...
   参见《[MindStudio Insight工具安装指南](https://gitcode.com/Ascend/msinsight/blob/master/docs/zh/install_guide/mindstudio_insight_install_guide.md)》下载并安装MindStudio Insight。
...
### 5.7 Ascend PyTorch Profiler接口采集性能数据代码样例
