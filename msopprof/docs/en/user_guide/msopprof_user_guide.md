...
| MC2 Operator Invocation Scenario | [MC2 Operator Invocation](./msopprof_usage.md#collecting-performance-data-of-mc2-operators) |
...
The `trace.json` file can be visualized using either the Chrome browser or MindStudio Insight, while the `visualize_data.bin` file can be visualized only using MindStudio Insight. For details about how to use the AscendC API for performance profiling, see **Debugging APIs** > **On-board Printing** > [PrintTimeStamp](https://www.hiascend.com/document/detail/en/canncommercial/850/API/ascendcopapi/atlasascendc_api_07_00002.html) in *Ascend C Operator Development API*.
