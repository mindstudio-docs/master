...
当前支持基于上板（msOpProf）或[仿真（msOpProf simulator）运行模式](./msopprof_simulator_user_guide.md)和不同文件形式（可执行文件或算子二进制.o文件）进行性能数据的采集和自动解析。

本文档介绍msOpProf上板运行模式的使用方法。
