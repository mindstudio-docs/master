...
```bash
cd ~/msserviceprofiler/artifacts
chmod +x mindstudio-service-profiler_<version>_<arch>.run
```
...
```text
mindstudio-msserviceprofiler package install success, the path is: '<install_path>'.
```

> [!NOTE]
>
> - 默认安装路径：root 用户为 `/usr/local/Ascend/cann`，非 root 用户为 `${HOME}/Ascend/cann`。
>
> - 如果要指定路径安装，则需添加`--install-path`，例如`./mindstudio-service-profiler_<version>_<arch>.run --install-path=./test --install`，则将此run包安装到当前目录下的test目录下。
>
> - 若系统中已安装该工具的旧版本，可使用 `--upgrade` 执行覆盖升级安装，升级路径通过 `--install-path` 或 `ASCEND_TOOLKIT_HOME` 环境变量指定。
