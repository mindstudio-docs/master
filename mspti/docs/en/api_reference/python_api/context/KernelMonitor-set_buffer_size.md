...
If `MsptiResult.MSPTI_SUCCESS` is returned, the operation is successful. If `MsptiResult.MSPTI_ERROR_INVALID_PARAMETER` is returned, the parameter setting is incorrect and the operation fails.
