# `CommunicationMonitor.set_buffer_size`<a name="ZH-CN_TOPIC_0000002267968082"></a>
...
Sets the size of the activity buffer before profiling starts to store the collected profile data.

During profiling, modifying the activity buffer size does not take effect. The modification takes effect only after the current profiling ends and the next profiling starts.
...
## Parameters<a name="section354791521716"></a>
...
<td class="cellrowborder" valign="top" width="72%" headers="mcps1.1.4.1.3 "><p id="p1871112011815"><a name="p1871112011815"></a><a name="p1871112011815"></a>Size of the activity buffer, in MB. The default value is 8 MB.</p>
<p id="p95300307247"><a name="p95300307247"></a><a name="p95300307247"></a>Only positive integers are supported. If you set any other invalid value, the function returns a failure and profiling uses the default activity buffer size.</p>
</td>
</tr>
</tbody>
</table>
...
The function returns `MsptiResult.MSPTI_SUCCESS` to indicate success. If it returns `MsptiResult.MSPTI_ERROR_INVALID_PARAMETER`, the parameter is set incorrectly and the setting fails.
