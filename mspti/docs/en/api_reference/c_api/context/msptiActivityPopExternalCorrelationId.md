...
`MSPTI_SUCCESS` indicates that the operation is successful. If the external correlation type is invalid, `MSPTI_ERROR_INVALID_PARAMETER` is returned. If the external correlation ID stack is empty, `MSPTI_ERROR_QUEUE_EMPTY` is returned when an ID is popped from the stack, indicating a failure.
