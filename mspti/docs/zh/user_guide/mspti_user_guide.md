...
本文档面向希望使用 msPTI（MindStudio Profiler Tools Interface）进行 NPU 应用性能分析的用户。msPTI 提供三种编程接口，分别适用于不同开发语言和分析场景。建议新用户先阅读《[msPTI 快速入门](../quick_start/mspti_quick_start.md)》完成环境准备，再根据自身需求选择对应的接口指南。
...
| 阶段 | 参考文档 | 说明 |
| --- | --- | --- |
| 环境准备与工具安装 | 《[msPTI 快速入门](../quick_start/mspti_quick_start.md)》 | 首次使用必读：硬件要求、软件安装、环境配置 |
